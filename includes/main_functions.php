<?php
require_once apm_BASE_DIR . '/includes/backcompat_functions.php';
require_once apm_BASE_DIR . '/includes/deprecated_functions.php';
require_once apm_BASE_DIR . '/includes/cleanup_functions.php';
require_once apm_BASE_DIR . '/lib/adodb/adodb.inc.php';
require_once apm_BASE_DIR . '/classes/apm/apmProject.php';
require_once apm_BASE_DIR . '/vendor/autoload.php';

spl_autoload_register ( 'apmProject_autoload' );
spl_autoload_register ( 'apm_old_autoload' );

/**
 * For all intents and purposes, this autoloader should be considered
 * deprecated.
 * As we move forward, we'll continue to simplify and clean this
 * up so that it should eventually be nothing except the module autoloader.
 */
function apm_old_autoload($class_name) {
	...
	switch ($name) {
		...
		case 'capmobject' : // Deprecated as of v3.0, TODO: remove this in v4.0
		...
		case 'apmacl' : // Deprecated as of v3.0, TODO: remove this in v4.0
		case 'apmajaxresponse' : // Deprecated as of v3.0, TODO: remove this in v4.0
			require_once apm_BASE_DIR . '/classes/deprecated.class.php';
			break;
		 ...
		 */
		case 'captcha' :
			require_once apm_BASE_DIR . '/lib/captcha/Captcha.class.php';
			break;
		case 'contact_vcard_build' :
			require_once apm_BASE_DIR . '/lib/PEAR/Contact_Vcard_Build.php';
			break;
		case 'contact_vcard_parse' :
			require_once apm_BASE_DIR . '/lib/PEAR/Contact_Vcard_Parse.php';
			break;
		case 'date' :
		case 'date_calc' :
		case 'date_human' :
		...
		case 'html_bbcodeparser_filter_lists' :
			...
			require_once apm_BASE_DIR . '/lib/PEAR/' . $filename;
			break;
		case 'html_bbcodeparser' :
			require_once apm_BASE_DIR . '/lib/PEAR/BBCodeParser.php';
			break;
		case 'cezpdf' :
			require_once apm_BASE_DIR . '/lib/ezpdf/class.ezpdf.php';
			break;
		case 'cpdf' :
			require_once apm_BASE_DIR . '/lib/ezpdf/class.pdf.php';
			break;
		case 'gacl' :
			require_once apm_BASE_DIR . '/lib/phpgacl/gacl.class.php';
			break;
		case 'gacl_api' :
			require_once apm_BASE_DIR . '/lib/phpgacl/gacl_api.class.php';
			break;
		case 'ganttgraph' :
			require_once apm_BASE_DIR . '/lib/jpgraph/src/jpgraph.php';
			require_once apm_BASE_DIR . '/lib/jpgraph/src/jpgraph_gantt.php';
			break;
		case 'phpmailer' :
			require_once apm_BASE_DIR . '/lib/PHPMailer/class.phpmailer.php';
			break;
		case 'xajax' :
			require_once apm_BASE_DIR . '/lib/xajax/xajax_core/xajax.inc.php';
			break;
		case 'xajaxresponse' :
			require_once apm_BASE_DIR . '/lib/xajax/xajax_core/xajaxResponse.inc.php';
			break;
		...
		default :
			...
			$pieces = (strpos ( $name, '_' ) === false) ? array (
					$name,
					$name 
			) : explode ( '_', $name );
			 ...
			 */
			$plural_pieces = array_map ( 'apm_pluralize', $pieces );
			if ('systems' == $plural_pieces [0]) {
				$plural_pieces [0] = 'system';
			}
			$path = implode ( '/', $plural_pieces );
			if (file_exists ( apm_BASE_DIR . '/modules/' . $path . '.class.php' )) {
				require_once apm_BASE_DIR . '/modules/' . $path . '.class.php';
 ...
 * @param null $default
 *        	string The default value to return if the key not found.
 * @return null The value of the setting, or the default value if not found.
 */
function apmgetConfig($key, $default = null) {
	global $apmconfig;
	
	if (isset ( $apmconfig [$key] )) {
		return $apmconfig [$key];
	} else {
		return $default;
	}
}
 ...
 */
function apmgetParam(&$arr, $name, $def = null) {
...
function convert2days($durn, $units) {
	switch ($units) {
		case 0 :
		case 1 :
			return $durn / apmgetConfig ( 'daily_working_hours' );
			break;
		case 24 :
			return $durn;
	}
...
function apmFindTaskComplete($start_date, $end_date, $percent) {
...
function apm_check_email($email) {
	return filter_var ( $email, FILTER_VALIDATE_EMAIL );
}
function apm_textarea($content) {
	$result = '';
...
function notifyNewExternalUser($emailAddress, $username, $logname, $logpwd, $emailUtility = null) {
	...
	$mail = (! is_null ( $emailUtility )) ? $emailUtility : new apm_Utilities_Mail ();
 ...
 * @param
 *        	$auth_mode
 * @return apm_Authenticators_Base
 */
function &getAuth($auth_mode) {
	switch ($auth_mode) {
		case 'ldap' :
			$auth = new apm_Authenticators_LDAP ();
			break;
		case 'pn' :
			$auth = new apm_Authenticators_PostNuke ();
			break;
		default :
