...
function notifyHR($address, $notUsed, $uaddress, $uusername, $logname, $notUsed2, $userid) {
	global $AppUI;
	$emailManager = new apm_Output_EmailManager ( $AppUI );
	$body = $emailManager->notifyHR ( $uusername, $logname, $uaddress, $userid );
	
	$mail = new apm_Utilities_Mail ();
	$mail->To ( $address );
...
function notifyNewUserCredentials($address, $username, $logname, $logpwd) {
	...
	$mail = new apm_Utilities_Mail ();
	$mail->To ( $address );
	$mail->Subject ( 'New Account Created - apmProject Project Management System' );
	$mail->Body ( $body );
	return $mail->Send ();
}
...
function buildTaskTree($task_data, $depth = 0, $projTasks, $all_tasks, $parents, $task_parent, $task_id) {
	...
	$output .= '<option value="' . $task_data ['task_id'] . '"' . $selected . '>' . str_repeat ( '&nbsp;', $depth * 3 ) . apmFormSafe ( $task_data ['task_name'] ) . '</option>';
...
function build_date_list(&$date_array, $row) {
	...
	if (! in_array ( $row ['task_dynamic'], CTask::$tracked_dynamics )) {
		$date = new apm_Utilities_Date ( $project->project_start_date );
	} elseif ($row ['task_milestone'] == 0) {
		$date = new apm_Utilities_Date ( $row ['task_end_date'] );
	} else {
		$date = new apm_Utilities_Date ( $row ['task_start_date'] );
	}
	$sdate = $date->format ( '%d/%m/%Y' );
	$shour = $date->format ( '%H' );
	$smin = $date->format ( '%M' );
	
	$date_array [$row ['task_id']] = array (
			$row ['task_name'],
			$sdate,
			$shour,
			$smin 
...
function __extract_from_showtask2($arr, $level, $today_view, $s, $m, $jsTaskId, $expanded) {
	...
	for($y = 0; $y < $level; $y ++) {
		if ($y + 1 == $level) {
			$image = apmfindImage ( 'corner-dots.gif', $m );
		} else {
			$image = apmfindImage ( 'shim.gif', $m );
		}
		$s .= '<img src="' . $image . '" width="16" height="12"  border="0" alt=""/>';
	}
	if ($arr ['task_description'] && ! $arr ['task_milestone']) {
		$s .= apmtoolTip ( 'Task Description', substr ( $arr ['task_description'], 0, 1000 ), true );
	}
	...
	if ($arr ['task_milestone'] > 0) {
		$s .= '&nbsp;<a href="./index.php?m=tasks&amp;a=view&amp;task_id=' . $arr ['task_id'] . '" ><b>' . $arr ['task_name'] . '</b></a>&nbsp;<img src="' . apmfindImage ( 'icons/milestone.gif' ) . '" />';
	} elseif ($arr ['task_dynamic'] == '1' || $is_parent) {
		$open_link = '<a href="javascript: void(0);"><img onclick="expand_collapse(\'' . $jsTaskId . '\', \'tblProjects\',\'\',' . ($level ++) . ');" id="' . $jsTaskId . '_collapse" src="' . apmfindImage ( 'icons/collapse.gif' ) . '" class="center" ' . (! $expanded ? 'style="display:none"' : '') . ' /><img onclick="expand_collapse(\'' . $jsTaskId . '\', \'tblProjects\',\'\',' . ($level ++) . ');" id="' . $jsTaskId . '_expand" src="' . apmfindImage ( 'icons/expand.gif' ) . '" class="center" ' . ($expanded ? 'style="display:none"' : '') . ' /></a>';
		$s .= $open_link;
...
function calcEndByStartAndDuration($task) {
	$end_date = new apm_Utilities_Date ( $task ['task_start_date'] );
...
function sort_by_item_title($title, $item_name, $item_type, $a = '') {
	...
	if ($show_icon) {
		$s .= '&nbsp;<img src="' . apmfindImage ( 'arrow-' . (($item_order == SORT_ASC) ? 'up' : 'down') . '.gif' ) . '" />';
	}
...
function displayTask($list, $task, $level, $display_week_hours, $fromPeriod, $toPeriod, $user_id) {
	...
	if ($level >= 1) {
		$tmp .= '<img src="' . apmfindImage ( 'corner-dots.gif' ) . '" width="16" height="12" alt="" style="float: left;">';
	}
	...
	$mod = ($task->task_priority > 0) ? 'up' : 'down';
	
	$tmp .= '<label class="label label-danger"><span class="glyphicon glyphicon-arrow-' . $mod . '"></span></label>';
	...
	$tmp .= '<select name="add_users" style="width:200px" size="' . $size . '" class="form-control" multiple="multiple" ondblclick="javascript:chAssignment(' . $user_id . ', 0, false)">';
...
function weekDates($display_allocated_hours, $fromPeriod, $toPeriod) {
	...
	$s = new apm_Utilities_Date ( $fromPeriod );
	$e = new apm_Utilities_Date ( $toPeriod );
	$sw = getBeginWeek ( $s );
...
function displayWeeks($list, $task, $level, $fromPeriod, $toPeriod) {
	...
	$st = new apm_Utilities_Date ( $task->task_start_date );
	$et = new apm_Utilities_Date ( $task->task_end_date );
	$stw = getBeginWeek ( $st );
...
function showRoleRows($roles = null) {
	...
	foreach ( $roles as $role ) {
		...
		$s .= '<tr><td>' . $name . '</td><td >' . $AppUI->_ ( $description ) . '</td>';
		$s .= '<td>';
		if ($canEdit) {
			
			$s .= '<a class="btn btn-xs btn-info" href="?m=system&u=roles&role_id=' . $id . '" role="button">
			<span class="glyphicon glyphicon-edit" data-placement="right" data-toggle="tooltip" data-container="body" data-original-title="' . $AppUI->_ ( 'Edit role ID or description' ) . '"></span>
			</a>';
			
			$s .= '<a class="btn btn-xs btn-default" href="?m=system&u=roles&a=viewrole&role_id=' . $id . '" role="button">
			<span class="glyphicon glyphicon-cog" data-placement="right" data-toggle="tooltip" data-container="body" data-original-title="' . $AppUI->_ ( 'Edit role permisions' ) . '"></span>
			</a>';
		}
		if ($canDelete && strpos ( $name, 'admin' ) === false) {
			$s .= '<a class="btn btn-xs btn-danger" href="javascript:delIt(' . $id . ')" role="button">
			<span class="glyphicon glyphicon-remove" data-placement="right" data-toggle="tooltip" data-container="body" data-original-title="' . $AppUI->_ ( 'Delete role' ) . '"></span>
			</a>';
		}
	}
...
function showRoleForm($roles, $role_id) {
	global $canEdit, $canDelete, $role_id, $AppUI, $roles;
	
	if (! empty ( $role_id )) {
		$key = array_search ( $role_id, array_column ( $roles, 'id' ) );
		$id = $roles [$key] ['id'];
		$name = $roles [$key] ['value'];
	...
	$s .= '<div class="table-responsive">
		...
		<thead>
		<tr><td colspan="2"><span class="title-icon glyphicon glyphicon-th-list"></span>' . $AppUI->_ ( $role_id ? 'Edit role' : 'Add role' ) . '</td></tr>
		</thead>
		<tbody>';
	...
	$s .= '<input type="hidden" name="role_id" value="' . $role_id . '" />';
	$s .= '<tr><td>' . $AppUI->_ ( 'Role ID' ) . '</td><td><input type="text" size="20" name="role_name" value="' . $name . '" class="form-control" /></td></tr>';
	$s .= '<tr><td>' . $AppUI->_ ( 'Description' ) . '</td><td><input type="text" size="50" name="role_description" class="form-control" value="' . $description . '"></td></tr>';
	$s .= ($role_id ? '' : '<tr><td>' . $AppUI->_ ( 'Copy role' ) . '</td><td>' . arraySelect ( $roles_arr, 'copy_role_id', 'class="form-control"', 0, true ) . '</td></tr>');
	$s .= '</tbody></table></div>';
	$s .= '<div><input type="submit" value="' . $AppUI->_ ( $role_id ? 'save' : 'add' ) . '" class="btn btn-info" /></div>';
	$s .= '</div>';
...
<tr>
    <td width=40>
        <a href="?m=system&amp;a=billingcode&amp;company_id=' . $company_id . '&amp;billingcode_id=' . $a ['billingcode_id'] . '" title="' . $AppUI->_ ( 'edit' ) . '">
            <img src="' . apmfindImage ( 'icons/stock_edit-16.png' ) . '" alt="Edit" /></a>';
	
	if ($a ['billingcode_status'] == 0)
		$s .= '<a href="javascript:delIt2(' . $a ['billingcode_id'] . ');" title="' . $AppUI->_ ( 'delete' ) . '">
            <img src="' . apmfindImage ( 'icons/stock_delete-16.png' ) . '" alt="Delete" /></a>';
...
function getFolders($parent) {
	...
	foreach ( $folders as $row ) {
		if (array_key_exists ( $row ['file_folder_id'], $allowed_folders_ary ) or array_key_exists ( $parent, $allowed_folders_ary )) {
			...
			$s .= '<tr><td colspan="20">';
			$s .= '<ul>';
			$s .= '<li><a href="./index.php?m=files&amp;a=addedit_folder&amp;file_folder_parent=' . $row ['file_folder_id'] . '&amp;file_folder_id=0">' . apmshowImage ( 'edit_add.png', '', '', 'new folder', 'add a new subfolder', 'files' ) . '</a></li>';
			$s .= '<li><a href="./index.php?m=files&amp;a=addedit&amp;folder=' . $row ['file_folder_id'] . '&amp;project_id=' . $project_id . '&amp;file_id=0">' . apmshowImage ( 'folder_new.png', '', '', 'new file', 'add new file to this folder', 'files' ) . '</a></li>';
			$s .= '<li><a href="./index.php?m=files&amp;a=addedit_folder&amp;folder=' . $row ['file_folder_id'] . '">' . apmshowImage ( 'filesaveas.png', '', '', 'edit icon', 'edit this folder', 'files' ) . '</a></li>';
			if ($m == 'files') {
				$s .= '<li class="info-text"><a href="./index.php?m=' . $m . '&amp;a=' . $a . '&amp;tab=' . $tab . '&folder=' . $row ['file_folder_id'] . '" name="ff' . $row ['file_folder_id'] . '">';
			}
			$s .= apmshowImage ( 'folder5_small.png', '22', '22', '', '', 'files' );
			...
			$s .= '</ul>';
			$s .= '<a class="small-delete" href="javascript: void(0);" onclick="if (confirm(\'Are you sure you want to delete this folder?\')) {document.frm_remove_folder_' . $row ['file_folder_id'] . '.submit()}">' . apmshowImage ( 'remove.png', '', '', 'delete icon', 'delete this folder', 'files' ) . '</a>';
			$s .= '</td></tr>';
			if ($file_count > 0) {
...
function displayFiles($AppUI, $folder_id, $task_id, $project_id, $company_id) {
	...
	echo $listTable->buildHeader ( $fieldNames );
	$customLookups = array (
			'file_category' => $file_types 
	);
...
/* 			$style = 'color:#000;';
			$s .= '<tr>';
			$s .= '<td colspan="20" style="text-align: left; ' . $style . '">';
		...
		$row['file_project_name']=array( $href, $latest_file ['project_name']);
		...
		if ($canEdit && (empty ( $latest_file ['file_checkout'] ) || ($latest_file ['file_checkout'] == 'final' && ($canEdit || $latest_file ['project_owner'] == $AppUI->user_id)))) {
			$s .= '<a class="btn btn-xs btn-info" role="button" href="./index.php?m=files&a=addedit&file_id=' . $latest_file ['file_id'] . '"><span class="glyphicon glyphicon-edit"></span></a>';
		}
		
		if ($canEdit && empty ( $latest_file ['file_checkout'] )) {
			$s .= '<a class="btn btn-xs btn-default" role="button" href="?m=files&a=co&file_id=' . $latest_file ['file_id'] . '"><span class="glyphicon glyphicon-arrow-up"></span></a>';
		} else {
			if ($latest_file ['file_checkout'] == $AppUI->user_id) {
				$s .= '<a class="btn btn-xs btn-default" role="button" href="?m=files&a=addedit&ci=1&file_id=' . $latest_file ['file_id'] . '"><span class="glyphicon glyphicon-arrow-down"></span></a>';
			} else {
		...
		if ($row ['file_versions'] > 1) {
			...
			$version_link = '<button type="button" class="btn btn-default btn-xs" data-toggle="modal" data-target="#versionFileModal' . $key . '" data-original-title="' . $AppUI->_ ( "Tools" ) . '">
			<span class="glyphicon glyphicon-cog"></span></button>';
			$hidden_table = '';
			
			$versionTable [$key] = new apm_Output_ListTable ( $AppUI );
			$hidden_table .= $versionTable [$key]->startTable ( 'bindedToolbar' );
			$hidden_table .= $versionTable [$key]->buildHeader ( $fieldNames );
			
			$sub_htmlHelper = new apm_Output_HTMLHelper ( $AppUI );
			...
			foreach ( $file_versions as $file ) {
				...
				if ($file ['file_version_id'] == $latest_file ['file_version_id']) {
					foreach ( $fieldList as $index => $column ) {
						$cells2 [$index] = $sub_htmlHelper->createCell ( $fieldList [$index], $file [$fieldList [$index]], $customLookups );
					}
					
					if ($canEdit) {
						$file ['file_options'] .= '<a class="btn btn-xs btn-info" role="button" href="./index.php?m=files&a=addedit&file_id=' . $file ['file_id'] . '"><span class="glyphicon glyphicon-edit"></span></a>';
					}
					if (canDelete ( 'files' )) {
						$file ['file_options'] .= '<form name="frm_remove_file_' . $latest_file ['file_id'] . '" action="?m=files" method="post" accept-charset="utf-8">
						<input type="hidden" name="dosql" value="do_file_aed" />
						<input type="hidden" name="del" value="1" />
						<input type="hidden" name="file_id" value="' . $file ['file_id'] . '" />
						<input type="hidden" name="redirect" value="' . $current_uri . '" />
						</form>';
						$file ['file_options'] .= '<a class="btn btn-xs btn-danger" role="button" href="javascript: void(0);" onclick="if (confirm(\'' . $AppUI->_ ( 'Are you sure you want to delete this file?' ) . '\')) {document.frm_remove_file_' . $file ['file_id'] . '.submit()}"><span class="glyphicon glyphicon-remove"></span></a>';
						// $s .= $hidden_table;
					}
				}
				$file ['file_version'] = array (
						0 => $file ['file_version'] 
				);
				$vfile [] = $file;
			}
			
			$hidden_table .= $versionTable [$key]->buildRows ( $vfile, $customLookups );
			$hidden_table .= $versionTable [$key]->endTable ();
			
			$modal [] = '<div class="modal fade" id="versionFileModal' . $key . '" tabindex="-1" role="dialog" aria-labelledby="versionFileModalLabel' . $key . '" aria-hidden="true">
        	<div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header">
        	<button type="button" class="close" data-dismiss="modal" aria-label="' . $AppUI->_ ( "Close" ) . '"><span aria-hidden="true">&times;</span></button>
        	<h4 class="modal-title">' . $AppUI->_ ( "Versions of file" ) . ' "' . $AppUI->_ ( $row ['file_name'] ) . '"</h4>
        	</div><div class="modal-body">' . $hidden_table . '</div></div></div></div>';
		}
		if (canDelete ( 'files' )) {
			...
			$s .= '<a class="btn btn-xs btn-danger" role="button" href="javascript: void(0);" onclick="if (confirm(\'' . $AppUI->_ ( 'Are you sure you want to delete this file?' ) . '\')) {document.frm_remove_file_' . $latest_file ['file_id'] . '.submit()}"><span class="glyphicon glyphicon-remove"></span></a>';
			// $s .= $hidden_table;
		}
		
		$row ['file_version'] = array (
				0 => $row ['file_version'],
				1 => '<span class="label label-warning">(' . $row ['file_versions'] . ')</span>' . $version_link 
		);
		$row ['file_options'] = $s;
	...
	echo $listTable->buildRows ( $files, $customLookups );
...
function getIcon($file_type) {
	...
	if (is_file ( apm_BASE_DIR . '/styles/' . $uistyle . '/images/modules/files/icons/' . $icon . '.png' )) {
		$result = $icon;
	} else {
...
function file_show_attr($AppUI, $form) {
	...
	if ($ci) {
		$str_out = '<tr><td>' . $form->addLabel ( 'Minor Revision' ) . '</td><td><input type="Radio" name="revision_type" value="minor" checked /></td></tr>';
		$str_out .= '<tr><td>' . $form->addLabel ( 'Major Revision' ) . '<input type="Radio" name="revision_type" value="major" /></td></tr>';
	} else {
		$str_out = '<tr><td>' . $form->addLabel ( 'Version' ) . '</td><td>';
	}
	...
	if ($ci) {
		...
		$str_out .= '<input type="text" name="file_version" maxlength="10" size="5" value="' . $the_value . '" class="form-control" />';
	}
	...
	// Category
	$str_out .= '<tr><td>' . $form->addLabel ( 'Category' ) . '</td><td>';
	$str_out .= arraySelect ( apmgetSysVal ( 'FileType' ), 'file_category', 'class="form-control"' . $select_disabled, $object->file_category, true ) . '</td></tr>';
	
	// ---------------------------------------------------------------------------------
	
	$str_out .= '<tr><td>' . $form->addLabel ( 'Project' ) . '</td><td>';
	$str_out .= projectSelectWithOptGroup ( $AppUI->user_id, 'file_project', 'size="1" class="form-control"' . $select_disabled, $file_project ) . '</td></tr>';
	
	// ---------------------------------------------------------------------------------
	
	// Task
	$str_out .= '<tr><td>' . $form->addLabel ( 'Task' ) . '</td><td>';
	$str_out .= '<input type="hidden" name="file_task" value="' . $file_task . '" /><input type="text" class="form-control" name="task_name" value="' . $task_name . '" size="40" disabled /><input type="button" class="btn btn-default" value="' . $AppUI->_ ( 'select task' ) . '..."' . $onclick_task . '/></td></tr>';
 ...
 *        	int Project_id
 * @param
 *        	int SQL-limit to limit the number of returned tasks
...
function projects_list_data($user_id = false) {
	...
	$fields = apm_System_Module::getSettings ( 'projects', 'index_list' );
	...
	$buffer = '<select name="department" id="department" onChange="document.pickCompany.submit()" class="form-control" style="width: 200px;">';
	$company = '';
 ...
 *        	array list of project elements to search
 * @param mixed $project_id
 *        	project id to search for
 ...
 *        	the id of the company we are searching departments
 * @param mixed $checked_array
 *        	an array with the ids of the departments that should be selected on the list
 * @param integer $dept_parent
 *        	used when to determine the starting level on the tree, or by recursion
 * @param integer $spaces
 *        	used by recursion to add spaces to form the visual tree on the <select> element
...
function userUsageWeeks() {
	...
	$ted = new apm_Utilities_Date ( Date_Calc::endOfWeek ( $task_end_date->day, $task_end_date->month, $task_end_date->year ) );
	$tsd = new apm_Utilities_Date ( Date_Calc::beginOfWeek ( $task_start_date->day, $task_start_date->month, $task_start_date->year ) );
	
	$week_difference = $end_date->workingDaysInSpan ( $start_date ) / count ( explode ( ',', apmgetConfig ( 'cal_working_days' ) ) );
...
function showWeeks() {
	...
	$ed = new apm_Utilities_Date ( Date_Calc::endOfWeek ( $end_date->day, $end_date->month, $end_date->year ) );
	$sd = new apm_Utilities_Date ( Date_Calc::beginOfWeek ( $start_date->day, $start_date->month, $start_date->year ) );
	
	$week_difference = ceil ( $ed->workingDaysInSpan ( $sd ) / count ( explode ( ',', apmgetConfig ( 'cal_working_days' ) ) ) );
	...
	for($i = 0; $i < $week_difference; $i ++) {
		$actual_date->addSeconds ( 168 * 3600 ); // + one week
		$working_days_count = $working_days_count + count ( explode ( ',', apmgetConfig ( 'cal_working_days' ) ) );
	}
	...
	foreach ( $user_list as $user_id => $user_data ) {
		...
		if (isset ( $user_usage [$user_id] )) {
			...
			$average_user_usage = number_format ( ($array_sum / ($week_difference * count ( explode ( ',', apmgetConfig ( 'cal_working_days' ) ) ) * apmgetConfig ( 'daily_working_hours' ))) * 100, 2 );
	...
	$total_hours_capacity = $working_days_count / 2 * apmgetConfig ( 'daily_working_hours' ) * count ( $user_usage );
	$total_hours_capacity_all = $working_days_count / 2 * apmgetConfig ( 'daily_working_hours' ) * count ( $user_list );
}
...
function showDays() {
	...
	foreach ( $user_list as $user_id => $user_data ) {
		...
		if (isset ( $user_usage [$user_id] )) {
			$table_rows .= '<tr><td nowrap="nowrap">(' . $user_data ['user_username'] . ') ' . $user_data ['contact_first_name'] . ' ' . $user_data ['contact_last_name'] . '</td>';
			$array_sum = array_sum ( $user_usage [$user_id] );
			$average_user_usage = number_format ( ($array_sum / ($working_days_count * apmgetConfig ( 'daily_working_hours' ))) * 100, 2 );
			$allocated_hours_sum += $array_sum;
	...
	$total_hours_capacity = $working_days_count * apmgetConfig ( 'daily_working_hours' ) * count ( $user_usage );
	$total_hours_capacity_all = $working_days_count * apmgetConfig ( 'daily_working_hours' ) * count ( $user_list );
}
...
function showRow($id = '', $key = 0, $title = '', $value = '') {
	...
	if (($sysval_id == $title) && $canEdit) {
		...
		$s .= '<td valign="top"><a name="' . $title . '"> </a>' . arraySelect ( $keys, 'sysval_key_id', 'size="1" class="form-control"', $key ) . '</td>';
		$s .= '<td valign="top"><input type="text" name="sysval_title" value="' . apmformSafe ( $title ) . '" class="form-control" /></td>';
		$s .= '<td valign="top"><textarea name="sysval_value" class="small" rows="5" cols="40">' . $value . '</textarea></td>';
		$s .= '<td><input type="submit" value="' . $AppUI->_ ( $id ? 'save' : 'add' ) . '" class="btn btn-info btn-xs" /></td><td>&nbsp;</td>';
	} else {
		$s = '<tr><td width="12" valign="top">';
		if ($canEdit) {
			$s .= '<a href="?m=system&u=syskeys&sysval_id=' . $title . '#' . $title . '" title="' . $AppUI->_ ( 'edit' ) . '">' . apmshowImage ( 'icons/stock_edit-16.png', 16, 16, '' ) . '</a></td>';
		}
		$s .= '<td valign="top">' . $keys [$key] . '</td>';
		$s .= '<td valign="top">' . apmformSafe ( $title ) . '</td>';
		$s .= '<td valign="top" colspan="2">' . $value . '</td>';
		$s .= '<td valign="top" width="16">';
		if ($canEdit && ! in_array ( $title, $fixedSysVals )) {
			$s .= '<a href="javascript:delIt(\'' . $title . '\')" title="' . $AppUI->_ ( 'delete' ) . '">' . apmshowImage ( 'icons/stock_delete-16.png', 16, 16, '' ) . '</a>';
		}
		$s .= '</td>';
	}
	$s .= '</tr>';
...
function showRow_keys($id = 0, $name = '', $label = '') {
	...
	if ($syskey_id == $id && $canEdit) {
		...
		$s .= '<tr>';
		$s .= '<td>&nbsp;</td>';
		$s .= '<td><input type="text" name="syskey_name" value="' . $name . '" class="form-control" /></td>';
		$s .= '<td><textarea name="syskey_label" class="small" rows="2" cols="40">' . $label . '</textarea></td>';
		$s .= '<td><input type="submit" value="' . $AppUI->_ ( $id ? 'edit' : 'add' ) . '" class="btn btn-info btn-xs" /></td>';
		$s .= '<td>&nbsp;</td>';
	} else {
		$s .= '<tr>';
		$s .= '<td width="12">';
		if ($canEdit) {
			$s .= '<a href="?m=system&u=syskeys&a=keys&syskey_id=' . $id . '"><img src="' . apmfindImage ( 'icons/pencil.gif' ) . '" alt="edit" /></a>';
			$s .= '</td>' . $CR;
		}
		$s .= '<td>' . $name . '</td>' . $CR;
		...
		$s .= '<td width="16">';
		if ($canEdit) {
			$s .= '<a href="javascript:delIt(' . $id . ')"><img align="absmiddle" src="' . apmfindImage ( 'icons/trash.gif' ) . '" alt="' . $AppUI->_ ( 'delete' ) . '" /></a>';
		}
		$s .= '</td>' . $CR;
	}
	$s .= '</tr>' . $CR;
...
function arraySelect(&$arr, $select_name, $select_attribs, $selected, $translate = false) {
	...
	reset ( $arr );
	$s = '<select id="' . $select_name . '" class="form-control" name="' . $select_name . '" ' . $select_attribs . '>';
...
function breadCrumbs(&$arr) {
	...
	$crumbs = array ();
	foreach ( $arr as $k => $v ) {
		$crumbs [] = '<a class="btn btn-default" href="' . $k . '"><span>' . $AppUI->_ ( $v ) . '</span></a>';
	}
	
	return implode ( '</td><td align="left" nowrap="nowrap">', $crumbs );
}
function apmgetUsers() {
	global $AppUI;
...
function apmgetUsersList($stub = null, $where = null, $orderby = 'contact_first_name, contact_last_name') {
	$q = getUsers ( $stub, $where, $orderby );
	return $q->loadList ();
}
function apmgetUsersHashList($stub = null, $where = null, $orderby = 'contact_first_name, contact_last_name') {
	$q = getUsers ( $stub, $where, $orderby );
...
// #
function apmshowModuleConfig($config) {
	global $AppUI;
 ...
 * @param
 *        	string Optional name of the current module
 */
function apmfindImage($name, $module = null) {
	// uistyle must be declared globally
	global $uistyle;
	if ($module && file_exists ( apm_BASE_DIR . '/modules/' . $module . '/images/' . $name )) {
		return './modules/' . $module . '/images/' . $name;
	} elseif ($module && file_exists ( apm_BASE_DIR . '/style/' . $uistyle . '/images/modules/' . $module . '/' . $name )) {
		return './style/' . $uistyle . '/images/modules/' . $module . '/' . $name;
	} elseif (file_exists ( apm_BASE_DIR . '/style/' . $uistyle . '/images/icons/' . $name )) {
		return './style/' . $uistyle . '/images/icons/' . $name;
	} elseif (file_exists ( apm_BASE_DIR . '/style/' . $uistyle . '/images/obj/' . $name )) {
		return './style/' . $uistyle . '/images/obj/' . $name;
	} elseif (file_exists ( apm_BASE_DIR . '/style/' . $uistyle . '/images/' . $name )) {
		return './style/' . $uistyle . '/images/' . $name;
	} elseif ($module && file_exists ( apm_BASE_DIR . '/style/' . apmgetConfig ( 'host_style' ) . '/images/modules/' . $module . '/' . $name )) {
		return './style/' . apmgetConfig ( 'host_style' ) . '/images/modules/' . $module . '/' . $name;
	} elseif (file_exists ( apm_BASE_DIR . '/style/' . apmgetConfig ( 'host_style' ) . '/images/icons/' . $name )) {
		return './style/' . apmgetConfig ( 'host_style' ) . '/images/icons/' . $name;
	} elseif (file_exists ( apm_BASE_DIR . '/style/' . apmgetConfig ( 'host_style' ) . '/images/obj/' . $name )) {
		return './style/' . apmgetConfig ( 'host_style' ) . '/images/obj/' . $name;
	} elseif (file_exists ( apm_BASE_DIR . '/style/modern/images/obj/' . $name )) {
		return './style/modern/images/obj/' . $name;
	} else {
		return './style/modern/images/' . $name;
	}
}
 ...
 *        	string The name of the image
 * @param
 *        	string The image width
 * @param
 *        	string The image height
 * @param
 *        	string The alt text for the image
 */
function apmshowImage($src, $notUsed = '', $notUsed2 = '', $alt = '', $title = '', $module = null) {
	global $m;
	
	if ($src == '') {
		return '';
	} elseif ($module) {
		$src = apmfindImage ( $src, $module );
	} else {
		$src = apmfindImage ( $src, $m );
	}
	
	if (! $alt && ! $title) {
		$result = '';
	} elseif ($alt && $title) {
		$result = apmtoolTip ( $alt, $title );
	} elseif ($alt && ! $title) {
		$result = apmtoolTip ( $m, $alt );
	} elseif (! $alt && $title) {
		$result = apmtoolTip ( $m, $title );
	}
...
function apmshowIcon($src, $notUsed = '', $notUsed2 = '', $alt = '', $title = '', $module = null) {
	global $m;
	...
	$result .= '<span class="glyphicons glyphicons-' . $src . '"></span>';
	if ($alt || $title) {
		$result .= apmendTip ();
	}
...
function buildPaginationNav($AppUI, $m, $tab, $xpg_totalrecs, $xpg_pagesize, $page) {
	...
	$xpg_break = false;
	$s = '<div class="pagination"><ul class="pagination pagination-sm">';
	...
	if ($xpg_totalrecs > $xpg_pagesize) {
		...
		// left buttoms
		if ($xpg_prev_page > 0) {
			$s .= '<li><a href="./index.php?m=' . $m . '&amp;tab=' . $tab . '&amp;page=' . $xpg_prev_page . '"><span aria-hidden="true">&laquo;</span></a></li>';
		} else {
			$s .= '<li class="disabled"><span aria-hidden="true">&laquo;</span></li>';
		}
		
		for($n = $page > 16 ? $page - 16 : 1; $n <= $xpg_total_pages; $n ++) {
			if ($n == $page) {
				$s .= '<li class="active"><a href="#">' . $n . '</a></li>';
			} else {
				$s .= '<li><a href="./index.php?m=' . $m . '&amp;tab=' . $tab . '&amp;page=' . $n . '">' . $n . '</a></li>';
			}
			if ($n >= 30 + $page - 15) {
				$xpg_break = true;
				break;
		...
		if ($xpg_next_page <= $xpg_total_pages) {
			$s .= '<li align="right" width="15%"><a href="./index.php?m=' . $m . '&amp;tab=' . $tab . '&amp;page=' . $xpg_total_pages . '" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
		} else {
			$s .= '<li class="disabled"><span aria-hidden="true">&raquo;</span></a></li>';
		}
		
		$sum = '
			<div class="pagination-tip label label-info btn-xs">' . $AppUI->_ ( 'Records' ) . ': ' . $xpg_totalrecs . ' | ' . $AppUI->_ ( 'Pages' ) . ': ' . $xpg_total_pages . '</div>
			</div>';
	}
	$s .= '</ul></div>' . $sum;
	return $s;
...
function apmgetSysVal($title) {
	...
	foreach ( $rows as $notUsed => $item ) {
		if ($item) {
			$arr [trim ( $item ['sysval_value_id'] )] = trim ( $AppUI->_ ( $item ['sysval_value'] ) );
...
function apmformatDuration($x) {
	global $AppUI;
	
	$dur_day = floor ( $x / apmgetConfig ( 'daily_working_hours' ) );
	$dur_hour = $x - $dur_day * apmgetConfig ( 'daily_working_hours' );
	$str = '';
	if ($dur_day > 1) {
...
/**
 */
function apmsetMicroTime() {
	global $microTimeSet;
	list ( $usec, $sec ) = explode ( ' ', microtime () );
	$microTimeSet = ( float ) $usec + ( float ) $sec;
}
function apmsetExecutionConditions() {
	$memoryLimt = (apmgetConfig ( 'reset_memory_limit' ) != '') ? apmgetConfig ( 'reset_memory_limit' ) : '64M';
 ...
 */
function apmformSafe($txt, $deslash = false) {
 ...
 * write their own standards.
 * Nothing that depends on locales
...
function dprint($file, $line, $level, $msg) {
	$max_level = ( int ) apmgetConfig ( 'debug' );
	$display_debug = apmgetConfig ( 'display_debug', false );
	if ($level <= $max_level) {
 ...
 * page.
 * This can be used to find post handlers, for instance.
 */
...
function getUsersArray() {
	return apmgetUsersHashList ();
}
function getUsersCombo($default_user_id = 0, $first_option = 'All users') {
	global $AppUI;
	
	$parsed = '<select name="user_id" class="form-control">';
	if ($first_option != '') {
...
function formatHours($hours) {
	...
	$hours = ( int ) $hours;
	$working_hours = apmgetConfig ( 'daily_working_hours' );
 ...
 */
function apmrequiredFields($requiredFields) {
	global $AppUI;
 ...
 * @param bool $revert
 *        	revert back to original config after test.
 * @return bool true if we have the minimum amount of RAM and if we can modify RAM
 */
function apmcheckMem($min = 0, $revert = false) {
	// First of all check if we have the minimum memory requirement.
	$want = apmgetBytes ( apmgetConfig ( 'reset_memory_limit' ) );
	$have = ini_get ( 'memory_limit' );
	// Try upping the memory limit based on our config
	ini_set ( 'memory_limit', apmgetConfig ( 'reset_memory_limit' ) );
	$now = apmgetBytes ( ini_get ( 'memory_limit' ) );
...
function apmtoolTip($header = '', $tip = '', $raw = false, $id = '') {
	global $AppUI;
 ...
 * @access public
 */
function apmwriteDebug($s, $t = '', $f = '?', $l = '?') {
	global $debug;
	
	$debug_file = apm_BASE_DIR . '/files/debug.log';
...
function HM2seconds($HM) {
	list ( $h, $m ) = explode ( ":", $HM );
	if (intval ( $h ) > 23 && intval ( $h ) < 0)
		$h = 0;
	if (intval ( $m ) > 59 && intval ( $m ) < 0)
		$m = 0;
	$seconds = 0;
 ...
 * screen.
 * The SQL file used is: install/sql/mysql/018_add_timezones.sql
 */
function apmgetTimezonesForInstall() {
	$file = apm_BASE_DIR . '/install/sql/018_add_timezones.sql';
...
function sendNewPass() {
	...
	$checkusername = preg_replace ( "/[^A-Za-z0-9]/", "", apmgetParam ( $_POST, 'checkusername', '' ) );
	$confirmEmail = trim ( apmgetParam ( $_POST, 'checkemail', '' ) );
	 ...
	 * Info: http://bugs.apmProject.net/view.php?id=457
	 */
	$qTest = new apm_Database_Query ();
	$qTest->addTable ( 'apmversion' );
	...
	$auth = new apm_Authenticators_SQL ();
	...
	if ($cur) {
		...
		$m = new apm_Utilities_Mail (); // create the mail
		$m->To ( $confirmEmail );
 ...
 *
 * @param
 *        	Date the starting date of the period
 * @param
 *        	Date the ending date of the period
 * @param
 *        	array by-ref an array of links to append new items to
 * @param
 *        	int the length to truncate entries by
 */
function getEventLinks($startPeriod, $endPeriod, $links, $notUsed = null, $minical = false) {
	...
	$cwd = explode ( ',', apmgetConfig ( 'cal_working_days' ) );
		...
		for($i = 0, $i_cmp = $start->dateDiff ( $end ); $i <= $i_cmp; $i ++) {
			...
			if (($row ['event_cwd'] && in_array ( $date->getDayOfWeek (), $cwd )) || ! $row ['event_cwd']) {
				if ($minical) {
					...
					$link ['href'] = '';
					$link ['alt'] = '';
					$link ['text'] = '<div class="event-row"><button class="btn btn-default btn-xs" type="button"><span class="glyphicon glyphicon-tag"></span></button><a href="' . $url . '" class="event">' . $row ['event_name'] . '</a>';
				}
 ...
 *        	int the length to truncate entries by
 * @param
 *        	int the company id to filter by
 */
function getTaskLinks($startPeriod, $endPeriod, $links, $strMaxLen, $company_id = 0, $minical = false, $userid = 0) {
	...
	foreach ( $tasks as $row ) {
		...
		if (! $minical) {
			...
			$link ['text'] = '<span>' . $row ['short_name'] . ($row ['task_milestone'] ? '&nbsp;' : '<label class="label label-default" data-original-title="' . $AppUI->_ ( 'Milestone' ) . '" data-container="body" data-toggle="tooltip" data-placement="right"><span class="glyphicon glyphicon-star"></span></label>') . '</span>';
		}
		
		// determine which day(s) to display the task
		$start = new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $row ['task_start_date'], '%Y-%m-%d %T' ) );
		$end = $row ['task_end_date'] ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $row ['task_end_date'], '%Y-%m-%d %T' ) ) : null;
		...
		if (($start->after ( $startPeriod )) && ($end && $end->after ( $startPeriod ) && $end->before ( $endPeriod ) && ! ($start->dateDiff ( $end )))) {
			if ($minical) {
				...
				$temp = $link;
				if ($a != 'day_view') {
					$temp ['text'] = '<button class="btn btn-default btn-xs" role="button"><span class="glyphicon glyphicon-play"></span></button>' . $temp ['text'] . ' (' . $end->format ( $tf ) . ') ';
				}
			}
			...
			if ($start->after ( $startPeriod ) && $start->before ( $endPeriod )) {
				if ($minical) {
					...
					$temp = $link;
					if ($a != 'day_view') {
						$temp ['text'] = '<button class="btn btn-default btn-xs" role="button"><span class="glyphicon glyphicon-play"></span></button>' . $temp ['text'] . ' (' . $start->format ( $tf ) . ') ';
					}
				}
			...
			if ($end && $end->after ( $startPeriod ) && $end->before ( $endPeriod ) && $start->before ( $end )) {
				if ($minical) {
					...
					$temp = $link;
					if ($a != 'day_view') {
						$temp ['text'] = '<button class="btn btn-default btn-xs" role="button"><span class="glyphicon glyphicon-stop"></span></button> ' . $temp ['text'] . ' (' . $end->format ( $tf ) . ') ';
					}
				}
				$links [$end->format ( FMT_TIMESTAMP_DATE )] [] = $temp;
			}
...
function getTaskTooltip($task_id) {
	...
	$start_date = ( int ) $task->task_start_date ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $task->task_start_date, '%Y-%m-%d %T' ) ) : null;
	$end_date = ( int ) $task->task_end_date ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $task->task_end_date, '%Y-%m-%d %T' ) ) : null;
...
function __extract_from_todo($user_id, $showArcProjs, $showLowTasks, $showInProgress, $showHoldProjs, $showDynTasks, $showPinned, $showEmptyDate, $task_type, $allowedTasks, $allowedProjects) {
	...
	if (! $showHoldProjs) {
		if (($on_hold_status = apmgetConfig ( 'on_hold_projects_status_id' )) != '') {
...
function __extract_from_contact_controller($controller) {
	...
	$notifyasked = apmgetParam ( $_POST, 'contact_updateask', 0 );
	if ($notifyasked && ! strlen ( $updatekey )) {
		$rnow = new apm_Utilities_Date ();
...
function __extract_from_tasks($min_view, $currentTabId, $project_id, $currentTabName, $AppUI) {
	...
	if ($min_view && isset ( $_GET ['task_status'] )) {
		$task_status = ( int ) apmgetParam ( $_GET, 'task_status', null );
		return $task_status;
...
function __extract_from_tasks_pinning($AppUI, $task_id) {
	if (isset ( $_GET ['pin'] )) {
		$pin = ( int ) apmgetParam ( $_GET, 'pin', 0 );
 ...
 * If the end date does not exist then set it for
...
function __extract_from_projects_gantt3($row) {
	if ($row ['task_start_date'] == '0000-00-00 00:00:00') {
		if ($row ['task_end_date'] == '0000-00-00 00:00:00') {
			$todaydate = new apm_Utilities_Date ();
 ...
 * If the start date does not exist then set it for
 * today. This avoids jpgraphs internal errors that render the gantt completely useless
 *
 * @param
 *        	$row
...
function __extract_from_view_messages4($s, $style, $row, $hideEmail, $editor, $AppUI, $new_messages, $bbparser, $m, $df, $tf, $canEdit, $canAdminEdit, $canDelete) {
	...
	if (! $hideEmail) {
		$s .= '&nbsp;';
		$s .= '<a href="mailto:' . $row ['contact_email'] . '">';
		$s .= '<img src="' . apmfindImage ( 'email.gif' ) . '" alt="email" />';
		$s .= '</a>';
	}
	...
	if ($row ['visit_user'] != $AppUI->user_id) {
		$s .= '<br />&nbsp;' . apmshowImage ( 'icons/stock_new_small.png' );
	...
	$s .= '<td style="' . $style . '" nowrap="nowrap">';
	$s .= '<img src="' . apmfindImage ( 'icons/posticon.gif', $m ) . '" alt="date posted" />' . $AppUI->formatTZAwareTime ( $row ['message_date'], $df . ' ' . $tf ) . '</td>';
	...
	if ($canEdit || $AppUI->user_id == $row ['forum_moderated'] || $AppUI->user_id == $row ['message_author'] || $canAdminEdit) {
		...
		$s .= apmshowImage ( 'icons/stock_edit-16.png', '16', '16' );
		$s .= '</td>';
	}
	if ($canDelete || $AppUI->user_id == $row ['forum_moderated'] || $AppUI->user_id == $row ['message_author'] || $canAdminEdit) {
		...
		$s .= apmshowImage ( 'icons/stock_delete-16.png', '16', '16' );
		$s .= '</a>';
		$s .= '</td></tr></table>';
	}
...
function loadModal($que, $oid=false)
{
	$q = new apm_Database_Query ();
	$q->addTable ( $que );
	$query_result = false;
	...
	switch ($que) {
		case 'contacts' :
			...
			if($oid) $q->addWhere ( 'contact_id IN ('.$oid.')');
		...
		case 'companies' :
			...
			if($oid) $q->addWhere ( 'company_id IN ('.$oid.')');
		...
		case 'projects' :
			...
			if($oid) $q->addWhere ( 'project_id IN ('.$oid.')');
			$q->addOrder ( 'project_name' );
			$resultList = $q->loadHashList ();			
			break;
		case 'tasks' :
			$title = 'Task';
			$q->addQuery ( 'task_id, task_name' );
			if($oid) $q->addWhere ( 'task_id IN ('.$oid.')');
			$resultList = $q->loadHashList ();
			break;
