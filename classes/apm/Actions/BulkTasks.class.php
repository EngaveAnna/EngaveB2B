<?php
class apm_Actions_BulkTasks extends CTask {
	 ...
	 *
	 * @param
	 *        	int Project ID of the tasks come from.
	 * @return bool
	 *
	...
	public function importTasks($from_project_id, $to_project_id, $project_start_date) {
		...
		$project_start_date = new apm_Utilities_Date ( $project_start_date );
		 ...
		 */
		$original_start_date = new apm_Utilities_Date ( $first_task ['task_start_date'] );
		...
		foreach ( $task_list as $orig_task ) {
			...
			$new_start_date = new apm_Utilities_Date ( $orig_task ['task_start_date'] );
			$new_start_date->addDays ( $timeOffset );
			
			$new_end_date = new apm_Utilities_Date ( $orig_task ['task_end_date'] );
			...
			$_newTask = new apm_Actions_BulkTasks ();
