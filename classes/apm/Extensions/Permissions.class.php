...
class apm_Extensions_Permissions extends gacl_api {
	...
	public function __construct($opts = null) {
		...
		$opts ['db_type'] = apmgetConfig ( 'dbtype' );
		$opts ['db_host'] = apmgetConfig ( 'dbhost' );
		$opts ['db_user'] = apmgetConfig ( 'dbuser' );
		$opts ['db_password'] = apmgetConfig ( 'dbpass' );
		$opts ['db_name'] = apmgetConfig ( 'dbname' );
		$opts ['db_table_prefix'] = apmgetConfig ( 'dbprefix' ) . $this->_db_acl_prefix;
		$opts ['db'] = $db;
		
		if (apmgetConfig ( 'debug', 0 ) > 10) {
			$this->_debug = true;
		}
	...
	public function checkModule($module, $op, $userid = null) {
		...
		$result = $this->apmacl_check ( 'application', $op, 'user', $userid, 'app', $module );
		return $result;
	}
	public function checkModuleItem($module, $op, $item = null, $userid = null) {
		if ('system' == $module) {
			 ...
			 * http://bugs.apmProject.net/view.php?id=1324
			 * The problem comes from the fact that all the other modules
			 * extend the apm_Core_BaseObject class and therefore their
		...
		$result = $this->apmacl_query ( 'application', $op, 'user', $userid, $module, $item );
	...
	public function checkModuleItemDenied($module, $op, $item, $user_id = null) {
		...
		$result = $this->apmacl_query ( 'application', $op, 'user', $user_id, $module, $item );
	...
	public function getItemACLs($module, $uid = null) {
		...
		return $this->apmsearch_acl ( 'application', 'view', 'user', $uid, $module );
	}
	...
	/**
	 * apmacl::removeACLPermissions() Removes the permissions for a given ACL ID
	 *
	 * @param mixed $acl_id        	
	 * @return
	 *
	...
	/**
	 * apmacl::removeModulePermissions() Removes the permissions from the results table for a module
	 *
	 * @param mixed $module        	
	 * @return
	 *
	...
	/**
	 * apmacl::removePermissions() Removes the permissions from the results table for a given user (example: when you delete a user)
	 *
	 * @param mixed $user_id        	
	 * @return
	 *
	...
	public function recalcPermissions($user_id = null, $user_aro_id = null, $role_id = null, $module = '', $notUsed = null) {
		...
		$q->addQuery ( 'a.value AS a_value, a.name AS a_name, ' . 'b.value AS b_value, b.name AS b_name, ' . 'c.value AS c_value, c.name AS c_name, ' . 'd.value AS d_value, d.name AS d_name, ' . 'e.value AS e_value, e.name AS e_name, ' . 'f.value AS f_value, f.name AS f_name' );
		$q->leftJoin ( $this->_db_acl_prefix . 'aco', 'b', 'a.value=b.section_value,' . apmgetConfig ( 'dbprefix' ) . $this->_db_acl_prefix . 'aro_sections c' );
		$q->leftJoin ( $this->_db_acl_prefix . 'aro', 'd', 'c.value=d.section_value,' . apmgetConfig ( 'dbprefix' ) . $this->_db_acl_prefix . 'axo_sections e' );
	...
	/**
	 * apmacl::apmacl_check()
	 * //apmacl_check is used for modules only
	 *
	 * @param mixed $notUsed
	 *        	it passes 'application' string by default and is not used
	 * @param mixed $op
	 *        	one of the acos 'access','view','add','delete','edit'
	 * @param mixed $notUsed2
	 *        	it passes 'user' string by default and is not used
	 * @param mixed $userid
	 *        	it passes the user_id
	 * @param mixed $notUsed3
	 *        	it passes 'app' string by default and is not used
	 * @param mixed $module
	 *        	it passes the modules name
	 * @return
	 *
	 */
	public function apmacl_check($notUsed = null, $op, $notUsed2 = null, $userid, $notUsed3 = null, $module) {
		global $apm_performance_acltime, $apm_performance_aclchecks;
		$q = new apm_Database_Query ();
		$q->addTable ( $this->_db_acl_prefix . 'permissions' );
		$q->addQuery ( 'access' );
		$q->addWhere ( 'module = \'' . $module . '\'' );
		...
		$res = $q->loadResult ();
		if (apm_PERFORMANCE_DEBUG) {
			++ $apm_performance_aclchecks;
			$apm_performance_acltime += array_sum ( explode ( ' ', microtime () ) ) - $startTime;
		}
		return $res;
	}
	public function apmacl_nuclear($userid, $module, $item, $mod_class = array()) {
		...
		// echo "<br><br> apmacl_nuclear function input args. Userid: ".$userid.". Module: ".$module.". Item: ".$item.". ";
	...
	/**
	 * apm_Extensions_Permissions::apmacl_query()
	 * //apmacl_query is used for items only
	 *
	 ...
	 *
	 */
	public function apmacl_query($notUsed = null, $op, $notUsed2 = null, $userid, $module, $item) {
		global $apm_performance_acltime, $apm_performance_aclchecks;
		...
		// This is a sensitive function so if the minimum permission request arguments are not provided don't permit anything to this item
		// echo "<br> apmacl_query function input args. Op: ".$op.". Userid: ".$userid.". Module: ".$module.". Item: ".$item.".<br><br>";
		...
		if ($module == 'task_log') {
			$mod_class = array (
					'mod_main_class' => 'CTask_Log',
					'permissions_item_table' => 'task_log',
					'permissions_item_field' => 'task_log_id',
					'permissions_item_label' => 'task_log_name',
					'mod_directory' => 'tasks' 
			);
		} elseif ($module == 'admin') {
			$mod_class = array (
					'mod_main_class' => 'CUser',
					'permissions_item_table' => 'users',
					'permissions_item_field' => 'user_id',
					'permissions_item_label' => 'user_username',
					'mod_directory' => 'admin' 
			);
		} elseif ($module == 'users') {
			$mod_class = array (
			...
			$mod_class = array (
					'mod_main_class' => 'CEvent',
					'permissions_item_table' => 'events',
					'permissions_item_field' => 'event_id',
					'permissions_item_label' => 'event_title',
		...
		if ($op == 'view') {
			...
			if (apm_PERFORMANCE_DEBUG) {
				// echo "<br>apm_PERFORMANCE_DEBUG is ON: </br>".apm_PERFORMANCE_DEBUG;
				$startTime = array_sum ( explode ( ' ', microtime () ) );
			}
			$res = $this->apmacl_nuclear ( $userid, $module, $item, $mod_class );
			if (apm_PERFORMANCE_DEBUG) {
				++ $apm_performance_aclchecks;
				$apm_performance_acltime += array_sum ( explode ( ' ', microtime () ) ) - $startTime;
			}
			...
			$nuclear = $this->apmacl_nuclear ( $userid, $module, $item, $mod_class );
	...
	public function apmsearch_acl($notUsed = null, $op, $notUsed2 = null, $userid, $module) {
		global $apm_performance_acltime, $apm_performance_aclchecks;
		$q = new apm_Database_Query ();
		$q->addTable ( $this->_db_acl_prefix . 'permissions' );
	 ...
	 * apmProject respects the permissions of the individual items it is
