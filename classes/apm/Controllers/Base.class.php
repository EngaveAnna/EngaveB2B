...
class apm_Controllers_Base {
	...
	public function process(\apm_Core_CAppUI $AppUI, array $myArray) {
		...
		/**
		 * The nonce validation only throws a warning as of v3.1 so that we don't break anyone's forms.
		 * As of v4.0
		 * this validation will be turned on and any form processing using this controller will have to include
		 * the __nonce field. See http://wiki.apmProject.net/index.php?title=Security_Nonce for more details.
		 */
		if (('' != $AppUI->__nonce) && ($AppUI->__nonce != $myArray ['__nonce'])) {
			error_log ( "Your submission is missing the CSRF nonce." );
