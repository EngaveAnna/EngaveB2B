 ...
 */
class apm_System_UpgradeManager {
	protected $action = '';
	protected $configDir = '';
	...
	public function getActionRequired() {
		if ($this->action == '') {
			...
			if (! file_exists ( $this->configFile ) || filesize ( $this->configFile ) == 0) {
				...
				} elseif (isset ( $this->_apmconfig )) {
					$this->configOptions = $this->_apmconfig;
					$this->action = 'upgrade';
				} else {
					/*
					 * This case should never be reached because if there is a config.
					 * php file, it should load either the $dPconfig or $apmconfig
			...
			if ($currentVersion < count ( $migrations )) {
				foreach ( $migrations as $update ) {
					...
					if ($myIndex > $currentVersion) {
						...
						$sql = "INSERT INTO apmversion " . " (db_version, code_version, last_db_update) " . " VALUES ($myIndex, '$version', now())";
						$dbConn->Execute ( $sql );
					}
				}
			}
	...
	public function convertSoftmag() {
		$dpVersion = '';
		set_time_limit ( 0 );
		...
		switch ($dpVersion) {
			...
			case '2.0.1' :
				...
				$allErrors = array_merge ( $allErrors, $errorMessages );
			/**
			 * This default case will handle any other version of Softmag but has been validated to handle:
			...
			default :
				...
				$errorMessages = $this->_applySQLUpdates ( 'dp_to_apm1.sql', $dbConn );
				$allErrors = array_merge ( $allErrors, $errorMessages );
				
				$this->_scrubSoftmagData ( $dbConn );
				
				$errorMessages = $this->_applySQLUpdates ( 'dp_to_apm2.sql', $dbConn );
				$allErrors = array_merge ( $allErrors, $errorMessages );
	...
	public function createConfigString($dbConfig) {
		$configFile = file_get_contents ( apm_BASE_DIR . '/includes/config-dist.php' );
	...
	public function testDatabaseCredentials($apmconfig) {
		$result = false;
		
		$this->_setConfigOptions ( $apmconfig );
	...
	protected function _getDatabaseVersion($dbConn) {
		$sql = "SELECT max(db_version) FROM apmversion";
		$res = $dbConn->Execute ( $sql );
	...
	protected function _prepareConfiguration() {
		$this->configDir = apm_BASE_DIR . '/includes';
		$this->configFile = apm_BASE_DIR . '/includes/config.php';
		$this->uploadDir = apm_BASE_DIR . '/files';
		$this->languageDir = apm_BASE_DIR . '/locales/en';
		$this->tempDir = apm_BASE_DIR . '/files/temp';
	}
	protected function _applySQLUpdates($sqlfile, $dbConn) {
		$sqlfile = apm_BASE_DIR . '/install/sql/' . $sqlfile;
		...
		for($i = 0; $i < $pieceCount; $i ++) {
			...
			if (! empty ( $pieces [$i] ) && $pieces [$i] != "#") {
				...
				if (! $dbConn->Execute ( $pieces [$i] )) {
					 ...
					 */
					if (strpos ( $errorMessage, 'Duplicate column name' ) === false && strpos ( $errorMessage, 'column/key exists' ) === false && strpos ( $errorMessage, 'Multiple primary key defined' ) && strpos ( $errorMessage, 'Duplicate key name' )) {
	...
	protected function _scrubSoftmagData($dbConn) {
		/*
		 * While this seems like a good place to use the core classes, it's
		 * really not. With all of the dependencies, it just gets to be a huge
		 * pain and ends up loading all kinds of unnecessary stuff.
		 */
		rmdir ( apm_BASE_DIR . '/db/' );
		$recordsUpdated = 0;
		...
		if ($res->RecordCount () > 0) {
			while ( ! $res->EOF ) {
				...
				foreach ( $values as $syskey ) {
					...
					$sql = "INSERT INTO sysvals (sysval_key_id, sysval_title, sysval_value, sysval_value_id) " . "VALUES ($sysvalKeyId, '$sysvalTitle', '$sysvalValue', $sysvalValId)";
					$dbConn->Execute ( $sql );
				}
				$recordsUpdated ++;
