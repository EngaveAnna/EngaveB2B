 ...
 *
 * NOTE: index.php and fileviewer.php MUST call apmsessionStart instead of trying to set their own sessions.
 */
class apm_System_Session {
	protected $q = null;
	...
	public function gc() {
		...
		$q->exec ();
		$q->clear ();
		if (apmgetConfig ( 'session_gc_scan_queue' )) {
	...
	public function convertTime($key) {
		...
		if (! apmgetConfig ( $key, 0 )) {
			return 86400;
		}
		
		$numpart = ( int ) apmgetConfig ( $key );
		$modifier = substr ( apmgetConfig ( $key ), - 1 );
	...
	public function start() {
		session_name ( 'apmProject' );
		if (ini_get ( 'session.auto_start' ) > 0) {
			session_write_close ();
		}
		if (apmgetConfig ( 'session_handling' ) == 'app') {
		...
		preg_match ( '_^(https?://)([^/]+)(:0-9]+)?(/.*)?$_i', apm_BASE_URL, $url_parts );
