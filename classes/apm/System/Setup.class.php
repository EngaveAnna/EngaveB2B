 ...
 * @abstract
 */
abstract class apm_System_Setup {
	protected $_errors;
	protected $_AppUI;
	protected $_perms;
	protected $_config;
	public function __construct(apm_Core_CAppUI $AppUI = null, array $config = null, apm_Database_Query $query = null) {
		$this->_AppUI = $AppUI;
		$this->_perms = $this->_AppUI->acl ();
		$this->_query = (is_null ( $query )) ? new apm_Database_Query () : $query;
	 ...
	 * array('require' => 'apmProject', 'comparator' => '>=', 'version' => '3'),
	...
	protected function _meetsRequirements() {
		...
		$requirements = (isset ( $this->_config ['requirements'] )) ? $this->_config ['requirements'] : array ();
		$modules = $this->_AppUI->getActiveModules ();
		
		foreach ( $requirements as $requirement ) {
			switch ($requirement ['require']) {
				// This gets the apmProject version
				case 'apmProject' :
			...
			if (! $result) {
				...
				$this->_errors [$requirement ['require']] = $requirement ['require'] . ' version should be ' . $requirement ['comparator'] . ' ' . $requirement ['version'] . ' instead it is ' . $version;
			}
		}
		return $result;
	}
