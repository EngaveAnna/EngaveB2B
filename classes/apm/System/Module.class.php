 ...
 */
class apm_System_Module extends apm_Core_BaseObject {
	public $mod_id = null;
	...
	public function __construct() {
		...
		$this->modules = apm_BASE_DIR . '/modules/';
	}
	...
	public function deploy(array $fileinfo) {
		...
		if (function_exists ( 'zip_open' )) {
			...
			$zipb = zip_open ( $this->filename );
			
			$moduleDir = '';
			$moduleName = '';
			
			while ( $zip_entry = zip_read ( $zipb ) ) {
				$filename = zip_entry_name ( $zip_entry );
				$zc = pathinfo ( $filename );
				
				if ($zc ['filename'] == 'setup') {
					$moduleDir = $zc ['dirname'] . '/';
					$moduleName = $zc ['dirname'];
				}
				$zipContent [] = $zc;
			}
	...
	// TODO: break these out to a separate apm_Core_ModuleConfig class
	...
	public function loadSettings($module, $configName = 'index_list') {
		$q = $this->_getQuery ();
		$q->addTable ( 'module_config' );
		$q->addQuery ( 'module_config_value, module_config_text, module_config_priority' );
		$q->addWhere ( "module_name = '$module'" );
	...
	public function storeSettings($moduleName, $configName, $displayColumns, $displayText, $modulePriority='1') {
		...
		foreach ( $displayColumns as $index => $column ) {
			...
			$q->addInsert ( 'module_config_priority', $modulePriority[$i] );
			$q->exec ();
			$q->clear ();
			$i ++;
		}
