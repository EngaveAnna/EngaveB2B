 ...
 */
class apm_System_HookHandler {
	protected $AppUI = null;
	public function __construct(apm_Core_CAppUI $AppUI) {
		$this->AppUI = $AppUI;
	}
					...
					try {
						...
						if (count ( $itemList )) {
							foreach ( $itemList as $calendarItem ) {
								$buffer .= apm_Output_iCalendar::formatCalendarItem ( $calendarItem, $module ['mod_directory'] );
							}
						}
					} catch ( Exception $exc ) {
						error_log ( 'Error in ' . __FUNCTION__ . ' -- ' . $exc->getMessage () );
