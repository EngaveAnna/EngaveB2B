...
class apm_FileSystem_Loader {
	...
	public function readDirs($path) {
		...
		if (is_dir ( apm_BASE_DIR . '/' . $path )) {
			$d = dir ( apm_BASE_DIR . '/' . $path );
			while ( false !== ($name = $d->read ()) ) {
				if (is_dir ( apm_BASE_DIR . '/' . $path . '/' . $name ) && $name != '.' && $name != '..' && $name != 'CVS' && $name != '.svn') {
					$dirs [$name] = $name;
				}
			}
			$d->close ();
	 ...
	 *        	string The path to read.
	 * @param
	 *        	string A regular expression to filter by.
