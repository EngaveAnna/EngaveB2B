 ...
 */
class apm_Authenticators_LDAP extends apm_Authenticators_Base {
	public $ldap_host;
	public $ldap_port;
	public $ldap_version;
	public $base_dn;
	...
	public function __construct() {
		...
		$this->fallback = isset ( $this->apmconfig ['ldap_allow_login'] ) ? $this->apmconfig ['ldap_allow_login'] : false;
		
		$this->ldap_host = $this->apmconfig ['ldap_host'];
		$this->ldap_port = $this->apmconfig ['ldap_port'];
		$this->ldap_version = $this->apmconfig ['ldap_version'];
		$this->base_dn = $this->apmconfig ['ldap_base_dn'];
		$this->ldap_search_user = $this->apmconfig ['ldap_search_user'];
		$this->ldap_search_pass = $this->apmconfig ['ldap_search_pass'];
		$this->filter = $this->apmconfig ['ldap_user_filter'];
		
		$this->ldap_complete_string = $this->apmconfig ['ldap_complete_string'];
	}
	public function authenticate($username, $password) {
		...
		if ($this->fallback == true) {
			$sqlAuth = new apm_Authenticators_SQL ();
