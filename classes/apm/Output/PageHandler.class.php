 ...
 */
class apm_Output_PageHandler {
	public function loadExtras(array &$storage, apm_Core_CAppUI $AppUI, $m, $type = 'tabs') {
		// Set up extra $type
		if (! isset ( $storage ['all_' . $type] [$m] )) {
			...
			foreach ( $AppUI->getActiveModules () as $dir => $notUsed ) {
				...
				$modules_items = $loader->readFiles ( apm_BASE_DIR . '/modules/' . $dir . '/', '^' . $m . '_' . substr ( $type, 0, - 1 ) . '.*\.php' );
				foreach ( $modules_items as $item ) {
					...
					$tabArray [] = array (
							...
							'file' => apm_BASE_DIR . '/modules/' . $dir . '/' . $filename,
							'module' => $dir 
					);
				}
			}
