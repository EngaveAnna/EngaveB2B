 ...
 */
class apm_Output_EmailManager {
	public $from = '';
	public $subject = '';
	public $body = '';
	protected $_AppUI = null;
	protected $templater = null;
	public function __construct(apm_Core_CAppUI $AppUI = null) {
		if (is_null ( $AppUI )) {
			trigger_error ( 'The apm_Output_EmailManager constructor should receive $AppUI (an apm_Core_CAppUI object) for proper usage.', E_USER_NOTICE );
		}
		
		$this->templater = new apm_Output_Email_Template ();
		
		$this->_AppUI = $AppUI;
	}
	public function getEventNotify(CEvent $event, $clash, $users) {
		$body = $this->_AppUI->_ ( 'Event' ) . ":\t" . $event->event_name . "\n";
		$body .= $this->_AppUI->_ ( 'URL' ) . ":\t" . apm_BASE_URL . "/index.php?m=events&a=view&event_id=" . $event->event_id . "\n";
		...
		$start_date = new apm_Utilities_Date ( $event->event_start_date );
		$end_date = new apm_Utilities_Date ( $event->event_end_date );
	 ...
	 *
	 */
	public function getCalendarConflictEmail(apm_Core_CAppUI $AppUI = null) {
	...
	public function getContactUpdateNotify(apm_Core_CAppUI $AppUI = null, CContact $contact) {
		$this->_AppUI = (! is_null ( $AppUI )) ? $AppUI : $this->_AppUI;
		...
		$body .= "\n\n" . apm_BASE_URL . "/updatecontact.php?updatekey=contact_updatekey";
	...
	public function getFileUpdateNotify(apm_Core_CAppUI $AppUI = null, CFile $file) {
		$this->_AppUI = (! is_null ( $AppUI )) ? $AppUI : $this->_AppUI;
	...
	public function getForumWatchEmail(CForum_Message $message, $forum_name, $message_from) {
		...
		$body .= "\n\n" . apm_BASE_URL . '/index.php?m=forums&a=viewer&forum_id=message_forum';
		$body .= "\n\nmessage_body";
	...
	public function getFileNotify(CFile $file) {
		...
		$body .= "\n" . $this->_AppUI->_ ( 'URL' ) . ':     ' . apm_BASE_URL . '/index.php?m=projects&a=view&project_id=' . $file->_project->project_id;
		
		if (intval ( $file->_task->task_id ) != 0) {
			$body .= "\n\n" . $this->_AppUI->_ ( 'Task' ) . ':    ' . $file->_task->task_name;
			$body .= "\n" . $this->_AppUI->_ ( 'URL' ) . ':     ' . apm_BASE_URL . '/index.php?m=tasks&a=view&task_id=' . $file->_task->task_id;
		...
		if ($this->_message != 'deleted') {
			$body .= "\n" . $this->_AppUI->_ ( 'URL' ) . ':     ' . apm_BASE_URL . '/fileviewer.php?file_id=' . $file->file_id;
	...
	public function getTaskNotify(CTask $task, $user, $projname) {
		...
		$start_date = new apm_Utilities_Date ( $this->_AppUI->formatTZAwareTime ( $task->task_start_date, '%Y-%m-%d %T' ) );
		$fmt_start_date = $start_date->format ( $user_prefs ['DISPLAYFORMAT'] );
		$end_date = new apm_Utilities_Date ( $this->_AppUI->formatTZAwareTime ( $task->task_end_date, '%Y-%m-%d %T' ) );
		...
		$body .= $this->_AppUI->_ ( 'URL', UI_OUTPUT_RAW ) . ":\t\t" . apm_BASE_URL . '/index.php?m=tasks&a=view&task_id=' . $task->task_id . "\n\n";
	...
	public function getTaskNotifyOwner(CTask $task) {
		...
		$body .= apmgetParam ( $_POST, 'task_log_description' );
	...
	public function getTaskRemind(CTask $task, $msg, $project_name, $contacts) {
		...
		$body .= $this->_AppUI->_ ( 'URL', UI_OUTPUT_RAW ) . ': ' . apm_BASE_URL . '/index.php?m=tasks&a=view&task_id=' . $task->task_id . '&reminded=1' . "\n\n";
	...
	public function getProjectNotify(CProject $project, $isNotNew) {
		...
		$body .= "\n" . $this->_AppUI->_ ( 'URL' ) . ':     ' . apm_BASE_URL . '/index.php?m=projects&a=view&project_id=' . $project->project_id;
	...
	public function notifyHR($username, $logname, $address, $userid) {
		$object = new stdClass ();
		$object->base_url = apm_BASE_URL;
		$object->company_name = apmgetConfig ( 'company_name' );
		$object->contact_name = $username;
	...
	public function notifyNewExternalUser($logname, $logpwd) {
		...
		$body .= "You may login at the following URL: " . apm_BASE_URL;
		$body .= "\n\n" . "Thank you very much.\n\n" . 'The company_name';
		$body .= " Support Staff.\n\n" . '****PLEASE KEEP THIS EMAIL FOR YOUR RECORDS****';
	...
	public function notifyNewUserCredentials($username, $logname, $logpwd) {
		...
		$body = "user_name,\n\n";
		$body .= "An access account has been created for you in our apmProject project management system.\n\n";
	...
	public function notifyPasswordReset($username, $password) {
		...
		$body = $this->_AppUI->_ ( 'sendpass0', UI_OUTPUT_RAW ) . ' user_name ' . $this->_AppUI->_ ( 'sendpass1', UI_OUTPUT_RAW ) . ' base_url ' . $this->_AppUI->_ ( 'sendpass2', UI_OUTPUT_RAW ) . ' password ' . $this->_AppUI->_ ( 'sendpass3', UI_OUTPUT_RAW );
