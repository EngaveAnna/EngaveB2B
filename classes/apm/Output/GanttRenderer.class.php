 ...
 */
class apm_Output_GanttRenderer {
	private $graph = null;
	...
	private $taskCount = 0;
	public function __construct(apm_Core_CAppUI $AppUI, $width) {
	...
	public function setDateRange($start_date, $end_date) {
		$min_d_start = new apm_Utilities_Date ( $start_date );
		$max_d_end = new apm_Utilities_Date ( $end_date );
	...
	public function addBar(array $columnValues, $caption = '', $height = '0.6', $barcolor = 'FFFFFF', $active = true, $progress = 0, $identifier = 0) {
		foreach ( $columnValues as $name => $value ) {
			switch ($name) {
				case 'start_date' :
					$start = $value;
					$startDate = new apm_Utilities_Date ( $value );
					$rowValues [] = $startDate->format ( $this->df );
					break;
				case 'end_date' :
					$endDate = new apm_Utilities_Date ( $value );
				...
				case 'actual_end' :
					if ('' == $value) {
						...
						$actual_end = $value;
						$actual_endDate = new apm_Utilities_Date ( $value );
	...
	public function addMilestone(array $columnValues, $start, $color = '#CC0000', $identifier = 0, $captionToTheLeft = false) {
		$tStartObj = new apm_Utilities_Date ( $start );
