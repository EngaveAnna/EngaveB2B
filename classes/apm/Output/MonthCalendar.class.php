 ...
 */
class apm_Output_MonthCalendar {
	/**
	 * #@+
	 * 
	 * @var Date
	...
	/**
	 * apm_Output_MonthCalendar::setDate()
	 *
	 * { Description }
	 *
	 * @param [type] $date        	
	...
	public function setDate($date = null) {
		$this->this_month = new apm_Utilities_Date ( $date );
		
		$d = $this->this_month->getDay ();
		$m = $this->this_month->getMonth ();
		$y = $this->this_month->getYear ();
		
		$this->prev_year = new apm_Utilities_Date ( $date );
		$this->prev_year->setYear ( $this->prev_year->getYear () - 1 );
		
		$this->next_year = new apm_Utilities_Date ( $date );
		...
		$this->prev_month = new apm_Utilities_Date ( $date );
		
		setlocale ( LC_TIME, 'en' );
		$date = Date_Calc::beginOfNextMonth ( $d, $m, $y, FMT_TIMESTAMP_DATE );
		setlocale ( LC_ALL, $this->_AppUI->user_lang );
		$this->next_month = new apm_Utilities_Date ( $date );
	}
	
	/**
	 * apm_Output_MonthCalendar::setStyles()
	 *
	 * { Description }
	 */
	...
	/**
	 * apm_Output_MonthCalendar::setLinkFunctions()
	 *
	 * { Description }
	 *
	 * @param string $day        	
	...
	/**
	 * apm_Output_MonthCalendar::setCallback()
	 *
	 * { Description }
	 */
	...
	/**
	 * apm_Output_MonthCalendar::setEvents()
	 *
	 * { Description }
	 */
	...
	/**
	 * apm_Output_MonthCalendar::setHighlightedDays()
	 * ie ['20040517'] => '#ff0000',
	 *
	 * { Description }
	 */
	...
	/**
	 * apm_Output_MonthCalendar::_drawTitle()
	 *
	 * { Description }
	 */
	private function _drawTitle() {
		...
		if ($this->showArrows) {
			...
			$s .= '<td><a href="' . $href . '"><span class="glyphicon glyphicon-chevron-left" ></span></a></td>';
		}
		...
		if ($this->showArrows) {
			...
			$s .= '<td ><a href="' . $href . '"><span class="glyphicon glyphicon-chevron-right" ></span></a></td>';
		}
	...
	/**
	 * apm_Output_MonthCalendar::_drawDays()
	 *
	 * { Description }
	 *
	...
	/**
	 * apm_Output_MonthCalendar::_drawMain()
	 *
	 * { Description }
	 */
	private function _drawMain() {
		...
		foreach ( $cal as $week ) {
			...
			foreach ( $week as $day ) {
				$this_day = new apm_Utilities_Date ( $day );
	...
	/**
	 * apm_Output_MonthCalendar::_drawEvents()
	 *
	 * { Description }
	 */
	private function _drawEvents($day) {
		...
		foreach ( $events as $e ) {
			...
			$s .= '<br />';
			$s .= $href ? '<a href="' . $href . '" class="event" >' : '';
			$s .= $e ['text'];
			$s .= $href ? '</a>' : '';
		}
		return $s;
