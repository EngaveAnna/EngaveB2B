 ...
 *
 * @todo    refactor to apm/System/AppUI
 */
class apm_Core_CAppUI {
	...
	public function __construct() {
		...
		$this->cfg ['locale_warn'] = apmgetConfig ( 'locale_warn' );
	 ...
	 * 
	 * @param string $name
	 *        	The class root file name (excluding .class.php)
	...
	public function getSystemClass($name = null) {
		...
		if ($name) {
			return apm_BASE_DIR . '/classes/' . $name . '.class.php';
		}
	}
	...
	public function getLibraryClass($name = null) {
		...
		if ($name) {
			return apm_BASE_DIR . '/lib/' . $name . '.php';
		}
	}
	...
	public function getModuleClass($name = null) {
		...
		if ($name) {
			return apm_BASE_DIR . '/modules/' . $name . '/' . $name . '.class.php';
		}
	}
	 ...
	 * 
	 * @param string $name
	 *        	The class root file name (excluding .ajax.php)
	 * @return string The path to the include file
	 */
	public function getModuleAjax($name = null) {
		if ($name) {
			return apm_BASE_DIR . '/modules/' . $name . '/' . $name . '.ajax.php';
		}
	}
	 ...
	 * 
	 * @return String value indicating the current apmProject version
	 */
	public function getVersion() {
		if (! isset ( $this->version_major )) {
			include apm_BASE_DIR . '/includes/version.php';
			$this->version_major = $apm_version_major;
			$this->version_minor = $apm_version_minor;
			$this->version_patch = $apm_version_patch;
			...
			if (isset ( $apm_version_prepatch )) {
				$this->version_string .= '-' . $apm_version_prepatch;
			}
		}
	...
	public function formatTZAwareTime($datetime = '', $format = '') {
		...
		if ('' == $format) {
			...
			$df = $format;
			$ts = new apm_Utilities_Date ( $ts->format ( 'Y-m-d H:i:s' ) );
		}
	...
	public function setStyle() {
		...
		if ($uistyle && ! is_dir ( apm_BASE_DIR . '/style/' . $uistyle )) {
			// fall back to host_style if user style is not installed
			$this->setPref ( 'UISTYLE', apmgetConfig ( 'host_style' ) );
		}
	}
	
	/**
	 * This creates and returns a theme object (which extends apm_Theme_Base)
	 * which is then used for all templating, etc.
	 *
	 * @return apm_Theme_Base object
	 */
	public function getTheme() {
		$this->setStyle ();
		$uistyle = ('' == $this->getPref ( 'UISTYLE' )) ? 'modern' : $this->getPref ( 'UISTYLE' );
	...
	public function readDirs($path) {
		trigger_error ( "AppUI->readDirs() has been deprecated in v3.0 and will be removed by v4.0. Please use apm_FileSystem_Loader->readDirs() instead.", E_USER_NOTICE );
	...
	public function readFiles($path, $filter = '.') {
		trigger_error ( "AppUI->readFiles() has been deprecated in v3.0 and will be removed by v4.0. Please use apm_FileSystem_Loader->readFiles() instead.", E_USER_NOTICE );
	...
	public function checkFileName($file) {
		trigger_error ( "AppUI->checkFileName() has been deprecated in v3.0 and will be removed by v4.0. Please use apm_FileSystem_Loader->checkFileName() instead.", E_USER_NOTICE );
	...
	public function makeFileNameSafe($file) {
		trigger_error ( "AppUI->makeFileNameSafe() has been deprecated in v3.0 and will be removed by v4.0. Please use apm_FileSystem_Loader->makeFileNameSafe() instead.", E_USER_NOTICE );
	 ...
	 * 
	 * @param
	 *        	string Locale abbreviation corresponding to the sub-directory name in the locales directory (usually the abbreviated language code).
	 */
	public function setUserLocale($loc = '', $set = true) {
		...
		if (! $loc) {
			$loc = $this->user_prefs ['LOCALE'] ? $this->user_prefs ['LOCALE'] : apmgetConfig ( 'host_locale' );
		}
	...
	public function loadLanguages() {
		if (isset ( $_SESSION ['LANGUAGES'] )) {
			...
			foreach ( $langs as $lang ) {
				if (file_exists ( apm_BASE_DIR . '/locales/' . $lang . '/lang.php' )) {
					include apm_BASE_DIR . '/locales/' . $lang . '/lang.php';
				}
			}
	 ...
	 * @param
	 *        	int Option flags, can be case handling or'd with output styles
	 * @return string
	 */
	 ...
	 *
	 * @param
	 *        	object The item to be temporarily stored
	 *        	
	 *        	
	 */
	 ...
	 *
	 * @param
	 *        	string The URL query string to append to the URL
	 * @param
	 *        	string A marker for a historic 'place, only -1 or an empty string is valid.
	 */
	 ...
	 * @param
	 *        	boolean If true, $msg is appended to the current string otherwise
	 ...
	 * 
	 * @param
	 *        	boolean If true the current message state is cleared.
	 */
	public function getMsg($reset = true) {
		...
		return $msg ? '<div class="alert alert-' . $class . '">' . '<span class="fa fa-alert ' . $icon . '"></span>' . '<a href="#" class="close" data-dismiss="alert">&times;</a>' . $msg . '</div>' : '';
	}
	 ...
	 * 
	 * @param
	 *        	string The label or key of the state variable
	 * @param
	 *        	mixed Value to assign to the label/key
	 */
	...
	public function login($username, $password) {
		$auth_method = apmgetConfig ( 'auth_method', 'sql' );
		...
		if (! isset ( $GLOBALS ['acl'] )) {
			$GLOBALS ['acl'] = new apm_Extensions_Permissions ();
		}
		...
		$q->addTable ( 'users' );
		$q->addQuery ( 'user_id, contact_first_name as user_first_name, ' . 'contact_last_name as user_last_name, contact_email as user_email, contact_display_name as user_display_name, ' . 'contact_company as user_company, contact_department as user_department, user_type, user_icon' );
	...
	/**
	 * @Function for register log out in apmProject table "user_acces_log"
	 */
	 ...
	 *        	int User id number
	 * @param
	 *        	bool If false (default) then sture internally, if true return pref array
	 */
	...
	public function getMenuModules() {
		$q = new apm_Database_Query ();
		$q->addTable ( 'modules' );
		$q->addQuery ( 'mod_directory, mod_ui_name, mod_ui_icon, mod_ui_text_icon' );
	 ...
	 * 
	 * @return object apm_Extensions_Permissions
	 */
	public function &acl() {
		if (! isset ( $GLOBALS ['acl'] )) {
			$GLOBALS ['acl'] = new apm_Extensions_Permissions ();
