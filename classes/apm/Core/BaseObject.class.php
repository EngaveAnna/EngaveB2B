 ...
 * @abstract
 */
abstract class apm_Core_BaseObject extends apm_System_Event implements apm_Interfaces_Listener {
	 ...
	 * This should be overridden by all submodules
	 *
	 * @param string $table
	 ...
	 * @param string $module
	 *        	name as stored in the 'mod_directory' of the 'modules' table, and the 'value' field of the 'gacl_axo' table. It is used for permission checking in situations where the table name is different from the module folder name. For compatibility sake this variable is set equal to the $table if not set as failsafe.
	 */
	public function __construct($table, $key, $module = '') {
		...
		$this->_tbl_prefix = apmgetConfig ( 'dbprefix', '' );
		$this->_query = new apm_Database_Query ();
		 ...
		 */
		global $AppUI;
		$this->_AppUI = is_null ( $AppUI ) ? new apm_Core_CAppUI () : $AppUI;
		$this->_perms = $this->_AppUI->acl ();
		global $locale_char_set;
		$this->_locale_char_set = $locale_char_set;
		
		/**
		 * This block does a lot and may need to be simplified..
		 * but the point
		 ...
		 */
		$this->_dispatcher = new apm_System_Dispatcher ();
	 ...
	 *
	 * @param
	 *        	Object A database connection (real or mocked)
	 */
	 ...
	 *
	 * @param
	 *        	Object A permissions/preferences object (real or mocked)
	 */
	 ...
	 *        	$hash
	 * @param null $prefix
	 *        	Defaults to null, prefix to use with hash keys
	 * @param bool $checkSlashes
	 *        	Defaults to true, strip any slashes from the hash values
	 * @param bool $bindAll        	
	 *
	 * @return bool
	 */
	 ...
	 *
	 * @param null $oid
	 *        	optional argument, if not specified then the value of current key is used
	 * @param bool $strip        	
	 *
	 * @return bool
	 */
	public function load($oid = null, $strip = true) {
		$this->_dispatcher->publish ( new apm_System_Event ( get_class ( $this ), 'preLoadEvent' ) );
		...
		$oid = $this->$k;
		if ($oid === null) {
			// echo "Function load: oid null </br>";
			return false;
		}
		if (! $this->canView ()) {
			// echo "Function load: canView false </br>";
			return false;
		}
		$q = $this->_getQuery ();
		...
		$this->_dispatcher->publish ( new apm_System_Event ( get_class ( $this ), 'postLoadEvent' ) );
	 ...
	 *
	 */
	public function loadFull($notUsed = null, $object_id) {
		$this->load ( $object_id );
	}
	...
	/**
	 * Return a apm_Database_Query object seeded with the table name.
	 * 
	 * @param string $alias
	 *        	optional alias for table queries.
	 * @return apm_Database_Query object
	 */
	...
	/**
	 * This function does just what you think it does.
	 ...
	 *
	 * @param
	 *        	object Object to trim class variables for
	 *        	
	 * @return none
	 */
	...
	public function store($updateNulls = false) {
		$result = false;
		$this->clearErrors ();
		$this->_dispatcher->publish ( new apm_System_Event ( get_class ( $this ), 'preStoreEvent' ));
		$this->apmTrimAll ();
		...
		$this->_dispatcher->publish ( new apm_System_Event ( get_class ( $this ), 'pre' . $this->_event . 'Event' ) );
		$q = $this->_getQuery ();
		
		/**
		 * Note that we have to check and perform the edit *first* because the
		 * create/add fills in the id that we're checking.
		 * Therefore, if we
		...
		if ($result) {
			...
			$this->_dispatcher->publish ( new apm_System_Event ( get_class ( $this ), 'post' . $this->_event . 'Event' ) );
			$this->_dispatcher->publish ( new apm_System_Event ( get_class ( $this ), 'postStoreEvent' ) );
		} else {
	...
	public function canDelete($notUsed = '', $oid = null, $joins = null) {
		...
		if (is_array ( $joins )) {
			foreach ( $joins as $table ) {
				...
				if ($records) {
					$this->_error ['noDeleteRecord-' . $table ['label']] = $this->_AppUI->_ ( 'You cannot delete this item. It is currently considered a ' . $table ['label'] );
				}
			}
		}
	...
	public function delete($oid = null) {
		...
		$this->_dispatcher->publish ( new apm_System_Event ( get_class ( $this ), 'preDeleteEvent' ) );
		...
		if ($result) {
			$this->_dispatcher->publish ( new apm_System_Event ( get_class ( $this ), 'postDeleteEvent' ) );
		} else {
	 ...
	 *        	$uid
	 * @param string $fields
	 *        	fields to be returned by the query, default is all
	 * @param string $orderby
	 *        	sort order for the query
	 * @param null $index
	 *        	name of field to index the returned array
	 * @param null $extra
	 *        	array of additional sql parameters (from and where supported)
	 ...
	 *
	 * @param int $uid        	
	 * @param apm_Database_Query $query        	
	 * @param string $index        	
	 *
	 ...
	 * store it.
	 * It is called regardless of whether or not the object is
	 * valid or the user has permissions. You might use this to make an
	 * object valid.
	 *
	 * @return \apm_Core_BaseObject
	 */
	 ...
	 * delete it.
	 * It is called regardless of whether or not the object can be
	 ...
	 * was stored properly.
	 * You might use this to send notifications or
	...
	protected function hook_postStore() {
		$custom_fields = new apm_Core_CustomFields ( $this->_tbl_module, 'addedit', $this->{$this->_tbl_key}, 'edit' );
	...
	/**
	 * This method is called only if $this->delete() was successful.
	 * It is
	...
	protected function hook_postDelete() {
		 ...
		 *       deleted.
		 */
		$custom_field = new apm_Core_CustomField ( 'notUsed', 'notUsed2', 'notUsed3', 'notUsed4', 'notUsed5', 'notUsed6' );
	 ...
	 * @access protected
	 *        
	 * @return apm_Database_Query Clean query object
	 */
