 ...
 */
class apm_Core_CustomFieldsParser {
	 ...
	 * Constructor
	 * 
	 * @return apm_Core_CustomFieldsParser
	 * @param
	 *        	char Field type: TaskCustomFields, CompanyCustomFields
	 */
	...
	public function _fetchFields() {
		$this->fields_array = apmgetSysVal ( $this->custom_record_type );
	}
	public function _fetchCustomRecordTypes() {
		...
		$this->custom_record_types = apmgetSysVal ( $field_types );
	}
	...
	public function parseEditField($key) {
		...
		switch ($field_config ['type']) {
			case 'select' :
				$parsed .= '<td align="left">' . arraySelect ( explode ( ',', $field_config ['selects'] ), 'custom_' . $key, 'size="1" class="form-control" ' . $field_config ['options'], (isset ( $this->previous_data [$key] ) ? $this->previous_data [$key] : '') ) . '</td>';
				break;
			case 'textarea' :
				$parsed .= '<td align="left"><textarea name="custom_' . $key . '" class="form-control" ' . $field_config ['options'] . '>' . (isset ( $this->previous_data [$key] ) ? $this->previous_data [$key] : '') . '</textarea></td>';
				break;
			case 'checkbox' :
				...
				foreach ( $options_array as $option ) {
					...
					$parsed .= '<input type="checkbox" class="form-control" value="' . $option . '" name="custom_' . $key . '[]" class="form-control" style="border:0" ' . $checked . ' ' . $field_config ['options'] . ' />' . $option . '<br />';
					$checked = '';
				}
				$parsed .= '</td>';
				break;
			default :
				$parsed .= '<td align="left"><input type="text" class="form-control" name="custom_' . $key . '" class="form-control" ' . $field_config ['options'] . ' value="' . (isset ( $this->previous_data [$key] ) ? $this->previous_data [$key] : '') . '" /></td>';
		}
		$parsed .= '</tr>';
		return $parsed;
	}
	public function parseViewField($key) {
		...
		switch ($field_config ['type']) {
			case 'text' :
				$parsed .= '<td class="hilite">' . apmformSafe ( (isset ( $this->previous_data [$key] ) ? $this->previous_data [$key] : '') ) . '</td>';
				break;
			case 'href' :
				$parsed .= '<td class="hilite"><a href="' . apmformSafe ( (isset ( $this->previous_data [$key] ) ? $this->previous_data [$key] : '') ) . '">' . apmformSafe ( (isset ( $this->previous_data [$key] ) ? $this->previous_data [$key] : '') ) . '</a></td>';
				break;
			case 'select' :
				$optionarray = explode ( ',', $field_config ['selects'] );
				$parsed .= '<td class="hilite" width="300">' . apmformSafe ( (isset ( $this->previous_data [$key] ) ? $optionarray [$this->previous_data [$key]] : '') ) . '</td>';
				break;
			case 'textarea' :
				$parsed .= '<td valign="top" class="hilite">' . apmformSafe ( (isset ( $this->previous_data [$key] ) ? $this->previous_data [$key] : '') ) . '</td>';
				break;
			case 'checkbox' :
				$optionarray = explode ( ',', $field_config ['selects'] );
				$parsed .= '<td align="left">';
				foreach ( $optionarray as $option ) {
					...
					$parsed .= '<input type="checkbox" value="' . $option . '" name="custom_' . $key . '[]" class="form-control" locked style="border:0" ' . $checked . ' ' . $field_config ['options'] . ' disabled />' . $option . '<br />';
				}
				$parsed .= '</td>';
				break;
		}
