 ...
 *
 * @package     apmProject\database
 *
 */
class apm_Database_Query {
	/**
	...
	/**
	 * apm_Database_Query constructor
	 *
	 * @param $prefix Database
	 *        	table prefix - will be appended to all apmProject table names
	 * @param $query_db Database
	 *        	type
	 */
	public function __construct($prefix = null, $query_db = null) {
		...
		if (isset ( $prefix )) {
			...
			$this->_table_prefix = apmgetConfig ( 'dbprefix', '' );
		}
		$this->_db = isset ( $query_db ) ? $query_db : $db;
		$this->_db_funcs = array (
				$this->dbfnNow () 
		);
	 ...
	 * it in PHP.
	 * It won't necessarily be faster but should be more
	...
	public function concat() {
		...
		$arr = func_get_args ();
		$conc_str = call_user_func_array ( array (
				&$this->_db,
				'Concat' 
		), $arr );
		return $conc_str;
	 ...
	 *
	 * Used internally with apm_Database_Query
	 *
	 * @param $varname Name
	 *        	variable to add/create
	 * @param $name Data
	 *        	add
	 * @param $id Index
	 *        	use in array.
	 ...
	 * @param $id Alias
	 *        	use in query/where/group clauses.
	 */
	 ...
	 * 
	 * @param $clause the
	 *        	type of clause to add
	 * @param $value the
	 *        	clause value
	 * @param $check_array defaults
	 *        	to true, iterates through each element in $value and adds them seperately to the clause
	 */
	 ...
	 * 
	 * @param $field The
	 *        	field to insert the value into
	 * @param $value The
	 *        	specified value
	 * @param $set Defaults
	 *        	to false. If true will check to see if the fields or values supplied are comma delimited strings instead of arrays
	 * @param $func Defaults
	 *        	to false. If true will not use quotation marks around the value - to be used when the value being inserted includes a function
	 */
	 ...
	 *        	to false. If true will check to see if the fields or values supplied are comma delimited strings instead of arrays
	 * @param $func Defaults
	 *        	to false. If true will not use quotation marks around the value - to be used when the value being updated includes a function
	 */
	...
	public function createDatabase($database) {
		$dict = NewDataDictionary ( $this->_db, apmgetConfig ( 'dbtype' ) );
		$dict->CreateDatabase ( $database );
	}
	public function DDcreateTable($table, $def, $opts) {
		$dict = NewDataDictionary ( $this->_db, apmgetConfig ( 'dbtype' ) );
		$query_array = $dict->ChangeTableSQL ( apmgetConfig ( 'dbprefix' ) . $table, $def, $opts );
	 ...
	 * 
	 * @param $table the
	 *        	name of the temporary table to drop
	 */
	...
	public function addField($name, $type) {
		...
		trigger_error ( "apm_Database_Query->addField() has been deprecated in v3.0 and will be removed by v4.0.", E_USER_NOTICE );
	}
	...
	public function alterField($name, $type) {
		...
		trigger_error ( "apm_Database_Query->alterField() has been deprecated in v3.0 and will be removed by v4.0.", E_USER_NOTICE );
	}
	...
	public function dropField($name) {
		...
		trigger_error ( "apm_Database_Query->dropField() has been deprecated in v3.0 and will be removed by v4.0.", E_USER_NOTICE );
	}
	
	/**
	 * Add an index.
	 * Fields should be separated by commas to create a multi-field index
	 */
	public function addIndex($name, $type) {
		...
		trigger_error ( "apm_Database_Query->addIndex() has been deprecated in v3.0 and will be removed by v4.0.", E_USER_NOTICE );
	}
	
	/**
	 * Add a primary key attribute.
	 * Fields should be separated by commas to create a multi-field primary key
	 */
	public function addPrimary($name) {
		...
		trigger_error ( "apm_Database_Query->addPrimary() has been deprecated in v3.0 and will be removed by v4.0.", E_USER_NOTICE );
	}
	...
	public function dropIndex($name) {
		...
		trigger_error ( "apm_Database_Query->dropIndex() has been deprecated in v3.0 and will be removed by v4.0.", E_USER_NOTICE );
	}
	...
	public function dropPrimary() {
		...
		trigger_error ( "apm_Database_Query->dropPrimary() has been deprecated in v3.0 and will be removed by v4.0.", E_USER_NOTICE );
	}
	 ...
	 *
	 * @param $query Where
	 *        	to use, not including WHERE keyword
	 */
	 ...
	 *        	table (without prefix)
	 * @param $alias Alias
	 *        	use instead of table name (required).
	 * @param $join Join
	 *        	(e.g. 'a.id = b.other_id')
	 ...
	 * @see addJoin()
	 * @param $table Name
	 *        	of table (without prefix)
	 * @param $alias Alias
	 *        	to use instead of table name
	 * @param $join Join
	 *        	condition
	 */
	 ...
	 *
	 * @param $query HAVING
	 *        	to use, not including HAVING keyword
	 */
	public function addHaving($query) {
		$this->addClause ( 'having', $query );
	}
	public function dbfnNowWithTZ($type=0) {
		switch($type)
		{
			case 0:
				$df = 'Y-m-d H:i:s';
	...
	public function dbfnNow() {
		$dbType = strtolower ( trim ( apmgetConfig ( 'dbtype' ) ) );
	 ...
	 *
	 * @param $date This
	 *        	the date we want to add to
	 * @param $interval This
	 *        	how much units we will be adding to the date
	 * @param $unit This
	 *        	the type of unit we are adding to the date
	 */
	 ...
	 *        	rows to limit.
	 * @param $start First
	 *        	to start extraction(row offset).
	 */
	...
	public function setPageLimit($page = 0, $pagesize = 0) {
		...
		if ($pagesize == 0) {
			$pagesize = apmgetConfig ( 'page_size' );
		}
	 ...
	 * 
	 * @param $clear Boolean,
	 *        	Clear the query after it has been executed
	...
	public function prepareInsert() {
		switch (strtolower ( trim ( apmgetConfig ( 'dbtype' ) ) )) {
			...
			case 'oracle' :
				...
				$quotedFieldnames = array_map ( array (
						$this,
						'quote_db' 
	 ...
	 * 
	 * @param $style ADODB
	 *        	fetch style. Can be ADODB_FETCH_BOTH, ADODB_FETCH_NUM or ADODB_FETCH_ASSOC
	 * @param $debug Defaults
	 *        	to false. If true, debug output includes explanation of query
	 * @return Handle to the query result
	 */
	public function &exec($style = ADODB_FETCH_BOTH, $debug = false) {
		global $ADODB_FETCH_MODE, $apm_performance_dbtime, $apm_performance_dbqueries;
		...
		if ($q = $this->prepare ()) {
			//file_put_contents ('logs.txt' , 'Log: '.date('Y-m-d [H:i:s]', time()).' -> '.serialize($q)."</br>Debug: ".$debug.chr (13). chr (10), FILE_APPEND);
			if ($debug) {
			...
			if (apm_PERFORMANCE_DEBUG) {
				++ $apm_performance_dbqueries;
				$apm_performance_dbtime += array_sum ( explode ( ' ', microtime () ) ) - $startTime;
			}
			return $this->_query_id;
		} else {
			if (apm_PERFORMANCE_DEBUG) {
	 ...
	 * 
	 * @param $maxrows Maximum
	 *        	number of rows to return
	 * @param $index Can
	 *        	be used to set the keys of the resulting arrays, useful to find records by primary key.
	 ...
	 * 
	 * @param $index Defaults
	 *        	to null, the field to use for array keys
	 * @return Associative array of rows, keyed with the field indicated by the $index parameter
	 */
	public function loadHashList($index = null, $clear=false) {
		if (! $this->exec ( ADODB_FETCH_ASSOC )) {
			return array ();
		}
		$hashlist = array ();
	 ...
	 * $keys = array_keys($hash);
	 * }
	 * $hashlist[$hash[$keys[0]]] = $hash[$keys[2]];
	 * }
	 * }
	 * $this->clear();
	 * return $hashlist;
	...
	/**
	 * Load database results into a apm_Core_BaseObject based object
	 * 
	 * @param
	 *        	&$object Reference to the object to propagate with database results
	 * @param $bindAll Defaults
	 *        	to false, Bind every field returned to the referenced object
	 * @param $strip Defaults
	 *        	to true
	 ...
	 *        	hash to bind
	 * @param
	 *        	&$obj A reference to the object to bind the hash to
	 * @param $prefix Defaults
	 *        	to null, prefix to use with hash keys
	 * @param $notUsed Defaults
	 *        	to true but isn't used anyway
	 * @param $bindAll Bind
	 *        	all values regardless of their existance as defined instance variables
	 */
	 ...
	 *
	 * @param $where_clause Either
	 *        	string or array of subclauses.
	 ...
	 *
	 * @param $order_clause Either
	 *        	or array of subclauses.
	 ...
	 *
	 * @param $limit Either
	 *        	with nr of records to retrieve or array with offset and nr of records to retrieve .
	 * @param $offset integer
	 *        	offset from where it should start retrieving.
	...
	public function insertObject($table, &$object, $keyName = null, $verbose = false) {
		...
		foreach ( $fields as $field ) {
			if ($field == 'task_creator' || $field == 'task_duration')
	 ...
	 *
	 * @return object new record object or null if error
	 *        
	 */
	public function duplicate() {
		 ...
		 * This method also appears in the apm_Utilities_Date and apm_Core_BaseObject (modified) class.
		 */
		return clone ($this);
	}
}