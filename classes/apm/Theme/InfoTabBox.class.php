 ...
 */
class apm_Theme_InfoTabBox extends apm_Theme_TabBox {
	public function show($extra = '', $js_tabs = false, $alignment = 'left') {
		global $AppUI, $apmconfig, $currentInfoTabId, $currentInfoTabName, $m;
		...
		if ($this->active < 0 || $AppUI->getPref ( 'TABVIEW' ) == 2) {
			...
			foreach ( $this->tabs as $k => $v ) {
				...
				$s .= '<td class="tabsp"><img src="' . apmfindImage ( 'shim.gif' ) . '" alt=""/></td>';
			}
			$s .= '</table></td></tr>';
