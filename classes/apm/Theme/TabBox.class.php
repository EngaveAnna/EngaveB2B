...
class apm_Theme_TabBox {
	 ...
	 * 
	 * @param
	 *        	string The base URL query string to prefix tab links
	 * @param
	 *        	string The base path to prefix the include file
	 * @param
	 *        	int The active tab
	 * @param
	 *        	string Optional javascript method to be used to execute tabs.
	...
	public function __construct($baseHRef = '', $baseInc = '', $active = 0, $javascript = null) {
		...
		$this->_uistyle = $this->_AppUI->getPref ( 'UISTYLE' ) ? $this->_AppUI->getPref ( 'UISTYLE' ) : apmgetConfig ( 'host_style' );
		if (! $this->_uistyle) {
			$this->_uistyle = 'apmProject';
		}
	}
	/**
	 ...
	 * @param
	 *        	The display title/name of the tab
	 */
	 ...
	 *
	 * @param
	 *        	string Can't remember whether this was useful
	 */
	public function show($extra = '', $js_tabs = false) {
		...
		if ($this->_AppUI->getPref ( 'TABVIEW' ) == 0) {
			...
			if ($extra) {
				...
				echo '<img src="' . apmfindImage ( 'shim.gif' ) . '" height="10" width="1" alt="" />';
			}
		}
		...
		if ($this->active < 0 || $this->_AppUI->getPref ( 'TABVIEW' ) == 2) {
			...
			foreach ( $this->tabs as $k => $v ) {
				...
				$s .= '<td width="1%" nowrap="nowrap" class="tabsp"><img src="' . apmfindImage ( 'shim.gif' ) . '" height="1" width="1" alt="" /></td>';
