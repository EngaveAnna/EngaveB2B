 ...
 */
class apm_Utilities_Mail extends PHPMailer {
	...
	public function __construct() {
		$this->defer = apmgetConfig ( 'mail_defer' );
		$this->canEncode = function_exists ( 'imap_8bit' ) && 'us-ascii' != $this->charset;
		$this->hasMbStr = function_exists ( 'mb_substr' );
		
		$this->Mailer = (apmgetConfig ( 'mail_transport', 'php' ) == 'smtp' ? 'smtp' : 'mail');
		$this->Port = apmgetConfig ( 'mail_port', '25' );
		$this->Host = apmgetConfig ( 'mail_host', 'localhost' );
		$this->Hostname = apmgetConfig ( 'mail_host', 'localhost' );
		$this->SMTPAuth = apmgetConfig ( 'mail_auth', false );
		$this->SMTPSecure = apmgetConfig ( 'mail_secure', '' );
		$this->SMTPDebug = apmgetConfig ( 'mail_debug', false );
		$this->Username = apmgetConfig ( 'mail_user' );
		$this->Password = apmgetConfig ( 'mail_pass' );
		$this->Timeout = apmgetConfig ( 'mail_timeout', 0 );
		$this->charset = 'utf-8';
		...
		$this->From ( apmgetConfig ( 'admin_email', 'admin@apmProject.net' ), apmgetConfig ( 'company_name' ) );
	}
	...
	public function Subject($subject) {
		$this->Subject = apmgetConfig ( 'email_prefix' ) . ' ' . $subject;
		return true;
	}
	 ...
	 * a confirmation is returned to the "From" address (or "ReplyTo" if defined)
	 ...
	 *
	 * @param string $to
	 *        	email address, accept both a single address or an array of addresses
	 * @param boolean $reset
	...
	/**
	 * Dequeue the email and transfer it.
	 * Called from the queue manager.
	 *
	 * @access public
	 */
