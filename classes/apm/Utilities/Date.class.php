<?php
/**
 * @package     apmProject\utilities
 */

/**
 * apmProject implementation of the Pear Date class
 *
 ...
 * @package apmProject\utilities
 */
class apm_Utilities_Date extends Date {
	public function __construct($datetime = null, $tz = '') {
		parent::__construct ( $datetime );
		if ($tz == '') {
			$this->setTZ ( apmgetConfig ( 'system_timezone', 'UTC' ) );
		} else {
			$this->setTZ ( $tz );
		}
	}
	...
	/**
	 * This method simply compares the two dates input.
	...
	public function compare($d1, $d2, $convertTZ = false) {
		...
		if ($days1 - $days2) {
			...
			if ($d1->hour - $d2->hour) {
				$comp_value = apmsgn ( $d1->hour - $d2->hour );
			} else {
				if ($d1->minute - $d2->minute) {
					$comp_value = apmsgn ( $d1->minute - $d2->minute );
				} else {
					if ($d1->second - $d2->second) {
						$comp_value = apmsgn ( $d1->second - $d2->second );
					}
				}
			}
		}
		return apmsgn ( $comp_value );
	}
	 ...
	 * 
	 * @param
	 *        	int Positive or negative number of months
	 */
	...
	public function isWorkingDay() {
		...
		if ($AppUI->isActiveModule ( 'holiday' )) {
			...
			require_once apm_BASE_DIR . "/modules/holiday/holiday_functions.class.php";
			if (HolidayFunctions::isHoliday ( $this )) {
				return false;
			}
		}
		
		$working_days = apmgetConfig ( 'cal_working_days' );
		$working_days = ((is_null ( $working_days )) ? array (
				'1',
				'2',
				'3',
				'4',
	...
	public function isSameDay($otherDay) {
		return ($this->getDay () == $otherDay->getDay () && $this->getMonth () == $otherDay->getMonth () && $this->getYear () == $otherDay->getYear ());
	}
	 ...
	 *
	 * @param bool $preserveHours
	 *        	Determine whether to set time to start of day or preserve the time of the given object
	 * @return $this
	 */
	public function next_working_day($preserveHours = false) {
		$do = clone $this;
		$end = ( int ) apmgetConfig ( 'cal_day_end' );
		$start = ( int ) apmgetConfig ( 'cal_day_start' );
	 ...
	 * 
	 * @param bool $preserveHours
	 *        	Determine whether to set time to end of day or preserve the time of the given object
	 * @return $this
	 */
	...
	public function addDuration($duration = '8', $durationType = '1') {
		...
		switch ($durationType) {
			case '24' :
				...
				break;
			case '1' :
				// get apm time constants
				$cal_day_start = ( int ) apmgetConfig ( 'cal_day_start' );
				$cal_day_end = ( int ) apmgetConfig ( 'cal_day_end' );
				$dwh = ( int ) apmgetConfig ( 'daily_working_hours' );
	 ...
	 *
	 * @param
	 *        	obj DateObject may be viewed as end date
	...
	public function calcDuration($endDate) {
		...
		$startDate = new apm_Utilities_Date ();
		$startDate->copy ( $this );
		
		// get apm time constants
		$day_start_hour = ( int ) apmgetConfig ( 'cal_day_start' );
		$day_end_hour = ( int ) apmgetConfig ( 'cal_day_end' );
		$work_hours = ( int ) apmgetConfig ( 'daily_working_hours' );
	...
	/**
	 * Clone the current apm_Utilities_Date object
	 *
	...
	public function duplicate() {
		 ...
		 * This method also appears in the apm_Database_Query and apm_Core_BaseObject (modified) class.
		 */
		return clone ($this);
	}
	 ...
	 *        	Type: 1=hours, 24=days
	 * @return apm_Utilities_Date The apm_Utilities_Date object of the finish date
	 */
	public function calcFinish($duration, $durationType) {
		
		// since one will alter the date ($this) one better copies it to a new instance
		$finishDate = new apm_Utilities_Date ();
		...
		$day_end_hour = ( int ) apmgetConfig ( 'cal_day_end' );
		$work_hours = ( int ) apmgetConfig ( 'daily_working_hours' );
		$min_increment = ( int ) apmgetConfig ( 'cal_day_increment' );
	...
	public function convertTZ($tz) {
		...
		$dt = new DateTime ( $this->format ( '%D %H%M%S' ), new DateTimeZone ( $this->tz ['id'] ) );
