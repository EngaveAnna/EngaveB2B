...
/*        var cdiv =this.createButtonToolbar();
        document.getElementById('module-tools').appendChild(cdiv);*/
        

        
        
        if(this.$table.hasClass('bindedToolbar'))
        {
        	this.createButtonToolbar();
        }
        else
        {
        	this.createButtonNavMenu();
        	$('#module-tools').append(this.$dropdownGroup);
        }
    ...
    // Create button i nav-menu
    ResponsiveTable.prototype.createButtonNavMenu = function() {
        var that = this;

        this.$dropdownGroup = $('<div class="btn-group dropdown-btn-group pull-right" />');
        this.$dropdownBtn = $('<button class="btn btn-default dropdown-toggle" data-toggle="dropdown"><span class="glyphicon glyphicon-th-list"></span>&nbsp;'+document.getElementById('dropdownBtn-label').value+'&nbsp;<span class="caret"></span></button>');
        this.$dropdownContainer = $('<ul class="dropdown-menu"/>');

         // Display-all btn
        if(this.options.addDisplayAllBtn) {
            // Create display-all btn
            this.$displayAllBtn = $('<a type="button" role="button" title="" class="btn btn-default" data-toggle="tooltip" data-placement="top" data-original-title="'+document.getElementById('displayAllBtn-label').value+'"><span class="glyphicon glyphicon-eye-open" data-toggle="tooltip" data-original-title="'+document.getElementById('displayAllBtn-label').value+'"></span></a>');
            // Add display-all btn to dropdown-btn-group
            this.$dropdownGroup.append(this.$displayAllBtn);

            if (this.$table.hasClass('display-all')) {
                // add 'btn-info' class to btn to indicate that display all is activated
                this.$displayAllBtn.addClass('btn-info');
            }
    ...
    ResponsiveTable.prototype.activateFocus = function() {
        ...
        if(this.$focusBtn){
            this.$focusBtn.toggleClass('btn-info');
        }
    ...
    ResponsiveTable.prototype.displayAll = function(activate, trigger) {
        if(this.$displayAllBtn){
            // add 'btn-info' class to btn to indicate that display all is activated
            this.$displayAllBtn.toggleClass('btn-info', activate);
        }
            ...
            if ( $th.is('[data-priority]') ) {
                ...
                .change(function(){ // bind change event on checkbox
                    ...
                    if(that.$table.hasClass('display-all')){
                        ...
                        //switch off button
                        that.$displayAllBtn.removeClass('btn-info');
                    }
