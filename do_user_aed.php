...
if (apmgetConfig ( 'activate_external_user_creation' ) != 'true') {
	die ( 'You should not access this file directly' );
}

$username = apmgetParam ( $_POST, 'user_username', 0 );
...
$email = apmgetParam ( $_POST, 'contact_email', 0 );
...
if (count ( $contact->getError () )) {
	...
	if (count ( $user->getError () )) {
		...
		notifyHR ( apmgetConfig ( 'admin_email', 'admin@apmProject.net' ), 'apm System Human Resources', $contact->contact_email, $contact->contact_first_name, $user->user_username, $_POST ['user_password'], $user->user_id );
