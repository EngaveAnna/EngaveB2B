...
class style_classic extends apm_Theme_Base {
	public function __construct($AppUI, $m = '') {
		$this->_uistyle = 'classic';
...
class CTabBox extends apm_Theme_TabBox {
	public function show($extra = '', $js_tabs = false, $alignment = 'left', $opt_flat = true) {
		...
		if ($this->active < 0 || $this->_AppUI->getPref ( 'TABVIEW' ) == 2) {
			...
			$s = '<div class="with-nav-tabs"><ul class="nav nav-tabs responsive nav-space">';
			...
			foreach ( $this->tabs as $k => $v ) {
				...
				$s .= '<li  ' . $class . ' id="tab' . $k . '"><a href="';
				$s .= $this->baseHRef . 'tab=' . $k . '#' . 'tab' . $k;
