...
$object_id = ( int ) apmgetParam ( $_GET, 'user_id', 0 );
...
<script language="javascript" type="text/javascript">
    function submitIt(){
        ...
        if (form.user_username.value.length < <?php echo apmgetConfig('username_min_len'); ?> && form.user_username.value != '<?php echo apmgetConfig('admin_username'); ?>') {
            alert("<?php echo $AppUI->_('adminValidUserName', UI_OUTPUT_JS); ?>"  + <?php echo apmgetConfig('username_min_len'); ?>);
            ...
            alert("<?php echo $AppUI->_('adminValidPassword', UI_OUTPUT_JS); ?>" + <?php echo apmgetConfig('password_min_len'); ?>);
...
<form name="frmDelete" action="./index.php?m=users" method="post"
	...
	<input type="hidden" name="dosql" value="do_user_aed" /> <input
		...
		name="company_id" value="<?php echo $object_id; ?>" />
</form>
<?php } ?>
</script>
<?php
