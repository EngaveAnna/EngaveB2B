...
if (isset ( $_POST ['task_type'] )) {
	$AppUI->setState ( 'ToDoTaskType', apmgetParam ( $_POST, 'task_type', '' ) );
}

$task_type = $AppUI->getState ( 'ToDoTaskType' ) !== null ? $AppUI->getState ( 'ToDoTaskType' ) : '';

$project_id = ( int ) apmgetParam ( $_GET, 'project_id', 0 );
$this_day = new apm_Utilities_Date ();
$date = (( int ) apmgetParam ( $_GET, 'date', '' )) ? $this_day->format ( FMT_TIMESTAMP_DATE ) : '';
...
if (canView ( 'users' )) { // let's see if the user has sysadmin access
	$other_users = true;
	if (($show_uid = apmgetParam ( $_REQUEST, 'show_user_todo', 0 )) != 0) { // lets see if the user wants to see anothers user mytodo
		$user_id = $show_uid;
		$no_modify = true;
...
$task_sort_item1 = apmgetParam ( $_GET, 'task_sort_item1', '' );
$task_sort_type1 = apmgetParam ( $_GET, 'task_sort_type1', '' );
$task_sort_item2 = apmgetParam ( $_GET, 'task_sort_item2', '' );
...
$task_priority = apmgetParam ( $_POST, 'task_priority', 99 );
$selected = apmgetParam ( $_POST, 'selected_task', 0 );
...
if ('todo' == $a) {
	$titleBlock = new apm_Theme_TitleBlock ( 'My Tasks To Do', 'icon.png', $m );
...
if ($m == 'tasks' && $a == 'todo') {
	...
	$tabBox->add ( apm_BASE_DIR . '/modules/tasks/todo_tasks_sub', 'My Tasks' );
	$tabBox->add ( apm_BASE_DIR . '/modules/tasks/todo_gantt_sub', 'My Gantt' );
	$tabBox->show ();
} else {
	include apm_BASE_DIR . '/modules/tasks/todo_tasks_sub.php';
}