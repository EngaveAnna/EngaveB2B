 ...
 * @license     Clear BSD
 * @link        http://www.softmag.eu
 */
class CTask_Log extends apm_Core_BaseObject {
	/**
	...
	protected function hook_preStore() {
		...
		if ($this->task_log_date) {
			$date = new apm_Utilities_Date ( $this->task_log_date );
	 ...
	 *        	not used
	 * @param int $task_id
	 *        	that task id of task this task log is for
	 *        	
	 * @return void
	 *
	 * @access protected
	...
	protected function updateTaskSummary($notUsed = null, $task_id) {
		...
		if ($this->_perms->checkModuleItem ( 'tasks', 'edit', $task_id )) {
			...
			$old_end_date = new apm_Utilities_Date ( $task->task_end_date );
			$new_end_date = new apm_Utilities_Date ( $this->task_log_task_end_date );
	 ...
	 *        	user id to test
	 * @param string $fields
	 *        	optional fields to be returned by the query, default is all
	 * @param string $orderby
	 *        	optional sort order for the query
	 * @param int $index
	 *        	optional name of field to index the returned array
	 * @param array $extra
	 *        	optional array of additional sql parameters (from and where supported)
	 *        	
	 * @return array
	 */
