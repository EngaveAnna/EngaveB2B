...
global $caller, $showWork, $showLabels, $showPinned, $showArcProjs, $showHoldProjs, $showDynTasks, $showLowTasks, $user_id;

apmsetExecutionConditions ( $apmconfig );

$project_id = ( int ) apmgetParam ( $_REQUEST, 'project_id', 0 );
$f = apmgetParam ( $_REQUEST, 'f', 0 );

$showLabels = ( int ) apmgetParam ( $_REQUEST, 'showLabels', 0 );
$showWork = ( int ) apmgetParam ( $_REQUEST, 'showWork', 0 );

$ganttTaskFilter = ( int ) apmgetParam ( $_REQUEST, 'ganttTaskFilter', 0 );
$showPinned = ( int ) apmgetParam ( $_REQUEST, 'showPinned', false );
$showArcProjs = ( int ) apmgetParam ( $_REQUEST, 'showArcProjs', false );
$showHoldProjs = ( int ) apmgetParam ( $_REQUEST, 'showHoldProjs', false );
$showDynTasks = ( int ) apmgetParam ( $_REQUEST, 'showDynTasks', false );
$showLowTasks = ( int ) apmgetParam ( $_REQUEST, 'showLowTasks', true );

// Get the state of formatting variables here /////////////////////////////////////////////////////
$showTaskNameOnly = ( int ) apmgetParam ( $_REQUEST, 'showTaskNameOnly', 0 );
$showNoMilestones = ( int ) apmgetParam ( $_REQUEST, 'showNoMilestones', 0 );
$showMilestonesOnly = ( int ) apmgetParam ( $_REQUEST, 'showMilestonesOnly', 0 );
$showhgrid = ( int ) apmgetParam ( $_REQUEST, 'showhgrid', 0 );
$printpdfhr = ( int ) apmgetParam ( $_REQUEST, 'printpdfhr', 0 );
$addLinksToGantt = ( int ) apmgetParam ( $_REQUEST, 'addLinksToGantt', 0 );
$monospacefont = ( int ) apmgetParam ( $_REQUEST, 'monospacefont', 0 );
// Get the state of formatting variables here /////////////////////////////////////////////////////
 ...
 */
$caller = apmgetParam ( $_REQUEST, 'caller', null );

$task = new CTask ();

if ($caller == 'todo') {
	$user_id = apmgetParam ( $_REQUEST, 'user_id', $AppUI->user_id );
...
foreach ( $proTasks as $row ) {
	...
	$tsd = new apm_Utilities_Date ( $row ['task_start_date'] );
	if ($tsd->before ( new apm_Utilities_Date ( $start_min ) )) {
		$start_min = $row ['task_start_date'];
	}
	
	$row ['task_end_date'] = __extract_from_projects_gantt4 ( $row );
	
	$ted = new apm_Utilities_Date ( $row ['task_end_date'] );
	if ($ted->after ( new apm_Utilities_Date ( $end_max ) )) {
		$end_max = $row ['task_end_date'];
	}
	if ($ted->after ( new apm_Utilities_Date ( $projects [$row ['task_project']] ['project_end_date'] ) ) || $projects [$row ['task_project']] ['project_end_date'] == '') {
...
$width = min ( apmgetParam ( $_GET, 'width', 600 ), 1400 );
$start_date = apmgetParam ( $_GET, 'start_date', $start_min );
$end_date = apmgetParam ( $_GET, 'end_date', $end_max );
...
if (! $start_date || ! $end_date) {
	...
	$d_start = new apm_Utilities_Date ();
	$d_end = new apm_Utilities_Date ();
...
$gantt = new apm_Output_GanttRenderer ( $AppUI, $width );
$gantt->localize ();
...
for($i = 0, $i_cmp = count ( $gantt_arr ); $i < $i_cmp; $i ++) {
	...
	if ($canAccess) {
		...
		$start = ( int ) ($a ['task_start_date']) ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $a ['task_start_date'], '%Y-%m-%d %T' ) ) : new apm_Utilities_Date ();
		$start = $start->getDate ();
		
		$end = ( int ) ($a ['task_end_date']) ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $a ['task_end_date'], '%Y-%m-%d %T' ) ) : new apm_Utilities_Date ();
		$end = $end->getDate ();
		...
		if ($flags == 'm') {
			...
			if ($showNoMilestones != '1') {
				$start = new apm_Utilities_Date ( $start );
				$start->addDays ( 0 );
			...
			$dur = $a ['task_duration'];
			if ($type == 24) {
				$dur *= $apmconfig ['daily_working_hours'];
			}
