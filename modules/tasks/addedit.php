...
$object_id = ( int ) apmgetParam ( $_GET, 'task_id', 0 );
...
$task_parent = ( int ) apmgetParam ( $_GET, 'task_parent', $object->task_parent );

// check for a valid project parent
$task_project = ( int ) $object->task_project;
if (! $task_project) {
	$task_project = ( int ) apmgetParam ( $_REQUEST, 'task_project', 0 );
	if (! $task_project) {
...
$parents = array ();
$projTasksWithEndDates = array (
		$object->task_id => $AppUI->_ ( 'None' ) 
...
$department_selection_list = arrayMerge ( array (
		'0' => '' 
...
if (isset ( $apmconfig ['check_task_dates'] ) && $apmconfig ['check_task_dates'])
	echo 'true';
else
	echo 'false';
?>;
...
var workHours = <?php echo apmgetConfig('daily_working_hours'); ?>;
//working days array from config.php
var working_days = new Array(<?php echo apmgetConfig('cal_working_days'); ?>);
var cal_day_start = <?php echo (int) apmgetConfig('cal_day_start'); ?>;
var cal_day_end = <?php echo (int) apmgetConfig('cal_day_end'); ?>;
var daily_working_hours = <?php echo (int) apmgetConfig('daily_working_hours'); ?>;
...
include ( apm_BASE_DIR . '/modules/tasks/ae_desc.php');
include ( apm_BASE_DIR . '/modules/tasks/ae_resource.php');
include ( apm_BASE_DIR . '/modules/tasks/ae_dates.php');
include ( apm_BASE_DIR . '/modules/tasks/ae_depend.php');
//$tabBox->show ( '', true );
