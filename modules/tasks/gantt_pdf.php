...
global $gantt_arr, $apmconfig, $gtask_sliced, $printpdfhr, $showNoMilestones;
...
$width = 1600;
$gantt_start_date = apmgetParam ( $_GET, 'start_date', $start_min );
$gantt_end_date = apmgetParam ( $_GET, 'end_date', $end_max );

$s1 = ($gantt_start_date) ? new apm_Utilities_Date ( $gantt_start_date ) : new apm_Utilities_Date ();
$e1 = ($gantt_end_date) ? new apm_Utilities_Date ( $gantt_end_date ) : new apm_Utilities_Date ();
...
foreach ( $gtask_sliced as $gts ) {
	...
	for($i = 0; $i < $gts_count; $i ++) {
		...
		$start_date = new apm_Utilities_Date ( $a ['task_start_date'] );
		$end_date = new apm_Utilities_Date ( $a ['task_end_date'] );
		...
		if ($flags == 'm') {
			...
			if ($showNoMilestones != '1') {
				$start = new apm_Utilities_Date ( $start_date );
				$start->addDays ( 0 );
			...
			if ($showWork == '1') {
				...
				$work_hours = $wh * $apmconfig ['daily_working_hours'];
			...
			$dur .= ' h';
			$enddate = new apm_Utilities_Date ( $end );
			$startdate = new apm_Utilities_Date ( $start );
	...
	$filename = apm_BASE_DIR . "/files/temp/GanttPNG_" . md5 ( time () ) . "_$page.png";
...
$output = new apm_Output_PDFRenderer ( 'A4', 'landscape' );
$pdf = $output->getPDF ();
...
$gpdfkey = apm_BASE_DIR . '/modules/tasks/images/ganttpdf_key.png';
$gpdfkeyNM = apm_BASE_DIR . '/modules/tasks/images/ganttpdf_keyNM.png';
