 ...
 */
$reminded = ( int ) apmgetParam ( $_GET, 'reminded', 0 );
if ($reminded) {
	$obj->clearReminder ();
}
...
$titleBlock = new apm_Theme_TitleBlock ( 'View Task', 'icon.png', $m );
$titleBlock->addCell ();
if ($canReadProject) {
...
$view = new apm_Controllers_View ( $AppUI, $obj, 'Task' );
...
if ($obj->task_dynamic != 1 && 0 == $obj->task_represents_project) {
	...
	$tabBox_show = 1;
	if (canView ( 'task_log' )) {
		$tabBox->add ( apm_BASE_DIR . '/modules/tasks/vw_logs', 'Task Logs' );
	}
	if ($task_log_id == 0) {
		if (canAdd ( 'task_log' )) {
			$tabBox->add ( apm_BASE_DIR . '/modules/tasks/vw_log_update', 'Log editor' );
		}
	} elseif (canEdit ( 'task_log' )) {
		$tabBox->add ( apm_BASE_DIR . '/modules/tasks/vw_log_update', 'Edit Log' );
...
if (count ( $obj->getChildren () ) > 0) {
	...
	$tabBox->add ( apm_BASE_DIR . '/modules/tasks/vw_tasks', 'Child Tasks' );
}
