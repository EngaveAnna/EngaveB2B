 ...
 */
class CTask extends apm_Core_BaseObject {
	 ...
	 *
	 */
	public static $tracked_dynamics = array (
			'0' => '0',
			'1' => '1',
			'2' => '31' 
	);
	 ...
	 *
	 */
	public static $tracking_dynamics = array (
			'0' => '21',
			'1' => '31' 
	);
	...
	public function isValid() {
		...
		if (apmgetConfig ( 'check_task_dates' )) {
		...
		static $addedit;
		if (! isset ( $addedit )) {
			$addedit = apmgetParam ( $_POST, 'dosql', '' ) == 'do_task_aed' ? true : false;
		}
		 ...
		 */
		if ($addedit) {
			$hdependencies = apmgetParam ( $_POST, 'hdependencies', '0' );
			if ($hdependencies) {
	 ...
	 * apm_Core_BaseObject once we're sure no one is using the $skipUpdate
	 * parameter any more.
	 */
	...
	public function getHoursScheduled($project_id) {
		...
		$q->clear ();
		$q->addTable ( 'tasks' );
		$q->addQuery ( ' SUM(task_duration * ' . apmgetConfig ( 'daily_working_hours' ) . ')' );
		$q->addWhere ( "task_project = $project_id AND task_duration_type <> 1 AND task_dynamic <> 1" );
	...
	public function updateDynamics() {
		...
		foreach ( $dynamics as $key => $data ) {
			...
			if (1 == $data ['task_duration_type']) {
				...
				$children_allocated_total = round ( $children_allocated_total / apmgetConfig ( 'daily_working_hours' ), 2 );
				$duration_type = 24;
			}
			...
			$q->clear ();
			$q->addTable ( 'tasks' );
			$q->addQuery ( 'SUM(task_percent_complete * task_duration * ' . apmgetConfig ( 'daily_working_hours' ) . ')' );
			$q->addWhere ( "task_path_enumeration LIKE '$path/%' AND task_duration_type <> 1 AND task_dynamic <> 1" );
			$weighted_hours_worked += ( float ) $q->loadResult ();
	...
	public function store($unused = null) {
		...
		if ($this->{$this->_tbl_key} && $this->canEdit ()) {
			...
			$stored = parent::store ();
				...
				echo "</br>There was an error in storing 1 this object.";
		...
		if (0 == $this->{$this->_tbl_key} && $this->canCreate ()) {
			$stored = parent::store ();
			if (! parent::store ()) {
				echo "</br>There was an error in storing this object.";
				$this->_error ['store'] = "There was an error in storing this object.";
			}
		}
		return $stored;
	...
	/**
	 *
	 * @param apm_Core_CAppUI $AppUI        	
	 * @param CProject $project_id
	 *        	The point of this function is to create/update a task to represent a
	 *        	subproject.
	 *        	
	 */
	public static function storeTokenTask(apm_Core_CAppUI $AppUI, $project_id) {
	...
	/**
	 * This function is run immediately after a Task is stored.
	 * It uses that Task's
	...
	public function pushDependencies($task_id, $lastEndDate) {
		...
		foreach ( $dependent_tasks as $_task_id => $_task_data ) {
			...
			$nsd = new apm_Utilities_Date ( $lastEndDate );
			
			$osd = new apm_Utilities_Date ( $_task_data ['task_start_date'] );
			
			$ned = new apm_Utilities_Date ( $_task_data ['task_end_date'] );
	 ...
	 *
	 * @param
	 *        	integer ID of the task we want dependencies
	 * @return string comma delimited list of tasks id's
	 *        
	 */
	...
	public function notifyOwner() {
		...
		if (count ( $users )) {
			$emailManager = new apm_Output_EmailManager ( $this->_AppUI );
	...
	public function email_log(&$log, $assignees, $task_contacts, $project_contacts, $others, $extras, $specific_user = 0) {
		...
		if (( int ) $this->task_id > 0 && ( int ) $this->task_project > 0) {
			...
			if ($save_email) {
				// TODO: This is where #38 - http://bugs.apmProject.net/view.php?id=38 - should be applied if a change is necessary.
	 ...
	 * 
	 * @param
	 *        	integer ID of the master task
	 * @param
	 *        	boolean true if is a dep call (recurse call)
	 * @param
	 *        	boolean false for no recursion (needed for calc_end_date)
	 *        	
	 */
	...
	public function getTaskDurationPerDay($use_percent_assigned = false) {
		$duration = $this->task_duration * ($this->task_duration_type == 24 ? apmgetConfig ( 'daily_working_hours' ) : $this->task_duration_type);
		$task_start_date = new apm_Utilities_Date ( $this->task_start_date );
		$task_finish_date = new apm_Utilities_Date ( $this->task_end_date );
	...
	public function getTaskDurationPerWeek($use_percent_assigned = false) {
		...
		$number_of_weeks_worked = $task_finish_date->workingDaysInSpan ( $task_start_date ) / count ( explode ( ',', apmgetConfig ( 'cal_working_days' ) ) );
	...
	public function updateAssigned($cslist, $perc_assign, $del = true, $rmUsers = false) {
		...
		foreach ( $tarr as $user_id ) {
			if (( int ) $user_id) {
				...
				if (apmgetConfig ( 'check_overallocation' ) && $perc > $alloc [$user_id] ['freeCapacity']) {
	 ...
	 * 
	 * @param
	 *        	string hash a hash for the returned hashList
	 * @param
	 *        	array users an array of user_ids calculating their assignment capacity
	...
	public function getAllocation($hash = null, $users = null, $get_user_list = false) {
		 ...
		 * handled properly.
		 */
		if (! apmgetConfig ( 'check_overallocation', false )) {
			if ($get_user_list) {
				$users_list = apmgetUsersHashList ();
	...
	public function canUserEditTimeInformation($project_owner = 0, $user_id = 0) {
		...
		if (apmgetConfig ( 'restrict_task_time_editing' ) == true && $this->task_id > 0) {
			...
			if (apmgetConfig ( 'restrict_task_time_editing' ) == false || $this->task_id == 0) {
	...
	public function addReminder() {
		...
		if (! apmgetConfig ( 'task_reminder_control' )) {
			return;
		}
		...
		$eq = new apm_System_EventQueue ();
		$pre_charge = apmgetConfig ( 'task_reminder_days_before', 1 );
		$repeat = apmgetConfig ( 'task_reminder_repeat', 100 );
		...
		$date = new apm_Utilities_Date ( $this->task_end_date );
		$today = new apm_Utilities_Date ( date ( 'Y-m-d' ) );
		...
		$eq->add ( array (
				$this,
				'remind' 
		), $args, 'tasks', false, $this->task_id, 'remind', $start_day, $day, $repeat );
	}
	 ...
	 *        	Module name (not used)
	 * @param string $notUsed2
	 *        	Type of event (not used)
	 * @param integer $id
	 *        	ID of task being reminded
	 * @param integer $owner
	 *        	Originator of event
	 * @param mixed $notUsed
	...
	public function remind($notUsed = null, $notUsed2 = null, $id, $owner, $notUsed3 = null) {
		...
		$starts = new apm_Utilities_Date ( $this->task_start_date );
		$expires = new apm_Utilities_Date ( $this->task_end_date );
		...
		if (! $project->project_active || $project->project_status == apmgetConfig ( 'template_projects_status_id', 0 )) {
			return - 1;
		}
	...
	public function hook_calendar($userId) {
		...
		foreach ( $taskList as $taskItem ) {
			// $taskArray[] = $taskItem;
			$taskArray [] = array_merge ( $taskItem, array (
					'sequence' => $taskItem ['sequence'],
					'endDate' => $taskItem ['startDate'],
					'name' => 'Start: ' . $taskItem ['name'],
					'UID' => 'tasks_' . $taskItem ['id'] . 'S' 
			) );
			$taskArray [] = array_merge ( $taskItem, array (
					'sequence' => $taskItem ['sequence'],
					'startDate' => $taskItem ['endDate'],
					'name' => 'End: ' . $taskItem ['name'],
	...
	public function hook_search() {
		...
		$search ['table_joins'] = array (
				array (
						'table' => 'task_log',
						'alias' => 'tl',
						'join' => 't.task_id = tl.task_log_task' 
				) 
		);
	...
	public function getTaskList($userId, $days = 30) {
		...
		$q->addQuery ( 'CONCAT(\'' . apm_BASE_URL . '/index.php?m=tasks&a=view&task_id=' . '\', t.task_id) as url' );
