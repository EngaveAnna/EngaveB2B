...
$viewtype = apmgetParam ( $_REQUEST, 'viewtype', 'normal' );
$hideEmail = apmgetConfig ( 'hide_email_addresses', false );
...
<form name="messageForm" method="post"
	action="?m=forums&forum_id=<?php echo $forum_id; ?>"
	...
	<tr>
		<td colspan="2">
			<div class="left" style="padding-left: 20px;">
                ...
                <input type="radio" name="viewtype" value="normal"
						<?php echo ($viewtype == 'normal') ? 'checked' : ''; ?>
						onclick="this.form.submit();" /><?php echo $AppUI->_('Normal')?>
                <input type="radio" name="viewtype" value="short"
						<?php echo ($viewtype == 'short') ? 'checked' : ''; ?>
						onclick="this.form.submit();" /><?php echo $AppUI->_('Collapsed')?>
                <input type="radio" name="viewtype" value="single"
						<?php echo ($viewtype == 'single') ? 'checked' : ''; ?>
						onclick="this.form.submit();" /><?php echo $AppUI->_('Single Message at a time')?>
            </form>
			</div>
			<div class="right">
				<input type="button" class="btn btn-default"
					value="<?php echo $AppUI->_('Sort By Date') . ' (' . $AppUI->_($sort) . ')'; ?>"
					onclick="javascript:window.location='./index.php?m=forums&a=viewer&forum_id=<?php echo $forum_id; ?>&message_id=<?php echo $message_id; ?>&sort=<?php echo $sort; ?>'" />
            <?php if ($canAuthor) { ?>
                <input type="button" class="btn btn-default"
					value="<?php echo $AppUI->_('Post Reply'); ?>"
					onclick="javascript:window.location='./index.php?m=forums&a=viewer&forum_id=<?php echo $forum_id; ?>&message_parent=<?php echo $message_id; ?>&post_message=1';" />
				<input type="button" class="btn btn-default"
					value="<?php echo $AppUI->_('New Topic'); ?>"
					onclick="javascript:window.location='./index.php?m=forums&a=viewer&forum_id=<?php echo $forum_id; ?>&message_id=0&post_message=1';" />
            <?php } ?>
        </div>
		</td>
	</tr>
...
foreach ( $messages as $row ) {
	...
	$date = intval ( $row ['message_date'] ) ? new apm_Utilities_Date ( $row ['message_date'] ) : null;
	if ($viewtype != 'single') {
		$s = '';
	}
