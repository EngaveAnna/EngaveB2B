...
function load_new_stuff(){
 ...
 $(".msgs").load("./index.php?m=forums&a=chat_msgs&suppressHeaders=true",function(){
  if(localStorage['lpid']!=$(".msgs .msg:last").attr("title")){
   scTop();
  }
 });
 $(".users").load("./index.php?m=forums&a=chat_users&suppressHeaders=true");
}
$(document).ready(function(){
 ...
 $("#msg_form").on("submit",function(){
  ...
  if(val!=""){
   ...
   $.post("./index.php?m=forums&a=chat_send&suppressHeaders=true",{msg:val},function(){
    load_new_stuff();
    $("#send_status").remove();
    t[0].reset();
   });
