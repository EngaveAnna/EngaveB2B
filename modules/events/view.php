...
$event_id = ( int ) apmgetParam ( $_GET, 'event_id', 0 );
...
$start_date = $event->event_start_date ? new apm_Utilities_Date ( $event->event_start_date ) : new apm_Utilities_Date ();
$end_date = $event->event_end_date ? new apm_Utilities_Date ( $event->event_end_date ) : new apm_Utilities_Date ();
...
$titleBlock = new apm_Theme_TitleBlock ( 'View Event', 'icon.png', $m );
...
if ($canEdit) {
	...
	$titleBlock->addCell ( '
		<form action="?m=events&amp;a=addedit" method="post" accept-charset="utf-8">
			<input type="submit" class="btn btn-default" value="' . $AppUI->_ ( 'new event' ) . '" />
		</form>', '', '', '' );
...
$view = new apm_Controllers_View ( $AppUI, $event, 'Event' );
