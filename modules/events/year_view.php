...
$titleBlock = new apm_Theme_TitleBlock ( 'Yearly Calendar', 'icon.png', $m );
...
$titleBlock->addCell ( arraySelect ( $event_filter_list, 'event_filter', 'onChange="document.pickFilter.submit()" class="form-control"', $event_filter, true ), '', '<form action="' . $_SERVER ['REQUEST_URI'] . '" method="post" name="pickFilter" accept-charset="utf-8">', '</form>' );
...
<table class="std view">
	<tr>
		<td align="center">
				...
				<tr>
					<td colspan="20" valign="top">
						<table border="0" cellspacing="1" cellpadding="2" width="100%"
							...
							<tr>
								<td><a
									href="<?php echo '?m=events&a=year_view&date=' . $prev_year; ?>"><img
										src="<?php echo apmfindImage('prev.gif'); ?>" alt="pre"
										title="pre" /></a></td>
								<th width="100%" align="center">
				                	<?php echo htmlspecialchars($date->format('%Y')); ?>
				                </th>
								<td><a
									href="<?php echo '?m=events&a=year_view&date=' . $next_year; ?>"><img
										src="<?php echo apmfindImage('next.gif'); ?>" alt="next"
										title="next" /></a></td>
							</tr>
						</table>
					</td>
			...
			<?php
			$minical = new apm_Output_MonthCalendar ( $date );
