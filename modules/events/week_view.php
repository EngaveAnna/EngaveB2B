...
if (isset ( $_REQUEST ['company_id'] )) {
	$AppUI->setState ( 'CalIdxCompany', ( int ) apmgetParam ( $_REQUEST, 'company_id', 0 ) );
}
...
$date = apmgetParam ( $_GET, 'date', null );
...
$this_week = new apm_Utilities_Date ( $date );
$dd = $this_week->getDay ();
$mm = $this_week->getMonth ();
$yy = $this_week->getYear ();

// prepare time period for 'events'
$first_time = new apm_Utilities_Date ( Date_Calc::beginOfWeek ( $dd, $mm, $yy, FMT_TIMESTAMP_DATE, LOCALE_FIRST_DAY ) );
$first_time->setTime ( 0, 0, 0 );
$last_time = new apm_Utilities_Date ( Date_Calc::endOfWeek ( $dd, $mm, $yy, FMT_TIMESTAMP_DATE, LOCALE_FIRST_DAY ) );
$last_time->setTime ( 23, 59, 59 );

$prev_week = new apm_Utilities_Date ( Date_Calc::beginOfPrevWeek ( $dd, $mm, $yy, FMT_TIMESTAMP_DATE, LOCALE_FIRST_DAY ) );
$next_week = new apm_Utilities_Date ( Date_Calc::beginOfNextWeek ( $dd, $mm, $yy, FMT_TIMESTAMP_DATE, LOCALE_FIRST_DAY ) );
...
$titleBlock = new apm_Theme_TitleBlock ( 'Week View', 'icon.png', $m );
...
$titleBlock->addCell ( '<form action="' . $_SERVER ['REQUEST_URI'] . '" method="post" name="pickCompany" accept-charset="utf-8">' . arraySelect ( $companies, 'company_id', 'onchange="document.pickCompany.submit()" class="form-control"', $company_id ) . '</form>' );
$titleBlock->addCell ( $AppUI->_ ( 'Company' ) . ':' );
$titleBlock->addCell ( '<form action="' . $_SERVER ['REQUEST_URI'] . '" method="post" name="pickFilter" accept-charset="utf-8">' . arraySelect ( $event_filter_list, 'event_filter', 'onchange="document.pickFilter.submit()" class="form-control"', $event_filter, true ) . '</form>' );
...
<table border="0" cellspacing="0" cellpadding="2" width="100%"
	...
	<tr>
		<td><a
			href="<?php echo '?m=events&a=week_view&date=' . $prev_week->format(FMT_TIMESTAMP_DATE); ?>"><img
				src="<?php echo apmfindImage('prev.gif'); ?>" alt="pre" /></a></td>
		<th width="100%">
		<?php echo $AppUI->_('Week') . ' ' . $first_time->format('%U - %Y') . ' - ' . $AppUI->_($first_time->format('%B')); ?>
	</th>
		<td><a
			href="<?php echo '?m=events&a=week_view&date=' . $next_week->format(FMT_TIMESTAMP_DATE); ?>"><img
				src="<?php echo apmfindImage('next.gif'); ?>" alt="next" /></a></td>
	</tr>
</table>
...
$workingDays = explode ( ',', apmgetConfig ( 'cal_working_days' ) );
...
?>
<tr>
		<td colspan="7" align="right" bgcolor="#efefe7"><a
			href="./index.php?m=events&a=day_view"><?php echo $AppUI->_('today'); ?></a>
		</td>
	</tr>
</table>