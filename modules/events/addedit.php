...
$object_id = intval ( apmgetParam ( $_GET, 'event_id', 0 ) );
...
// get the passed timestamp (today if none)
$event_project = ( int ) apmgetParam ( $_GET, 'project_id', 0 );
...
$start_date = intval ( $object->event_start_date ) ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $object->event_start_date, '%Y-%m-%d %T' ) ) : new apm_Utilities_Date ();
$end_date = intval ( $object->event_end_date ) ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $object->event_end_date, '%Y-%m-%d %T' ) ) : $start_date;
...
$titleBlock = new apm_Theme_TitleBlock ( ($object_id ? 'Edit Event' : 'Add Event'), 'icon.png', $m );
...
$inc = intval ( apmgetConfig ( 'cal_day_increment' ) ) ? intval ( apmgetConfig ( 'cal_day_increment' ) ) : 30;
if (! $object_id && ! $is_clash) {
	
	$seldate = new apm_Utilities_Date ( $date, $AppUI->getPref ( 'TIMEZONE' ) );
	...
	if ($h && $h < apmgetConfig ( 'cal_day_end' )) {
		...
		$seldate->setTime ( apmgetConfig ( 'cal_day_start' ), 0, 0 );
		$seldate->convertTZ ( 'UTC' );
		$object->event_start_date = $seldate->format ( FMT_TIMESTAMP );
		$seldate->convertTZ ( $AppUI->getPref ( 'TIMEZONE' ) );
		$seldate->setTime ( apmgetConfig ( 'cal_day_end' ), 0, 0 );
		$seldate->convertTZ ( 'UTC' );
		$object->event_end_date = $seldate->format ( FMT_TIMESTAMP );
	}
}
