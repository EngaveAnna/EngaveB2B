 ...
 */
class CEvent extends apm_Core_BaseObject {
	 ...
	 * @param
	 *        	Date Start date of the Date Object
	 * @param
	 *        	Date End date of the Date Object
	 * @param
	 *        	integer Type of Recurrence
	 * @param
	 *        	integer Times of Recurrence
	 * @param
	 *        	integer Time of Recurrence
	...
	public function getRecurrentEventforPeriod($start_date, $end_date, $event_start_date, $event_end_date, $event_recurs, $event_times_recuring, $j) {
		...
		$eventStart = new apm_Utilities_Date ( $event_start_date );
		$eventEnd = new apm_Utilities_Date ( $event_end_date );
	...
	public static function getEventsForPeriod($start_date, $end_date, $filter = 'all', $user_id = null, $project_id = 0, $company_id = 0) {
		...
		foreach ( $queries as $query_set ) {
			$$query_set = new apm_Database_Query ();
	...
	public function getEventsInWindow($start_date, $end_date, $start_time, $end_time, $users = null) {
		...
		$q = $this->_getQuery ();
		$q->addTable ( 'events', 'e' );
		$q->addQuery ( "e.event_id, event_name, event_sequence, event_owner, event_description, " . "ue.user_id, event_cwd, event_start_date, event_end_date, event_updated, event_recurs" );
	...
	protected function hook_preStore() {
		...
		if (! $this->event_recurs) {
			...
			$start_date = new apm_Utilities_Date ( $this->event_start_date );
			$end_date = new apm_Utilities_Date ( $this->event_end_date );
	...
	public function getCalendarEvents($userId, $days = 30) {
		...
		$events = $this->getEventsInWindow ( $nowFormatted, $NdaysFormatted, '0000', '2359', array (
				$userId 
		) );
		
		$start = new apm_Utilities_Date ( $nowFormatted );
		$end = new apm_Utilities_Date ( $NdaysFormatted );
		
		foreach ( $events as $event ) {
			for($j = 0; $j <= $event ['event_recurs']; $j ++) {
				$myDates = $this->getRecurrentEventforPeriod ( $start, $end, $event ['event_start_date'], $event ['event_end_date'], $event ['event_recurs'], $event ['event_times_recuring'], $j );
				/*
				 ...
				 */
				$url = apm_BASE_URL . '/index.php?m=events&a=view&event_id=' . $event ['event_id'];
				$eventList [] = array (
						'id' => $event ['event_id'],
						'name' => $event ['event_name'],
						'sequence' => $event ['event_sequence'],
						'description' => $event ['event_description'],
						'startDate' => $myDates [0]->format ( FMT_DATETIME_MYSQL ),
						'endDate' => $myDates [1]->format ( FMT_DATETIME_MYSQL ),
						'updatedDate' => $event ['event_updated'],
						'url' => $url 
				);
			}
		}
	...
	public function calculateRows($this_date, $end_date, $increment) {
		...
		if (1 == $ends_later) {
			...
			$count = (($end_date->getHour () * 60 + $end_date->getMinute ()) - ($this_date->getHour () * 60 + $this_date->getMinute ())) / $increment;
		}
