 ...
 */
class CContact extends apm_Core_BaseObject {
	...
	public function isValid() {
		...
		if (mb_strlen ( $this->contact_email ) && ! apm_check_email ( $this->contact_email )) {
	...
	public function canCreate() {
		...
		if ('true' == apmgetConfig ( 'activate_external_user_creation' )) {
			return true;
		}
	...
	public function getContactById($id, $load=false)
	{
		$q = $this->_query;
		$q->addTable ( 'contacts' );
		$q->addQuery ( '*' );
		$q->addWhere ( 'contact_id = '.$id );
	...
	public static function getProposedProjects($contactId) {
		$q = new apm_Database_Query ();
		$q->addQuery ( 'p.project_id, p.project_name' );
		$q->addTable ( 'project_contacts', 'pc' );
		$q->addJoin ( 'projects', 'p', 'p.project_id = pc.project_id', 'inner' );
		$q->addWhere ( "contact_id =  $contactId" );
		$q->addWhere ( "p.project_status = 1" );
	...
	public function hook_search() {
		...
		$search ['table_joins'] = array (
				array (
						'table' => 'contacts_methods',
						'alias' => 'cm',
						'join' => 'c.contact_id = cm.contact_id' 
				) 
		);
