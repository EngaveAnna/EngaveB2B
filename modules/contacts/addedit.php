...
$object_id = ( int ) apmgetParam ( $_GET, 'contact_id', 0 );
...
function addContactMethod(field, value) {
    ...
    $('#contact_methods_' + index + '_').append('<label><select id="method_select_' + index + '" name="contact_methods[field][' + index + ']" size="1" class="form-control" /></label>');
    /* Add text field for the contact method value to the table row */
    $('#contact_methods_' + index + '_').append('<input type="text" name="contact_methods[value][' + index + ']" size="25" maxlength="255" class="form-control" value="' + (value ? value : "") + '" /><?php echo apmtoolTip('Contact Method', 'Remove') ?><a id="remove_contact_method" href="javascript:removeContactMethod(\'' + index + '\')"><img src="<?php echo apmfindImage('icons/remove.png'); ?>" alt="" /></a><?php echo apmendTip() ?>');
