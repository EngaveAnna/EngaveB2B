...
$table_name = apmgetParam ( $_GET, 'table_name', 'companies' );
$company_id = ( int ) apmgetParam ( $_GET, 'company_id', 0 );
$dept_id = ( int ) apmgetParam ( $_GET, 'dept_id', 0 );
$select_list = array ();
...
$myId = ( int ) apmgetParam ( $_POST, $id_field, 0 );
...
if ($myId) {
	...
	if ($table_name == 'companies') {
		...
		if ($update_address) {
			$update_fields = array (
					'company_address1' => 'contact_address1',
...
<form name="frmSelector"
	action="./index.php?m=contacts&a=select_contact_company&dialog=1&table_name=<?php echo $table_name . '&' . $additional_get_information; ?>"
	...
	echo arraySelect ( $select_list, $id_field, 'class="form-control"', $dataId );
	?>
				</td>
		</tr>
		<tr>
			<td><input type="button" class="btn btn-default"
				value="<?php echo $AppUI->_('cancel'); ?>" onclick="window.close()" />
			</td>
			<td align="right"><input type="submit" class="btn btn-default"
				value="<?php echo $AppUI->_('Select', UI_CASE_LOWER); ?>" /></td>
		</tr>
	</table>
</form>
<?php
