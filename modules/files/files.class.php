 ...
 */
class CFile extends apm_Core_BaseObject {
	public $file_id = null;
	...
	public function getFileSystem() {
		if (is_null ( $this->_file_system )) {
			$this->setFileSystem ( new apm_FileSystem_Local () );
		}
	...
	public function hook_cron() {
		...
		foreach ( $unindexedFiles as $file_id => $notUsed ) {
			...
			$indexer = new apm_FileSystem_Indexer ( $this->_getQuery () );
			$indexer->index ( $this );
		}
		$this->indexer = false;
	}
	...
	public function indexStrings() {
		trigger_error ( "CFile->indexStrings() has been deprecated in v3.2 and will be removed by v5.0. Please use apm_FileSystem_Indexer->index() instead.", E_USER_NOTICE );
		$indexer = new apm_FileSystem_Indexer ( $this->_getQuery () );
		$indexer->index ( $this );
	}
