...
if (! isset ( $company_id )) {
	$company_id = ( int ) apmgetParam ( $_REQUEST, 'company_id', 0 );
}
...
if (! isset ( $task_id )) {
	$task_id = ( int ) apmgetParam ( $_REQUEST, 'task_id', 0 );
}
...
<table id="tblFolders" class="tbl list">
	<tr>
		<td nowrap="nowrap" colspan="20">
			<ul>
                <?php if ($folder_id) { ?>
                <li><a
					href="./index.php?m=<?php echo $m; ?>&amp;&a=<?php echo $a; ?>&amp;tab=<?php echo $tab; ?>&folder=0"><?php echo apmshowImage('home.png', '22', '22', 'back to root folder', '', 'files'); ?></a></li>
                <?php if (array_key_exists($cfObj->file_folder_parent, $allowed_folders_ary)): ?>
                    <li><a
					href="./index.php?m=<?php echo $m; ?>&amp;&a=<?php echo $a; ?>&amp;tab=<?php echo $tab; ?>&folder=<?php echo $cfObj->file_folder_parent; ?>"><?php echo apmshowImage('back.png', '22', '22', 'folder icon', 'back to parent folder', 'files'); ?></a></li>
        		<?php endif; ?>
                <li><a
					href="./index.php?m=<?php echo $m; ?>&amp;tab=<?php echo $tab; ?>&a=addedit_folder&folder=<?php echo $cfObj->file_folder_id; ?>"
					title="edit the <?php echo $cfObj->file_folder_name; ?> folder"><?php echo apmshowImage('filesaveas.png', '22', '22', 'folder icon', 'edit folder', 'files'); ?></a></li>
                <?php } ?>
                <li class="info-text"><?php echo apmshowImage('folder5_small.png', '22', '22', '', '', 'files'); ?> <strong><?php echo (isset($cfObj) && $cfObj->file_folder_name) ? $cfObj->file_folder_name : "Root"; ?></strong></li>
                <?php if (isset($cfObj) && $cfObj->file_folder_description != '') { ?>
                    <li class="info-text"><?php echo apm_textarea($cfObj->file_folder_description); ?></li>
                <?php } ?>
            </ul>
		</td>
	</tr>
    ...
    <?php
				...
				asort ( $sprojects );
				$sprojects = array (
						'O' => '(' . $AppUI->_ ( 'Move to Project', UI_OUTPUT_RAW ) . ')' 
				) + array (
						'0' => '(' . $AppUI->_ ( 'All Projects', UI_OUTPUT_RAW ) . ')' 
				) + $sprojects;
				?>
	<tr>
		<td colspan="50" align="right">
			<form name="frm_bulk" id="frm_bulk" method="post"
				action="?m=files&a=do_files_bulk_aed" accept-charset="utf-8">
				<input type="hidden" name="redirect"
					value="<?php echo $current_uri; ?>" />
                <?php echo arraySelect($sprojects, 'bulk_file_project', 'style="width:180px" class="form-control"', 'O'); ?>
                <?php echo arraySelectTree($folders, 'bulk_file_folder', 'style="width:180px;" class="form-control"', 'O'); ?>
                <input type="button" class="button"
					value="<?php echo $AppUI->_('Go'); ?>"
					onclick="if (confirm('Are you sure you wish to apply the options on the selected files?')) document.frm_bulk.submit();" />
			</form>
		</td>
	</tr>
</table>