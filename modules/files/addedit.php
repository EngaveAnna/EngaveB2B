...
$folder = ( int ) apmgetParam ( $_GET, 'folder', 0 );
$object_id = ( int ) apmgetParam ( $_GET, 'file_id', 0 );
$ci = apmgetParam ( $_GET, 'ci', 0 ) == 1 ? true : false;
$preserve = $apmconfig ['files_ci_preserve_attr'];
...
if (file_exists ( apm_BASE_DIR . '/modules/helpdesk/config.php' )) {
	include (apm_BASE_DIR . '/modules/helpdesk/config.php');
}
...
$file_task = ( int ) apmgetParam ( $_GET, 'file_task', $object->file_task );
$file_parent = ( int ) apmgetParam ( $_GET, 'file_parent', 0 );
$file_project = ( int ) apmgetParam ( $_GET, 'project_id', 0 );
