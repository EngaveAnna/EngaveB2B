...
$debug = false;
$callback = apmgetParam ( $_GET, 'callback', 0 );
$table = apmgetParam ( $_GET, 'table', 0 );
$user_id = apmgetParam ( $_GET, 'user_id', 0 );
$dialog = ( int ) apmgetParam ( $_GET, 'dialog', 0 );
...
switch ($table) {
	...
	case 'forums' :
		...
		break;
	case 'projects' :
		$project_company = apmgetParam ( $_GET, 'project_company', 0 );
...
if (! $ok) {
	...
	$list = arrayMerge ( array (
			0 => $AppUI->_ ( '[none]' ) 
	), $query_result ? $query_result : $resultList );
//	echo db_error ();
	?>
...
<form name="frmSelector" accept-charset="utf-8">
    ...
    <?php echo $AppUI->_('Select') . ' ' . $AppUI->_($title) . ':' ?>
    </div>
...
<div>				
<input type="button" class="btn btn-default" value="<?php echo $AppUI->_('cancel'); ?>" onclick="window.close()" />
</div>		
