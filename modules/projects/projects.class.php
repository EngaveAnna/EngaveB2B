 ...
 */
class CProject extends apm_Core_BaseObject {
	...
	public $project_target_budget = 0.00;
	...
	public function isValid() {
		...
		if ('' == $this->project_color_identifier) {
			// APMoff $this->_error['project_color_identifier'] = $baseErrorMsg . 'project color identifier is not set';
			$this->project_color_identifier = "FFFFFF";
		}
	...
	protected function hook_postDelete() {
		...
		$q->clear ();
		$task = new apm_Actions_BulkTasks ();
	...
	public function importTasks($from_project_id, CTask $newTask = null) {
		$newTask = new apm_Actions_BulkTasks ();
	...
	/**
	 * * Overload of the apmObject::getAllowedRecords
	 * * to ensure that the allowed projects are owned by allowed companies.
	 * *
	 * * @see apmObject::getAllowedRecords
	 */
	...
	/**
	 * Overload of the apmObject::getDeniedRecords
	...
	protected function hook_preStore() {
		...
		$this->project_url = str_replace ( array (
				'"',
				'"',
				'<',
				'>' 
		), '', $this->project_url );
		$this->project_demo_url = str_replace ( array (
				'"',
				'"',
				'<',
				'>' 
		...
		if ($this->project_start_date) {
			$date = new apm_Utilities_Date ( $this->project_start_date );
			$this->project_start_date = $date->format ( FMT_DATETIME_MYSQL );
		}
		
		if ($this->project_end_date) {
			$date = new apm_Utilities_Date ( $this->project_end_date );
	...
	public function getProposedProjects($userId) {
		$q = $this->_getQuery ();
		$q->addTable ( 'projects', 'pr' );
		$q->addQuery ( 'pr.project_id, project_color_identifier, project_name, project_start_date, project_end_date, project_company, project_parent' );
	...
	/**
	 * Note that this returns the *count* of projects.
	 * If this is zero, it is
	...
	public static function updatePercentComplete($project_id) {
		...
		$working_hours = (apmgetConfig ( 'daily_working_hours' ) ? apmgetConfig ( 'daily_working_hours' ) : 8);
	...
	/**
	 * This is an unnecessary function as of v2.x.
	 * Instead of calculating this on
