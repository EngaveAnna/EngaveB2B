...
$end_date = intval ( $project->project_end_date ) ? new apm_Utilities_Date ( $project->project_end_date ) : null;
...
$titleBlock = new apm_Theme_TitleBlock ( 'View Project', 'icon.png', $m );
$titleBlock->addCrumb ( '?m=' . $m, $m . ' list' );

if ($canEdit) {
	$titleBlock->addCrumb ( '?m=projects&a=addedit&project_id=' . $project_id, 'edit this project' );
	$titleBlock->addCrumb ( '?m=links&a=addedit&project_id=' . $project_id, 'new link', '', true );
	$titleBlock->addCrumb ( '?m=events&a=addedit&project_id=' . $project_id, 'new event', '', true );
	$titleBlock->addCrumb ( '?m=files&a=addedit&project_id=' . $project_id, 'new file', '', true );
	if ($canDelete) {
		$titleBlock->addCrumbDelete ( 'delete project', $canDelete );
	}
}
if (canAdd ( 'tasks' )) {
	$titleBlock->addCrumb ( '?m=tasks&a=addedit&task_project=' . $project_id, 'new task', '', true );
}
$titleBlock->show ();

$view = new apm_Controllers_View ( $AppUI, $project, 'Project' );
...
if ($canViewTask && $AppUI->isActiveModule ( 'tasks' )) {
	$tabBox->add ( apm_BASE_DIR . '/modules/tasks/vw_tasks', 'Tasks' );
	unset ( $status [0] );
	$tabBox->add ( apm_BASE_DIR . '/modules/tasks/vw_tasks', 'Tasks (Inactive)' );
	unset ( $status [- 1] );
	
	foreach ( $status as $id => $statusName ) {
		$tabBox->add ( apm_BASE_DIR . '/modules/tasks/tasks', $AppUI->_ ( 'Tasks' ) . ' (' . $AppUI->_ ( $statusName ) . ')' );
	}
	
	$tabBox->add ( apm_BASE_DIR . '/modules/tasks/viewgantt', 'Gantt Chart' );
	if ($canViewTaskLog) {
		$tabBox->add ( apm_BASE_DIR . '/modules/projects/vw_logs', 'Task Logs' );
	}
}
