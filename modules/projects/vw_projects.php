...
$projectStatuses = apmgetSysVal ( 'ProjectStatus' );
...
((! $row ['project_active'] && $project_status_filter == - 3))) // flat archived projects
{
						...
						$none = false;
						$end_date = intval ( $row ['project_end_date'] ) ? new apm_Utilities_Date ( $row ['project_end_date'] ) : null;
						$actual_end_date = intval ( $row ['project_actual_end_date'] ) ? new apm_Utilities_Date ( $row ['project_actual_end_date'] ) : null;
						...
						echo '<tr><td height="1" colspan="12" style="border-bottom: 1px solid;padding:0px;" bgcolor="#FFFFFF"><img src="' . apmfindImage ( 'shim.gif' ) . '" /></td></tr>';
					}
				}
				if ($none) {
