...
if (is_array ( $st_projects_arr )) {
	foreach ( $st_projects_arr as $project ) {
		...
		if ($line ['project_id']) {
			...
			$s = '<tr>';
			$s .= '<td><a href="./index.php?m=projects&a=addedit&project_id=' . $s_project->project_id . '"><img src="' . apmfindImage ( 'icons/' . ($project_id == $s_project->project_id ? 'pin' : 'pencil') . '.gif' ) . '" /></a></td>';
			foreach ( $fieldList as $field ) {
				if ('project_name' == $field) {
					$s .= '<td class="_name">';
					if ($level) {
						$s .= str_repeat ( '&nbsp;&nbsp;&nbsp;&nbsp;', ($level - 1) ) . apmshowImage ( 'corner-dots.gif', 16, 12 ) . '&nbsp;' . '<a href="./index.php?m=projects&a=view&project_id=' . $s_project->project_id . '">' . $s_project->project_name . '</a>';
					} else {
