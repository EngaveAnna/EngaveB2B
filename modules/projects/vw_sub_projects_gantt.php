...
global $AppUI, $company_id, $dept_ids, $department, $locale_char_set, $proFilter, $projectStatus, $showInactive, $showLabels, $showAllGantt, $user_id, $apmconfig, $project_id, $project_original_id;
...
$projectStatus = arrayMerge ( array (
		'-2' => $AppUI->_ ( 'All w/o in progress' ) 
), $projectStatus );
$user_id = apmgetParam ( $_REQUEST, 'user_id', $AppUI->user_id );

if ($AppUI->user_id == $user_id) {
	$projectStatus = arrayMerge ( array (
			'-3' => $AppUI->_ ( 'My projects' ) 
	), $projectStatus );
} else {
	$projectStatus = arrayMerge ( array (
			'-3' => $AppUI->_ ( 'User\'s projects' ) 
	), $projectStatus );
}

$proFilter = apmgetParam ( $_REQUEST, 'proFilter', '0' );
$company_id = apmgetParam ( $_REQUEST, 'company_id', 0 );
$department = apmgetParam ( $_REQUEST, 'department', 0 );
$showLabels = apmgetParam ( $_REQUEST, 'showLabels', 1 );
$showInactive = apmgetParam ( $_REQUEST, 'showInactive', 1 );
$original_project_id = apmgetParam ( $_REQUEST, 'original_project_id', 1 );
...
$width = apmgetParam ( $_GET, 'width', 600 );
$start_date = apmgetParam ( $_GET, 'start_date', 0 );
$end_date = apmgetParam ( $_GET, 'end_date', 0 );

$showAllGantt = apmgetParam ( $_REQUEST, 'showAllGantt', '1' );
...
if (! is_array ( $projects ) || sizeof ( $projects ) == 0) {
	$d = new apm_Utilities_Date ();
	$columnValues = array (
			'project_name' => $AppUI->_ ( 'No projects found' ),
			'start_date' => $d->getDate (),
			'end_date' => $d->getDate (),
			'actual_end' => '' 
	);
	...
	if (is_array ( $projects )) {
		...
		if ($showAllGantt) {
			...
			foreach ( $proTasks as $rec ) {
				...
				$tsd = new apm_Utilities_Date ( $rec ['task_start_date'] );
				...
				$ted = new apm_Utilities_Date ( $rec ['task_end_date'] );
		...
		foreach ( $projects as $p ) {
			...
			$end_date = new apm_Utilities_Date ( $end_date );
			...
			$actual_enddate = new apm_Utilities_Date ( $actual_end );
			...
			$columnValues = array (
					...
					'start_date' => $start,
					'end_date' => $end,
					'actual_end' => $actual_enddate->getDate () 
			);
			...
			if ($showAllGantt) {
				...
				for($i = 0, $i_cmp = count ( $gantt_arr [$p ['project_id']] ); $i < $i_cmp; $i ++) {
					...
					$tStartObj = new apm_Utilities_Date ( $tStart );
					$tEndObj = new apm_Utilities_Date ( $tEnd );
					...
					if ($t ['task_milestone'] != 1) {
						...
						$columnValues = array (
								'project_name' => $name,
								'start_date' => $tStartObj->getDate (),
								'end_date' => $tEndObj->getDate (),
								'actual_end' => '' 
						);
						...
						$milestoneDate = new apm_Utilities_Date ( $milestone );
						$gantt->addMilestone ( array (
								$name,
								'',
								$milestoneDate->format ( $df ) 
						), $t ['task_start_date'] );
					}
