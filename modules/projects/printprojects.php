...
$project_types = apmgetSysVal ( 'ProjectType' );
$project_statuses = apmgetSysVal ( 'ProjectStatus' );
?>
<style type="text/css">
...
<table class="prjprint">
	<tr>
		<th><?php echo $AppUI->_('Project List'); ?></th>
	</tr>
	<tr>
		<th>
            <?php
												...
												$fixed_project_status_file = array (
	...
	<tr>
		<th>
            <?php
												$txt = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $AppUI->_ ( 'Search' ) . ':&nbsp;' . '<input type="text" disabled class="form-control" SIZE="20" name="projsearchtext" onChange="document.searchfilter.submit();" value=' . "'$search_text'" . 'title="' . $AppUI->_ ( 'Search in name and description fields' ) . '"/>&nbsp;';
												$txt .= $AppUI->_ ( 'Type' ) . ':&nbsp;' . arraySelect ( $project_types, 'project_type', 'size="1" disabled class="form-control"', $project_type, false );
												$user_list = array (
														0 => '(all)' 
												) + CProject::getOwners ();
												$txt .= $AppUI->_ ( 'Owner' ) . ':&nbsp;' . arraySelect ( $user_list, 'project_owner', 'size="1" disabled class="form-control"', $owner, false );
...
require (apm_BASE_DIR . '/modules/projects/vw_projects.php');