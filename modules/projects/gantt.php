...
global $AppUI, $dept_ids, $apmconfig;
...
$statusFilter = ( int ) apmgetParam ( $_REQUEST, 'proFilter', - 1 );
$company_id = apmgetParam ( $_REQUEST, 'company_id', 0 );
$department = apmgetParam ( $_REQUEST, 'department', 0 );
$showLabels = apmgetParam ( $_REQUEST, 'showLabels', 0 );
$showInactive = apmgetParam ( $_REQUEST, 'showInactive', 0 );
$sortTasksByName = apmgetParam ( $_REQUEST, 'sortTasksByName', 0 );
$addPwOiD = apmgetParam ( $_REQUEST, 'addPwOiD', 0 );
...
$showAllGantt = apmgetParam ( $_REQUEST, 'showAllGantt', '0' );
...
if (! $start_date || ! $end_date) {
	...
	for($i = 0, $i_cmp = $projectCount; $i < $i_cmp; $i ++) {
		...
		$d_start = new apm_Utilities_Date ( $start );
		$d_end = new apm_Utilities_Date ( $end );
...
if (! is_array ( $projects ) || count ( $projects )==0) {
	...
	foreach ( $projects as $p ) {
		...
		$start_date = ( int ) ($p ['project_start_date']) ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $p ['project_start_date'], '%Y-%m-%d %T' ) ) : new apm_Utilities_Date ();
		$start = $start_date->getDate ();
		
		$end_date = ( int ) ($p ['project_end_date']) ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $p ['project_end_date'], '%Y-%m-%d %T' ) ) : new apm_Utilities_Date ();
		$end = $end_date->getDate ();
		
		$actual_end = ( int ) ($p ['project_actual_end_date']) ? new apm_Utilities_Date ( $AppUI->formatTZAwareTime ( $p ['project_actual_end_date'], '%Y-%m-%d %T' ) ) : $end_date;
		...
		if ($showAllGantt) {
			...
			foreach ( $tasks as $t ) {
				...
				foreach ( $workers as $w ) {
					$columnValues = array (
							'user_name' => '    * ' . $w ['contact_display_name'],
							'start_date' => $t ['task_start_date'],
							'end_date' => $t ['task_end_date'],
							'actual_end' => '' 
					);
