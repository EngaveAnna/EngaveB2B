	...
	public function preview() {
		...
		$output .= '
          <tr>
            ...
            <td><input type="text" name="project_start_date" value="' .str_replace ( "T", " ",  $reader->proj->summary ['Start']) . '" /></td>
          </tr>
          <tr>
            <td align="right">' . $AppUI->_ ( 'End Date' ) . ':</td>
            <td><input type="text" name="project_end_date" value="' .str_replace ( "T", " ",  $reader->proj->summary ['Finish']) . '" /></td>
          </tr>
          <tr>
      ...
      <tr>
		...
		foreach ( $reader->proj->tasks->children () as $task ) {
			if ($task ['ID'] != 0 && trim ( $task ['Name'] ) != '') {
				...
				$output .= '<input type="hidden" name="tasks[' . $task ['ID'] . '][task_start_date]" value="'.str_replace ( "T", " ", $task ['Start'] ). '" />';
				$output .= '<input type="hidden" name="tasks[' . $task ['ID'] . '][task_end_date]" value="' .str_replace ( "T", " ",  $task ['Finish']). '" />';
