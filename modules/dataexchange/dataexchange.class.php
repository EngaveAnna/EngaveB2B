...
class CImporter {
	...
	protected function _updateTask($AppUI, $project_id, $task) {
		$q = new apm_Database_Query ();
		$q->addTable ( 'tasks' );
		$q->addUpdate ( 'task_name', apmgetParam ( $task, 'task_name', null));
		$q->addUpdate ( 'task_project', $project_id);
		$q->addUpdate ( 'task_description', apmgetParam ( $task, 'task_description',null));
		$q->addUpdate ( 'task_start_date', apmgetParam ($task, 'task_start_date',null));
		$q->addUpdate ( 'task_end_date', apmgetParam ($task, 'task_end_date',null));
		$q->addUpdate ( 'task_duration', $task ['task_duration']);
		$q->addUpdate ( 'task_milestone', ( int ) $task ['task_milestone']);
		$q->addUpdate ( 'task_owner', ( int ) $task ['task_owner']);
		$q->addUpdate ( 'task_dynamic', ( int ) $task ['task_dynamic']);
		$q->addUpdate ( 'task_priority', ( int ) $task ['task_priority']);
		$q->addUpdate ( 'task_percent_complete', $task ['task_percent_complete']);
		$q->addUpdate ( 'task_duration_type', 1 );
		$q->addWhere ( 'task_id = ' . $task ['task_id'] );
		$q->addTable ( 'tasks' );
		$q->exec ();
		$r = db_insert_id ();
		return (! is_array ( $r )) ? $r : $task ['task_id'];
	}
	protected function _processTask($AppUI, $project_id, $task) {
		$myTask = new CTask ();
		$myTask->task_name = apmgetCleanParam ( $task, 'task_name', null );
		$myTask->task_project = $project_id;
		$myTask->task_description = apmgetCleanParam ( $task, 'task_description', '' );
	...
	protected function _processProject(apm_Core_CAppUI $AppUI, $company_id, $projectInfo) {
		$projectName = apmgetParam ( $projectInfo, 'new_project', 'New Project' );
		$projectStartDate = apmgetParam ( $projectInfo, 'project_start_date', 'New Project' );
		$projectEndDate = apmgetParam ( $projectInfo, 'project_end_date', 'New Project' );
		$projectOwner = apmgetParam ( $projectInfo, 'project_owner', $AppUI->user_id );
		$projectStatus = apmgetParam ( $projectInfo, 'project_status', 0 );
