...
<table class="std list update">
	...
	if (count ( $updatesApplied ) > 0) {
		foreach ( $updatesApplied as $update ) {
			?>
				<tr>
		<td colspan="2">Database update - <?php echo $update; ?> - applied</td>
	</tr>
				<?php
		}
	} else {
		?>
			<tr>
		<td colspan="2">No database updates applied</td>
	</tr>
			<?php
	}
	...
	if (count ( $errorMessages ) > 0) {
		...
		?>
			<tr>
		<td colspan="2">Note: Errors noting 'Duplicate entry', 'Table already
			exists', or 'Unknown table' may not be problems.</td>
	</tr>
			<?php
	} else {
		?>
