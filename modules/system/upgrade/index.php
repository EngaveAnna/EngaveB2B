...
$titleBlock = new apm_Theme_TitleBlock ( $AppUI->_ ( 'Upgrade System' ), 'control-center.png', $m );
$titleBlock->addCrumb ( '?m=system', 'system admin' );
$titleBlock->show ();

$failedImg = apmshowImage ( 'log-error.gif', 16, 16, $AppUI->_ ( 'Failed' ) );
$okImg = apmshowImage ( 'log-notice.gif', 16, 16, $AppUI->_ ( 'OK' ) );
...
<table class="tbl update">
	...
	<tr>
		<td colspan="2">There is a check for (minimal) requirements appended
			below for troubleshooting. Since your system is being upgraded, all
			requirements should be met, but it never hurts checking anyway.</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td align="left">
			...
			if (version_compare ( PHP_VERSION, MIN_PHP_VERSION, '<' )) {
				echo '<b class="error">' . $failedImg . ' (' . PHP_VERSION . '): apmProject requires PHP ' . MIN_PHP_VERSION . '+. Please upgrade!</b>';
				$continue = false;
			} else {
	...
	<tr>
		...
		<td align="left">
		<?php
		if (! ini_get ( 'file_uploads' ) && is_writable ( apm_BASE_DIR . '/files' )) {
	...
	<tr>
		<td class="title" colspan="2"><br />Check requirements for optional
			components</td>
	</tr>
	<tr>
	...
	<tr>
		<td class="item" align="left" colspan="2" style="text-align: center;"><b
			class="error">Please confirm that your database has been backed up
				properly before running this process. You can never have too many
				backups.</b></td>
	</tr>
	<tr>
		<td colspan="2" align="center">
			<form
				action="<?php echo apm_BASE_URL; ?>/index.php?m=system&u=upgrade&a=perform"
				method="post" name="form" id="form" accept-charset="utf-8">
		  	<?php if ($continue) { ?>
		  		<input class="btn btn-default" type="submit" name="next"
					value="Perform Upgrade &raquo;" />
		  	<?php } else { ?>
		  		<input class="btn btn-default" type="button"
					value="Upgrade Stopped"
					onClick="alert('The above issues must be fixed before continuing.')" />
		  	<?php } ?>
			</form>
		</td>
	</tr>
