...
$titleBlock = new apm_Theme_TitleBlock ( 'Edit User Preferences', 'myevo-weather.png', $m );
$perms = &$AppUI->acl ();
if ($canEdit) {
...
?>
<form name="changeuser" action="./index.php?m=<?php echo $m; ?>"
	method="post" accept-charset="utf-8"
	classes="addedit system-preference">
	<input type="hidden" name="dosql" value="do_preference_aed" /> <input
		type="hidden" name="pref_user" value="<?php echo $user_id; ?>" /> <input
		type="hidden" name="del" value="0" />
    <?php echo $form->addNonce(); ?>
    
    
        <div class="panel panel-default">
		<div class="panel-heading">
 <?php echo $AppUI->_('User Preferences'); ?>: <?php echo $user_id ? $user : $AppUI->_('Default'); ?>
    </div>
...
?>
							...
							<tr>
								...
								<td>
<?php
$uis = $prefs ['UISTYLE'] ? $prefs ['UISTYLE'] : 'apmProject';
...
?>
							...
							<tr>
								...
								<td><input type="hidden" name="pref_name[TASKLOGEMAIL]"
									id="task_log_email_defaults"
...
echo '<p>' . $AppUI->_ ( 'Email Assignees' ) . '&nbsp;<input type="checkbox" name="tl_assign" id="tl_assign"';
if ($tl_assign) {
	echo ' checked="checked"';
}
echo ' /></p>';
echo '<p>' . $AppUI->_ ( 'Email Task Contacts' ) . '&nbsp;<input type="checkbox" name="tl_task" id="tl_task"';
if ($tl_task) {
	echo 'checked="checked"';
}
echo ' /></p>';
echo '<p>' . $AppUI->_ ( 'Email Project Contacts' ) . '&nbsp;<input type="checkbox" name="tl_proj" id="tl_proj"';
if ($tl_proj) {
	echo ' checked="checked"';
}
echo ' /></p>';
?>
							...
							<tr>
								...
								<td><input type="text" class="form-control"
									name="pref_name[TASKLOGSUBJ]"
									value="<?php echo $prefs['TASKLOGSUBJ']; ?>" /></td>
							</tr>
							<tr>
        ...
        <?php
								...
								echo arraySelect ( $record_method, 'pref_name[TASKLOGNOTE]', 'class="form-control" size="1"', $prefs ['TASKLOGNOTE'], false );
								?>
	</td>
							</tr>
							<tr>
								<td align="right"><?php echo $AppUI->_('Show Parent Tasks Expanded by Default'); ?>:</td>
								<td><input type="hidden" name="pref_name[TASKSEXPANDED]"
									id="tasks_expanded"
									value="<?php echo $prefs['TASKSEXPANDED']; ?>" />
        <?php
