...
$module = apmgetParam ( $_REQUEST, 'module', 'admin' );
$lang = apmgetParam ( $_REQUEST, 'lang', $AppUI->user_locale );

// read the installed modules
$loader = new apm_FileSystem_Loader ();
$modules = arrayMerge ( $loader->readDirs ( 'modules' ), array (
		'common' => 'common',
		'styles' => 'styles' 
) );
asort ( $modules );
...
$localeFile = apm_BASE_DIR . '/modules/' . $modules [$module] . '/locales/en/' . $modules [$module] . '.inc';
if (file_exists ( $localeFile )) {
	readfile ( $localeFile );
} else {
	$localeFile = apm_BASE_DIR . '/locales/en/' . $modules [$module] . '.inc';
	if (file_exists ( $localeFile )) {
		readfile ( $localeFile );
	}
}
...
if ($lang != 'en') {
	ob_start ();
	// read language files from module's locale directory preferably
	$localeFile = apm_BASE_DIR . '/modules/' . $modules [$module] . '/locales/' . $lang . '/' . $modules [$module] . '.inc';
	if (file_exists ( $localeFile )) {
		readfile ( $localeFile );
	} else {
		$localeFile = apm_BASE_DIR . '/locales/' . $lang . '/' . $modules [$module] . '.inc';
		if (file_exists ( $localeFile )) {
			readfile ( $localeFile );
		}
	}
...
$titleBlock = new apm_Theme_TitleBlock ( 'Translation Management', 'rdf2.png', $m );
$titleBlock->addCell ( ' ', '', '<form action="?m=system&a=translate" method="post" name="modlang" accept-charset="utf-8">', '' );
$titleBlock->addCell ( arraySelect ( $modules, 'module', 'size="1" class="form-control" onchange="document.modlang.submit();"', $module ) );
$titleBlock->addCell ( $AppUI->_ ( 'Module' ) );
$temp = $AppUI->setWarning ( false );
$titleBlock->addCell ( arraySelect ( $locales, 'lang', 'size="1" class="form-control" onchange="document.modlang.submit();"', $lang, true ), '', '', '</form>' );
...
<form action="?m=system&a=translate_save" method="post" name="editlang"
	...
	<input type="hidden" name="module"
		value="<?php echo $modules[$module]; ?>" /> <input type="hidden"
	...
	<table class="tbl list">
        ...
        <?php
								...
								if ($lang == 'en') {
									$s .= '<tr>';
									$s .= '<td><input type="text" name="trans[' . $index . '][abbrev]" value="" size="20" class="form-control" /></td>';
									$s .= '<td><input type="text" name="trans[' . $index . '][english]" value="" size="40" class="form-control" /></td>';
								...
								foreach ( $trans as $k => $langs ) {
									$s .= '<tr><td>';
									if ($k != $langs ['english']) {
										$k = apmformSafe ( $k, true );
										if ($lang == 'en') {
											$s .= '<input type="text" name="trans[' . $index . '][abbrev]" value="' . $k . '" size="20" class="form-control" />';
										} else {
											$s .= $k;
										}
									} else {
									...
									$s .= '</td><td>';
									$langs ['english'] = apmformSafe ( $langs ['english'], true );
									if ($lang == 'en') {
										if (mb_strlen ( $langs ['english'] ) < 40) {
											$s .= '<input type="text" name="trans[' . $index . '][english]" value="' . $langs ['english'] . '" size="40" class="form-control" />';
										} else {
										...
										$s .= $langs ['english'];
										$s .= '<input type="hidden" name="trans[' . $index . '][english]" value="' . ($k ? $k : $langs ['english']) . '" size="20" class="form-control" />';
									}
									$s .= '</td><td>';
									if ($lang != 'en') {
										$langs ['lang'] = apmformSafe ( $langs ['lang'], true );
										if (mb_strlen ( $langs ['lang'] ) < 40) {
											$s .= '<input type="text" name="trans[' . $index . '][lang]" value="' . $langs ['lang'] . '" size="40" class="form-control" />';
										} else {
        ...
        <tr>
			<td colspan="4" align="right"><input type="submit"
				value="<?php echo $AppUI->_('save'); ?>" class="btn btn-info" /></td>
		</tr>
	</table>
</form>
