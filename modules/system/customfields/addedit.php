...
$field_id = ( int ) apmgetParam ( $_GET, 'field_id', 0 );
$module_id = ( int ) apmgetParam ( $_GET, 'module', 0 );
...
$field = new apm_Core_CustomFieldManager ();
...
$titleBlock = new apm_Theme_TitleBlock ( $ttl, 'customfields.png', $m );
...
$custom_fields = new apm_Core_CustomFields ( $module->mod_name, 'addedit', null, 'edit' );
...
if ($field_id) {
	...
	if (is_object ( $cf )) {
		...
		if ($field_htmltype == 'select') {
			$select_options = new apm_Core_CustomOptionList ( $field_id );
			$select_options->load ();
...
<form method="post" action="?m=system&u=customfields" id="addEditForm"
	name="addEditForm" accept-charset="utf-8">
	...
	<table class="std addedit well">
		...
		<tr>
			...
			<td><input type="text" class="form-control" name="field_name"
				maxlength="100" value="<?php echo $field_name ?>"
				onblur="javascript:filterFieldName(this)" /></td>
		</tr>
		<tr>
			<td><label><?php echo $AppUI->_('Field Description') ?>:</label></td>
			<td><input type="text" class="form-control" name="field_description"
				size="40" maxlength="250" value="<?php echo $field_description ?>" />
			</td>
		</tr>
		<tr>
			<td><label><?php echo $AppUI->_('Field Display Type') ?>:</label></td>
			<td>
            <?php echo arraySelect($html_types, 'field_htmltype', 'id="htmltype" class="form-control" onChange="javascript:showAttribs()"', $field_htmltype); ?>
            </td>
		</tr>
		<tr>
			<td><label><?php echo $AppUI->_('Field Published') ?>:</label></td>
			<td>
            <?php echo arraySelect(apmgetSysVal('GlobalYesNo'), 'field_published', 'id="fieldpublished" class="form-control"', $field_published); ?>
            </td>
		</tr>
		<tr>
			<td><label><?php echo $AppUI->_('Field Display Order') ?>:</label></td>
			<td><input type="text" class="form-control" name="field_order"
				size="4" maxlength="3" value="<?php echo (int) $field_order ?>" /></td>
		</tr>
		<tr>
			<td><label><?php echo $AppUI->_('HTML Tag Options') ?>:</label></td>
			<td><input type="text" class="form-control" name="field_extratags"
				size="80" value="<?php echo $field_extratags ?>" /></td>
		</tr>
		<tr id="datarow">
            ...
            <?php
												if ($field_htmltype == 'select') {
													...
													foreach ( $select_items as $id => $item ) {
														...
														echo '<img src="' . apmfindImage ( 'remove.png' ) . '" /></a>&nbsp;';
														echo $item . '</li>';
													}
													echo '<li><input type="text" name="select_newitem" id="select_newitem" class="form-control" />&nbsp;
                            <input type="button" value="' . $AppUI->_ ( 'Add' ) . '" onclick="javascript:addItem()" class="form-control" /></li>';
													echo '</ul>';
												}
												?>
            </td>
		...
		<tr>
			<td><input type="button" class="btn btn-warning" name="cancel"
				value="<?php echo $AppUI->_('cancel'); ?>"
				onclick="javascript:if(confirm('<?php echo $AppUI->_('Are you sure you want to cancel?', UI_OUTPUT_JS); ?>')){location.href = './index.php?m=system&u=customfields';}" />
			</td>
			<td align="right"><input type="button" class="btn btn-info"
				value="<?php echo $AppUI->_('save'); ?>" onclick="submitIt()" /></td>
		</tr>
	</table>
</form>