...
$mod_id = ( int ) apmgetParam ( $_GET, 'mod_id' );
$view = apmgetParam ( $_GET, 'v' );
...
$titleBlock = new apm_Theme_TitleBlock ( 'Customize ' . $module->mod_name . ' Module :: ' . $view, 'modules/system/control-center.png', $m, $m . '.' . $a );
$titleBlock->addCrumb ( '?m=system', 'system admin' );
$titleBlock->addCrumb ( '?m=system&a=viewmods', 'modules list' );
$titleBlock->show ();

$fields = apm_System_Module::getSettings ( $module->mod_directory, $view );
...
<form name="frmConfig" id="frmConfig"
	action="?m=<?php echo $m; ?>&u=modules" method="post"
	accept-charset="utf-8">
	<input type="hidden" name="dosql" value="do_module_config_aed" /> <input
		type="hidden" name="mod_id" value="<?php echo $mod_id; ?>" /> <input
		type="hidden" name="module_config_name" value="<?php echo $view ?>" />

	<table id="tblConfig" class="">
		<tr>
		...
		foreach ( $fields as $field => $text ) {
			?><tr>
			<td><input type="checkbox" name="display[<?php echo $field; ?>]"
				checked="checked" /></td>
			<td>
					<?php echo arraySelect(range(0, $orderMax), "order[$field]", 'class="form-control" size="1"', $order); ?>
				</td>
			<td class="center"><input type="hidden" name="displayFields[]"
				value="<?php echo $field; ?>" />
					<?php echo $field; ?>
				</td>
			<td class="center"><input type="text" name="displayNames[]"
				value="<?php echo htmlspecialchars($text, ENT_QUOTES); ?>" size="25"
		...
		foreach ( $properties as $property => $value ) {
			...
			?><tr>
			<td><input type="checkbox" name="display[<?php echo $property; ?>]" />
			</td>
			<td><input type="hidden" name="order[<?php echo $property; ?>]"
				value="<?php echo $order ?>" /></td>
			<td class="center"><input type="hidden" name="displayFields[]"
				value="<?php echo $property; ?>" />
					<?php echo $property; ?>
				</td>
			<td class="center"><input type="text" name="displayNames[]"
				value="<?php echo $AppUI->_($value); ?>" size="25" maxlength="50"
		...
		?>
  		<tr>
			<td colspan="2"><input class="btn btn-danger" type="button"
				name="cancel" value="<?php echo $AppUI->_('cancel'); ?>" /></td>
			<td colspan="2" class="center"><input class="btn btn-info"
				type="submit" name="submit" value="<?php echo $AppUI->_('save'); ?>" />
			</td>
		</tr>
	</table>
</form>
