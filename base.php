...
define ( 'apm_PERFORMANCE_DEBUG', false );
define ( 'MIN_PHP_VERSION', '5.3.8' );

if (apm_PERFORMANCE_DEBUG) {
	global $apm_performance_time, $apm_performance_dbtime, $apm_performance_old_dbqueries, $apm_performance_dbqueries, $apm_performance_acltime, $apm_performance_aclchecks, $apm_performance_memory_marker, $apm_performance_setuptime;
	$apm_performance_time = array_sum ( explode ( ' ', microtime () ) );
	if (function_exists ( 'memory_get_usage' )) {
		$apm_performance_memory_marker = memory_get_usage ();
	}
	$apm_performance_acltime = 0;
	$apm_performance_aclchecks = 0;
	$apm_performance_dbtime = 0;
	$apm_performance_old_dbqueries = 0;
	$apm_performance_dbqueries = 0;
}
...
// Note: This resolves http://bugs.apmProject.net/view.php?id=1081 on IIS, but I'm not sure I like it..
...
define ( 'apm_BASE_DIR', $baseDir );
define ( 'apm_BASE_URL', $baseUrl );

// Set the ADODB directory
if (! defined ( 'ADODB_DIR' )) {
	define ( 'ADODB_DIR', apm_BASE_DIR . '/lib/adodb' );
}

/*
 * This is set to get past the Softmag security sentinel. It is only
