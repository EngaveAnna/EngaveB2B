...
// apm apmProject | gregorerhardt 20030909:
// apm commented out some lines for running without PEAR base class
// apm all these commented lines begin with //apm
// apm changes concern in particular PEAR error handling
// apm this is not the original Contact_Vcard_Build.php file.
// apm to get the original file: check pear.php.net or
// apm contact the author mentioned above
...
class Contact_Vcard_Build {
	 ...
	 *        	The vCard version to build; affects which
	...
	function Contact_Vcard_Build($version = '3.0') {
		// apm $this->PEAR();
		// apm $this->setErrorHandling(PEAR_ERROR_PRINT);
		$this->reset ( $version );
	}
	 ...
	 *        
	 * @param mixed $text
	 *        	The string or array or strings to escape.
	 *        	
	...
	function escape(&$text) {
		if (is_object ( $text )) {
			...
			// apm return $this->raiseError('The escape() method works only with string literals and arrays.');
	 ...
	 *        	The parameter name, such as TYPE, VALUE,
	 *        	or ENCODING.
	 *        	
	 ...
	 *        	
	 * @param string $comp
	 *        	The vCard component for which this is a
	 *        	paramter (ADR, TEL, etc). If null, will be the component that was
	 *        	last set or added-to.
	 *        	
	 * @param mixed $iter
	 *        	An integer vCard component iteration that this
	...
	function addParam($param_name, $param_value, $comp = null, $iter = null) {
		...
		if (! is_integer ( $iter ) || $iter < 0) {
			...
			// apm return $this->raiseError("$iter is not a valid iteration number for $comp; must be a positive integer.");
		} else {
			
			$result = $this->validateParam ( $param_name, $param_value, $comp, $iter );
			
			// apm if (PEAR::isError($result)) {
			// apm return $result;
			// apm } else {
	 ...
	 *        
	 * @param string $name
	 *        	The parameter name (e.g., TYPE or ENCODING).
	 *        	
	 * @param string $text
	 *        	The parameter value (e.g., HOME or BASE64).
	 *        	
	 * @param string $comp
	 *        	Optional, the component name (e.g., ADR or
	 *        	PHOTO). Only used for error messaging.
	 *        	
	 * @param string $iter
	 *        	Optional, the iteration of the component.
	...
	function validateParam($name, $text, $comp = null, $iter = null) {
		...
		if (preg_match ( '/[^a-zA-Z0-9\-]/i', $text )) {
			...
			// apm $result = $this->raiseError("vCard [$comp] [$iter] [$name]: The parameter value may contain only a-z, A-Z, 0-9, and dashes (-).");
			...
			switch ($name) {
				...
				case 'TYPE' :
					if (! in_array ( $text, $types )) {
						// apm $result = $this->raiseError("vCard 2.1 [$comp] [$iter]: $text is not a recognized TYPE.");
					} else {
						$result = true;
					}
					break;
				...
				case 'ENCODING' :
					if ($text != '7BIT' && $text != '8BIT' && $text != 'BASE64' && $text != 'QUOTED-PRINTABLE') {
						// apm $result = $this->raiseError("vCard 2.1 [$comp] [$iter]: $text is not a recognized ENCODING.");
					} else {
						$result = true;
					}
					break;
				...
				case 'VALUE' :
					if ($text != 'INLINE' && $text != 'CONTENT-ID' && $text != 'CID' && $text != 'URL' && $text != 'VCARD') {
						// apm $result = $this->raiseError("vCard 2.1 [$comp] [$iter]: $text is not a recognized VALUE.");
					} else {
						$result = true;
					}
					break;
				...
				default :
					// apm $result = $this->raiseError("vCard 2.1 [$comp] [$iter]: $name is an unknown or invalid parameter name.");
					break;
			}
			...
			switch ($name) {
				...
				case 'ENCODING' :
					if ($text != '8BIT' && $text != 'B') {
						// apm $result = $this->raiseError("vCard 3.0 [$comp] [$iter]: The only allowed ENCODING parameters are 8BIT and B.");
					} else {
						$result = true;
					}
					break;
				...
				case 'VALUE' :
					if ($text != 'BINARY' && $text != 'PHONE-NUMBER' && $text != 'TEXT' && $text != 'URI' && $text != 'UTC-OFFSET' && $text != 'VCARD') {
						// apm $result = $this->raiseError("vCard 3.0 [$comp] [$iter]: The only allowed VALUE parameters are BINARY, PHONE-NUMBER, TEXT, URI, UTC-OFFSET, and VCARD.");
					} else {
						$result = true;
					}
					break;
				...
				default :
					// apm $result = $this->raiseError("vCard 3.0 [$comp] [$iter]: Unknown or invalid parameter name ($name).");
					break;
			}
		} else {
			
			// apm $result = $this->raiseError("[$comp] [$iter] Unknown vCard version number or other error.");
		}
	 ...
	 *        
	 * @param string $comp
	 *        	The component to get parameters for (ADR, TEL,
	 *        	etc).
	 *        	
	 * @param int $iter
	 *        	The vCard component iteration to get the param
	 ...
	 *        
	 * @param string $version
	 *        	The vCard version to reset to ('2.1' or
	 ...
	 *        
	 * @param string $comp
	 *        	The component to get metadata for (ADR, TEL,
	 *        	etc).
	 *        	
	 * @param int $iter
	 *        	The vCard component iteration to get the metadata
	 ...
	 *        
	 * @param string $comp
	 *        	The component to set the value for ('N',
	 *        	'ADR', etc).
	 *        	
	 * @param int $iter
	 *        	The component-iteration to set the value for.
	 *        	
	 * @param int $part
	 *        	The part number of the component-iteration to set
	 *        	the value for.
	 *        	
	 * @param mixed $text
	 *        	A string or array; the set of repeated values
	 ...
	 *        	
	 * @param int $part
	 *        	The part number of the component-iteration to get
	 *        	the value for.
	 *        	
	 * @param mixed $rept
	 *        	The repetition number within the part to get;
	...
	function getValue($comp, $iter = 0, $part = 0, $rept = null) {
		if ($rept === null && is_array ( $this->value [$comp] [$iter] [$part] )) {
	 ...
	 *        	
	 * @param mixed $prefix
	 *        	Single (string) or multiple (array) honorific
	 ...
	 */
	function getName() {
		return $this->getMeta ( 'N', 0 ) . $this->getValue ( 'N', 0, VCARD_N_FAMILY ) . ';' . $this->getValue ( 'N', 0, VCARD_N_GIVEN ) . ';' . $this->getValue ( 'N', 0, VCARD_N_ADDL ) . ';' . $this->getValue ( 'N', 0, VCARD_N_PREFIX ) . ';' . $this->getValue ( 'N', 0, VCARD_N_SUFFIX );
	}
	 ...
	 *        
	 * @param string $text
	 *        	Override the automatic generation of FN from N
	...
	function setFormattedName($text = null) {
		...
		if ($text === null) {
			...
			if (is_array ( $this->value ['N'] )) {
				...
				// apm return $this->raiseError('FN not specified and N not set; cannot set FN.');
			}
		}
	 ...
	 *        
	 * @param string $text
	 *        	The text value of the verson text ('3.0' or '2.1').
	 *        	
	 * @return mixed Void on success, or a PEAR_Error object on failure.
	 *        
	 */
	function setVersion($text = '3.0') {
		...
		if ($text != '3.0' && $text != '2.1') {
			// apm return $this->raiseError('Version must be 3.0 or 2.1 to be valid.');
		} else {
	 ...
	 *        
	 * @param string $text
	 *        	The text value of the data-source text.
	 *        	
	 * @return void
	 *
	 */
	 ...
	 *        
	 * @param string $text
	 *        	The text value of the displayed data-source
	...
	function setSourceName($text = null) {
		...
		if ($text === null) {
			if (is_array ( $this->value ['SOURCE'] )) {
				...
				// apm return $this->raiseError('NAME not specified and SOURCE not set; cannot set NAME.');
			}
		}
	 ...
	 *        
	 * @param string $text
	 *        	The value to set for this component.
	 *        	
	 * @return void
	 *
	 */
	 ...
	 *        
	 * @param string $lat
	 *        	The value to set for the longitude part
	 *        	(decimal, + or -).
	 *        	
	 * @param string $lon
	 *        	The value to set for the latitude part
	 *        	(decimal, + or -).
	 *        	
	 * @return void
	 *
	 ...
	 */
	function getGeo() {
		return $this->getMeta ( 'GEO', 0 ) . $this->getValue ( 'GEO', 0, VCARD_GEO_LAT, 0 ) . ';' . $this->getValue ( 'GEO', 0, VCARD_GEO_LON, 0 );
	}
	 ...
	 *        
	 * @param mixed $pob
	 *        	String (one repetition) or array (multiple
	 ...
	 *
	 */
	function addAddress($pob, $extend, $street, $locality, $region, $postcode, $country) {
		$this->autoparam = 'ADR';
	 ...
	 *        
	 * @param int $iter
	 *        	The component iteration-number to get the value
	 *        	for.
	 *        	
	...
	function getAddress($iter) {
		if (! is_integer ( $iter ) || $iter < 0) {
			...
			// apm return $this->raiseError('ADR iteration number not valid.');
		} else {
			
			return $this->getMeta ( 'ADR', $iter ) . $this->getValue ( 'ADR', $iter, VCARD_ADR_POB ) . ';' . $this->getValue ( 'ADR', $iter, VCARD_ADR_EXTEND ) . ';' . $this->getValue ( 'ADR', $iter, VCARD_ADR_STREET ) . ';' . $this->getValue ( 'ADR', $iter, VCARD_ADR_LOCALITY ) . ';' . $this->getValue ( 'ADR', $iter, VCARD_ADR_REGION ) . ';' . $this->getValue ( 'ADR', $iter, VCARD_ADR_POSTCODE ) . ';' . $this->getValue ( 'ADR', $iter, VCARD_ADR_COUNTRY );
		}
	}
	...
	function getLabel($iter) {
		if (! is_integer ( $iter ) || $iter < 0) {
			// apm return $this->raiseError('LABEL iteration number not valid.');
		} else {
			return $this->getMeta ( 'LABEL', $iter ) . $this->getValue ( 'LABEL', $iter, 0 );
		}
	}
	...
	function getTelephone($iter) {
		if (! is_integer ( $iter ) || $iter < 0) {
			// apm return $this->raiseError('TEL iteration number not valid.');
		} else {
			return $this->getMeta ( 'TEL', $iter ) . $this->getValue ( 'TEL', $iter, 0 );
		}
	}
	...
	function getEmail($iter) {
		if (! is_integer ( $iter ) || $iter < 0) {
			// apm return $this->raiseError('EMAIL iteration number not valid.');
		} else {
			return $this->getMeta ( 'EMAIL', $iter ) . $this->getValue ( 'EMAIL', $iter, 0 );
		}
	}
	...
	function getCategories() {
		return $this->getMeta ( 'CATEGORIES', 0 ) . $this->getValue ( 'CATEGORIES', 0, 0 );
	}
	 ...
	 *        
	 * @param mixed $text
	 *        	String (one ORG part) or array (of ORG
	 ...
	 *        
	 * @param array $src
	 *        	One vCard entry as parsed using
	 *        	Contact_Vcard_Parse.
	 *        	
	 * @return void
	 *
	...
	function fetch() {
		...
		if (! is_array ( $this->value ['VERSION'] )) {
			// apm return $this->raiseError('VERSION not set (required).');
		}
		
		// FN component is required
		if (! is_array ( $this->value ['FN'] )) {
			// apm return $this->raiseError('FN component not set (required).');
		}
		
		// N component is required
		if (! is_array ( $this->value ['N'] )) {
			// apm return $this->raiseError('N component not set (required).');
		}
		...
		if (is_array ( $this->value ['NAME'] ) && $this->value ['VERSION'] [0] [0] [0] == '3.0') {
			$lines [] = $this->getSourceName ();
		}
		
		// data source (3.0 only)
		if (is_array ( $this->value ['SOURCE'] ) && $this->value ['VERSION'] [0] [0] [0] == '3.0') {
			$lines [] = $this->getSource ();
		}
		
		// nicknames (3.0 only)
		if (is_array ( $this->value ['NICKNAME'] ) && $this->value ['VERSION'] [0] [0] [0] == '3.0') {
		...
		// categories (3.0 only)
		if (is_array ( $this->value ['CATEGORIES'] ) && $this->value ['VERSION'] [0] [0] [0] == '3.0') {
		...
		// prodid (3.0 only)
		if (is_array ( $this->value ['PRODID'] ) && $this->value ['VERSION'] [0] [0] [0] == '3.0') {
		...
		if (is_array ( $this->value ['SORT-STRING'] ) && $this->value ['VERSION'] [0] [0] [0] == '3.0') {
		...
		// class (3.0 only)
		if (is_array ( $this->value ['CLASS'] ) && $this->value ['VERSION'] [0] [0] [0] == '3.0') {
			$lines [] = $this->getClass ();
		}
