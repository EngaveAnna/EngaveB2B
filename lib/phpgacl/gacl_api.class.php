...
class gacl_api extends gacl {
	 ...
	 *        
	 * @param
	 *        	string ACO Section Value
	 * @param
	 *        	string ACO Value
	 * @param
	 *        	string ARO Section Value
	 * @param
	 *        	string ARO Value
	 * @param
	 *        	string Return Value of ACL
	 */
	function consolidated_edit_acl($aco_section_value, $aco_value, $aro_section_value, $aro_value, $return_value) {
		...
		if (is_array ( $acl_ids ) and $acl_ids_count > 0) {
			...
			foreach ( $acl_ids as $acl_id ) {
				...
				if (! $this->shift_acl ( $acl_id, array (
		...
		if (is_array ( $acl_ids ) and $acl_ids_count == 1) {
			...
			if (! $this->append_acl ( $acl_id, array (
					$aro_section_value => array (
							$aro_value 
					) 
			) )) {
			...
			if (! $this->add_acl ( array (
					$aco_section_value => array (
							$aco_value 
					) 
			), array (
					$aro_section_value => array (
			...
			), NULL, NULL, NULL, TRUE, TRUE, $return_value, NULL )) {
	 ...
	 *        	string ARO Value
	 * @param
	 *        	string ARO Group Name
	 * @param
	 *        	string AXO Section Value
	 * @param
	 *        	string AXO Value
	 * @param
	 *        	string AXO Group Name
	 * @param
	 *        	string Return Value
	 */
	...
	function shift_acl($acl_id, $aro_array = NULL, $aro_group_ids = NULL, $axo_array = NULL, $axo_group_ids = NULL, $aco_array = NULL) {
		...
		if ($update == 1) {
			...
			if ($this->count_all ( $acl_array ['aco'] ) == 0 or ($this->count_all ( $acl_array ['aro'] ) == 0 and ($this->count_all ( $acl_array ['axo'] ) == 0 or $acl_array ['axo'] == FALSE) and (count ( $acl_array ['aro_groups'] ) == 0 or $acl_array ['aro_groups'] == FALSE) and (count ( $acl_array ['axo_groups'] ) == 0 or $acl_array ['axo_groups'] == FALSE))) {
	 ...
	 *        	array Array of Group IDs
	 * @param
	 *        	array Array of ACL IDs to ignore from the result set.
	 *        	
	 */
	 ...
	 *        	string Note
	 * @param
	 *        	string ACL Section Value
	 * @param
	 *        	int ACL ID # Specific Request
	 *        	
	 */
	function add_acl($aco_array, $aro_array, $aro_group_ids = NULL, $axo_array = NULL, $axo_group_ids = NULL, $allow = 1, $enabled = 1, $return_value = NULL, $note = NULL, $section_value = NULL, $acl_id = FALSE) {
		...
		if (! empty ( $section_value ) and ! $this->get_object_section_section_id ( NULL, $section_value, 'ACL' )) {
		...
		if ($this->is_conflicting_acl ( $aco_array, $aro_array, $aro_group_ids, $axo_array, $axo_group_ids, array (
				$acl_id 
		) )) {
	 ...
	 *        
	 * @param
	 *        	string Group Type, either 'ARO' or 'AXO'
	 */
	 ...
	 *        
	 * @param
	 *        	array Output from gacl_api->sorted_groups($group_type)
	 * @param
	 *        	array Output type desired, either 'TEXT', 'HTML', or 'ARRAY'
	 * @param
	 *        	int Root of tree to produce
	 * @param
	 *        	int Current level of depth
	 * @param
	 *        	array Pass the current formatted groups object for appending via recursion.
	 */
	 ...
	 *        	int Group ID #
	 * @param
	 *        	int Group Type, either 'ARO' or 'AXO'
	 * @param
	 *        	string Either 'RECURSE' or 'NO_RECURSE', to recurse while fetching group children.
	 */
	 ...
	 *        	string Group Type, either 'ARO' or 'AXO'
	 * @param
	 *        	string Option, either 'RECURSE' or 'NO_RECURSE'
	 */
	 ...
	 * Edits a group
	 *
	 * @return s bool Returns TRUE if successful, FALSE otherwise
	 *        
	 * @param
	 *        	int Group ID #
	 * @param
	 ...
	 *        
	 * @param
	 *        	string Table name of group type
	 * @param
	 *        	int Group ID #
	 * @param
	 *        	int Left value of Group
	 ...
	 *        	int Group ID #
	 * @param
	 *        	bool If TRUE, child groups of this group will be reparented to the current group's parent.
	 * @param
	 *        	string Group Type, either 'ARO' or 'AXO'
	 */
	 ...
	 *        
	 * @param
	 *        	string Filter to this section value
	 * @param
	 *        	int Returns hidden objects if 1, leaves them out otherwise.
	 * @param
	 *        	string Object Type, either 'ACO', 'ARO', 'AXO', or 'ACL'
	 */
	 ...
	 *        
	 * @param
	 *        	int Returns hidden objects if 1, does not if 0.
	 * @param
	 *        	string Object Type, either 'ARO' or 'AXO' (groupable types)
	 */
	 ...
	 *        
	 * @param
	 *        	string Filter for section value
	 * @param
	 *        	int Returns hidden objects if 1, does not if 0
	 * @param
	 *        	string Object Type, either 'ACO', 'ARO', 'AXO'
	 */
	 ...
	 *        	int Object ID #
	 * @param
	 *        	string Object Type, either 'ACO', 'ARO', or 'AXO'
	 */
	 ...
	 *        	int Object ID #
	 * @param
	 *        	string Object Type, either 'ARO' or 'AXO'
	 * @param
	 *        	string Option, either 'RECURSE', or 'NO_RECURSE'
	 */
	 ...
	 *        	int Display Order
	 * @param
	 *        	int Hidden Flag, either 1 to hide, or 0 to show.
	 * @param
	 *        	string Object Type, either 'ACO', 'ARO', or 'AXO'
	 */
	 ...
	 *        	string Object Type, either 'ACO', 'ARO', or 'AXO'
	 * @param
	 *        	bool Erases all referencing objects if TRUE, leaves them alone otherwise.
	 */
	 ...
	 *        	int Display Order
	 * @param
	 *        	int Hidden flag, hides section if 1, shows section if 0
	 * @param
	 *        	string Object Type, either 'ACO', 'ARO', 'AXO', or 'ACL'
	 */
	 ...
	 *        
	 * @param
	 *        	int Object Section ID #
	 * @param
	 *        	string Object Section Name
	 * @param
	 *        	string Object Section Value
	 * @param
	 *        	int Display Order
	 * @param
	 *        	int Hidden Flag, hide object section if 1, show if 0
	 * @param
	 *        	string Object Type, either 'ACO', 'ARO', 'AXO', or 'ACL'
	 */
	 ...
	 *        
	 * @param
	 *        	int Object Section ID # to delete
	 * @param
	 *        	string Object Type, either 'ACO', 'ARO', 'AXO', or 'ACL'
	 * @param
	 *        	bool Erases all section objects assigned to the section
	 */
