...
final class xajax {
	...
	private function verifySession() {
		...
		if ($sessionID === '') {
			$this->objResponseManager->debug ( 'Must enable sessions to use challenge/response.' );
			return false;
		}
		return true;
	}
	...
	public function processRequest() {
		...
		if ($this->canProcessRequest ()) {
			...
			if (isset ( $this->aProcessingEvents [XAJAX_PROCESSING_EVENT_BEFORE] )) {
				...
				$this->aProcessingEvents [XAJAX_PROCESSING_EVENT_BEFORE]->call ( array (
						&$bEndRequest 
				) );
			...
			if (true === $mResult) {
				...
				if (isset ( $this->aProcessingEvents [XAJAX_PROCESSING_EVENT_AFTER] )) {
					...
					$this->aProcessingEvents [XAJAX_PROCESSING_EVENT_AFTER]->call ( array (
							$bEndRequest 
					) );
			...
			if ($this->bErrorHandler) {
				...
				if (! empty ( $sErrorMessage )) {
					if (0 < strlen ( $this->sLogFile )) {
						...
						if (NULL != $fH) {
							fwrite ( $fH, $this->objLanguageManager->getText ( 'LOGHDR:01' ) . strftime ( "%b %e %Y %I:%M:%S %p" ) . $this->objLanguageManager->getText ( 'LOGHDR:02' ) . $sErrorMessage . $this->objLanguageManager->getText ( 'LOGHDR:03' ) );
							fclose ( $fH );
						} else {
	...
	public function autoCompressJavascript($sJsFullFilename = NULL, $bAlways = false) {
		...
		if (! file_exists ( $realJsFile ) || true == $bAlways) {
			...
			if (! file_exists ( $srcFile )) {
				trigger_error ( $this->objLanguageManager->getText ( 'CMPRSJS:RDERR:01' ) . dirname ( $realJsFile ) . $this->objLanguageManager->getText ( 'CMPRSJS:RDERR:02' ), E_USER_ERROR );
			}
				...
				if ('.' != $sName && '..' != $sName && is_dir ( $sFolder . '/' . $sName )) {
					...
					if ('.inc.php' == substr ( $sName, strlen ( $sName ) - 8, 8 )) {
						...
						if (file_exists ( $sPath )) {
							...
							$fH = @fopen ( $sNewPath, 'w' );
							if (! $fH) {
								trigger_error ( $this->objLanguageManager->getText ( 'CMPRSPHP:WTERR:01' ) . $sNewPath . $this->objLanguageManager->getText ( 'CMPRSPHP:WTERR:02' ), E_USER_ERROR );
							} else {
								fwrite ( $fH, $compressedScript );
								fclose ( $fH );
							}
			...
			while ( ! (false === ($sName = readdir ( $handle ))) ) {
				if ('.' != $sName && '..' != $sName && is_dir ( $sFolder . '/' . $sName )) {
					...
					if ('.inc.php' == substr ( $sName, strlen ( $sName ) - 8, 8 )) {
						$sName = substr ( $sName, 0, strlen ( $sName ) - 8 );
						$sPath = $sFolder . '/' . $sName . '.inc.php';
						if ('xajaxAIO' != $sName && 'xajaxCompress' != $sName) {
							if (file_exists ( $sPath )) {
