...
class GIFEncoder extends Captcha {
	...
	function GIFEncoder($GIF_src, $GIF_tim, $GIF_lop, $GIF_dis, $GIF_red, $GIF_grn, $GIF_blu, $GIF_mod) {
			...
			if (ord ( substr ( $GIF_buf [$i], $seek_p - 1, 1 ) ) != 0x21) {
				if ($transColor > - 1) {
					...
					for($j = 0; $j < (2 << (ord ( $GIF_buf [$i] {10} ) & 0x07)); $j ++) {
						if (ord ( $rgb {3 * $j} ) == (($transColor >> 16) & 0xFF) && ord ( $rgb {3 * $j + 1} ) == (($transColor >> 8) & 0xFF) && ord ( $rgb {3 * $j + 2} ) == (($transColor >> 0) & 0xFF)) {
							$tranid = true;
							break;
						}
					}
